/**
 * ******************************************************************** $Source:
 * /cvsroot/hibiscus/hbci4java/src/org/kapott/hbci/tools/UpdateBLZProperties.java,v $ $Revision: 1.1
 * $ $Date: 2012/04/16 22:24:40 $ $Author: willuhn $
 *
 ...
 * http://www.hbci-zka.de/institute/institut_hersteller.htm bezogen werden) parst, un die
 * aktualisierten Daten in die blz.properties von HBCI4Java uebernimmt.
 */
public class UpdateBLZProperties {
    private static final Charset CHARSET_INPUT = StandardCharsets.ISO_8859_1;
    private static final Charset CHARSET_OUTPUT = StandardCharsets.UTF_8;

    /**
     * @param args 1. Pfad/Dateiname zu "fints_institute.csv". 2. Pfad/Dateiname zu
     *     "blz.properties". 3. Pfad/Dateiname zur neuen "blz.properties". 4. optional:
     *     Pfad/Dateiname zur BLZ-Datei der Bundesbank. Falls angegeben, werden die eventuell
    ...
    public static void main(String[] args) throws Exception {
        if (args == null || args.length < 3 || args.length > 4) {
            System.err.println(
                    "benoetigte Parameter: 1) fints_institute.csv, 2) blz.properties, 3) zu schreibende blz.properties");
            System.err.println(
                    "optionaler Parameter: 4) BLZ_20160606.txt (BLZ-Update der Bundesbank)");
            System.exit(1);
        }
        ...
        try {
            ...
            f1 =
                    new BufferedReader(
                            new InputStreamReader(new FileInputStream(args[0]), CHARSET_INPUT));
            int count = 0;
            ...
            if (args.length == 4) {
                ...
                if (f.exists() && f.isFile() && f.canRead()) {
                    f4 =
                            new BufferedReader(
                                    new InputStreamReader(new FileInputStream(f), CHARSET_INPUT));
            ...
            f2 =
                    new BufferedReader(
                            new InputStreamReader(new FileInputStream(args[1]), CHARSET_OUTPUT));
            f3 =
                    new BufferedWriter(
                            new OutputStreamWriter(new FileOutputStream(args[2]), CHARSET_OUTPUT));
