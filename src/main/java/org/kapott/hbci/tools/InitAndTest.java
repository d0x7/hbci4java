...
/**
 * Tool zum Initialisieren und Testen eines HBCI-Passports. Dieses Tool dient einerseits als Vorlage
 * für die Benutzung von <em>HBCI4Java</em> in eigenen Anwendungen und gleichzeitig als Tool, um ein
 * HBCI-Passport einzurichten und zu initialisieren.
 *
 * <p>Für das Einrichten und Initialisieren eines HBCI-Passports gibt es keine speziellen
 * Funktionen, statt dessen wird ein Passport einfach so benutzt, als ob es schon vorhanden wäre.
 * Alle fehlenden Daten holt sich <em>HBCI4Java</em> selbstständig entweder über den
 * Callback-Mechanismus vom Anwender oder durch spezielle HBCI-Dialoge von der Bank.
 *
 * <p>Aus diesem Grund kann dieses Tool sowohl zum Testen eines schon existierenden Passports wie
 * auch zum Einrichten eines neuen Passports verwendet werden, ohne etwas am Programmcode zu ändern.
 * In der gleichen Weise wie dieses Tool kann auch eine komplexere Anwendung <em>HBCI4Java</em>
 * benutzen, in diesem Tool fehlt nur das Hinzufügen von Geschäftsvorfällen zum HBCI-Dialog.
 *
 * <p>Beim Start des Programmes mit <code>java&nbsp;org.kapott.hbci.tools.InitAndTest</code> werden
 * verschiedene Parameter über die Standardeingabe abgefragt. Konkret handelt es sich dabei um alle
 * HBCI-Parameter, die für die Initialisierung eines Passports und dessen Verwendung benötigt
 * werden. Die HBCI-Parameter werden mit den eingegeben Werten initialisiert. Anschließend wird ein
 * Passport-Objekt erzeugt und ein "leerer" HBCI-Dialog (d.h. einer, der keine Geschäftsvorfälle
 * enthält) ausgeführt.
 *
 * <p>Bei diesem Vorgang sorgt <em>HBCI4Java</em> selbst dafür, dass alle relevanten Daten vorhanden
 * sind bzw. initialisiert diese entspechend. Läuft das Programm fehlerfrei durch, so ist
 * sichergestellt, dass das benutzte Passport (=Sicherheitsmedium) korrekt initialisiert und
 * funktionsbereit ist.
 */
public final class InitAndTest {
    ...
    public static void main(String[] args) throws IOException {
        try {
            ...
            hbciHandle =
                    new HBCIHandler(
                            HBCIUtils.getParam("client.passport.hbciversion.default"), passport);
    ...
    private static void readParam(String paramName, String def, String descr) throws IOException {
        ...
        System.out.println(
                "press ENTER to accept the default; '-' to set no value for this parameter");
    ...
    private static void readBasicParams() throws IOException {
        readParam(
                ...
                null,
                "local tcp-port to be used for outgoing connections");
        readParam(
    ...
    private static void readHBCIVersion() throws IOException {
        ...
        readParam(
                "client.passport.hbciversion.default",
                ((pversion.length() != 0) ? pversion : "210"),
