 ...
 * <p>
 *
 * <p>Alle Jobs, bei deren Ausführung ein Fehler auftritt, werden nicht in die "normale"
 * Ausgabedatei aufgenommen. Statt dessen wird eine zweite Aus- gabedatei erzeugt, die den gleichen
 * Namen wie die "normale" Ausgabedatei plus ein Suffix ".err" hat. In dieser Fehlerdatei wird für
 * jeden fehler- haften Job folgende Struktur geschrieben (String in "<>" wird durch die jeweiligen
 * werte ersetzt):
 *
 * <pre>
 *    jobid:JOBID
 ...
 *  </pre>
 *
 * <p>das ist zwar nicht besonders schön, reicht aber vielleicht erst mal (?) Alternativ dazu könnte
 * ich anbieten, dass eine vollständige Fehlernachricht über den *kompletten* Batch-Vorgang in eine
 * Fehlerdatei geschrieben wird, sobald *irgendein* Job nicht sauber ausgeführt wurde (das hätte den
 * Vorteil, dass auch Fehler, die nicht direkt mit einem bestimmten Job in Verbindung stehen [z.B.
 * Fehler bei der Dialog-Initialisierung] ordentlich geloggt werden).
 */
public class HBCIBatch {
    private static final int STATE_NEED_JOBNAME = 1; // state-flags für
    private static final int STATE_NEED_JOBPARAMS = 2; //   batch-file-parser
            ...
            try {
                ...
                try {
                    ...
                    Hashtable<String, Object> jobs =
                            new Hashtable<String, Object>(); // liste aller jobs
                        ...
                        if (state == STATE_NEED_JOBNAME && line.length() != 0) {
                            ...
                            if (jobname.equals("--")) {
                                ...
                                if (jobname.startsWith("_")) {
                                    ...
                                    resultMode = "toString";
                                    customerId =
                                            (tok.hasMoreTokens() ? tok.nextToken().trim() : null);
                                } else {
                    ...
                    try {
                        ...
                        for (Enumeration<String> jobIds = jobs.keys(); jobIds.hasMoreElements(); ) {
                            ...
                            if (job.getJobResult().isOK()) {
                                ...
                                errWriter.println(
                                        job.getJobResult().getGlobStatus().getErrorString());
                                errWriter.println("job status:");
                                errWriter.println(
                                        job.getJobResult().getJobStatus().getErrorString());
                                errWriter.println();
                            }
                        }
                    } finally {
