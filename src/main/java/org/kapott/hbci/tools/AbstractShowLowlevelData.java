...
/**
 * Basisklasse für die beiden <code>ShowLowlevelGV*</code>-Tools. Diese Klasse wird nur intern
 * verwendet.
 */
public class AbstractShowLowlevelData {
    protected static void showData(String gvname, Document syntax, int minLevel) {
        ...
        System.out.println(
                "jobname:"
                        + gvname.substring(0, versionpos)
                        + " version:"
                        + gvname.substring(versionpos));
    ...
    protected static void displayContentRef(
            String path, Element ref, Document syntax, int indent, int currentLevel, int minLevel) {
        if (ref.getAttribute("type").length() != 0) {
            ...
            if (ref.getNodeName().equals("DE")) {
                ...
                if (currentLevel >= minLevel) {
                    ...
                    System.out.println(
                            spaces + "GROUP:" + name + " {" + minnum + "," + maxnum + "}");
