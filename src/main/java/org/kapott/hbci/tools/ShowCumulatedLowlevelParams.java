...
/**
 * Dieses Tool ist nur für interne Verwendung bei der Entwicklung von HBCI4Java gedacht. Damit lässt
 * sich prüfen, ob die Highlevel-Klassen für die einzelnen GVs tatsächlich alle möglichen
 * Lowlevel-Parameter eines bestimmten GV kennen und mit richtigen Werten füllen (jeweils abhängig
 * von den bankseitig unterstützten GV-Versionen)
 */
public class ShowCumulatedLowlevelParams {
    private static void extractParams(
            Document doc, Element node, String path, List<String> params) {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (child.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (refnode.equals("DE")) {
                    ...
                    if (def == null) {
                        // System.out.println("warning: type '"+reftype+"' referenced in
                        // '"+localPath+"' not
                        // found");
                        String p = localPath + " (+)";
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (segcode == null || segversion == null) {
                System.out.println(
                        "warning: SEGdef with id " + segdefid + " has no segcode or segversion");
                continue;
            }
