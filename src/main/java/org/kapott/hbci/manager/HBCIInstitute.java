...
public final class HBCIInstitute implements IHandlerData {
    ...
    void updateBPD(Properties result) {
        ...
        if (p.size() != 0) {
            ...
            passport.setBPD(p);
            HBCIUtils.log(
                    "installed new BPD with version " + passport.getBPDVersion(),
                    HBCIUtils.LOG_INFO);
            HBCIUtilsInternal.getCallback()
                    .status(passport, HBCICallback.STATUS_INST_BPD_INIT_DONE, passport.getBPD());
        }
    }
    ...
    void extractKeys(Properties result) {
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                byte[] keyExponent =
                        result.getProperty(head + ".PubKey.exponent").getBytes(Comm.ENCODING);
                byte[] keyModulus =
                        result.getProperty(head + ".PubKey.modulus").getBytes(Comm.ENCODING);

                KeyFactory fac = KeyFactory.getInstance("RSA");
                KeySpec spec =
                        new RSAPublicKeySpec(
                                new BigInteger(+1, keyModulus), new BigInteger(+1, keyExponent));
                Key key = fac.generatePublic(spec);

                if (keyType.equals("S")) {
                    passport.setInstSigKey(
                            new HBCIKey(keyCountry, keyBLZ, keyUserId, keyNum, keyVersion, key));
                    foundChanges = true;
                } else if (keyType.equals("V")) {
                    passport.setInstEncKey(
                            new HBCIKey(keyCountry, keyBLZ, keyUserId, keyNum, keyVersion, key));
                    foundChanges = true;
                }
            }
        ...
        if (foundChanges) {
            HBCIUtilsInternal.getCallback()
                    .status(passport, HBCICallback.STATUS_INST_GET_KEYS_DONE, null);
            acknowledgeNewKeys();
        }
    }
    ...
    private boolean isBPDExpired() {
        ...
        try {
            ...
            if (days > 0) maxMillis = days * 24 * 60 * 60 * 1000L;
        ...
        if (bpd != null) {
            ...
            HBCIUtils.log(
                    "[BPD] last update: " + (lastUpdate == 0 ? "never" : new Date(lastUpdate)),
                    HBCIUtils.LOG_INFO);
        }
    ...
    public void fetchBPD() {
        ...
        if (version.equals("0")
            ...
            try {
                ...
                // Bank den anonymen Abruf nicht unterstuetzt, wuerde dieser Abruf hier
                // fehlschlagen,
                // der erneute Versuch mit authentifiziertem Dialog wuerde jedoch nicht zum
                // Neuabruf der BPD fuehren, da dort (in HBCIUser#fetchUPD bzw.
                ...
                // "if" hier aus einem der anderen beiden o.g. Gruende (BPD-Expiry oder neue
                // HBCI-Version)
                // gelandet sind.
                if (!version.equals("0")) {
                    HBCIUtils.log(
                            "resetting BPD version from " + version + " to 0", HBCIUtils.LOG_INFO);
                    passport.getBPD().setProperty("BPA.version", "0");
                    passport.saveChanges();
                }

                HBCIUtilsInternal.getCallback()
                        .status(passport, HBCICallback.STATUS_INST_BPD_INIT, null);
                ...
                if (!status.isOK()) {
                    HBCIUtils.log(
                            "fetching BPD failed: " + status.getErrorString(), HBCIUtils.LOG_ERR);
                    throw new ProcessException(
                            HBCIUtilsInternal.getLocMsg("ERR_INST_BPDFAILED"), status);
                }
            } catch (Exception e) {
                if (e instanceof HBCI_Exception) {
                    HBCI_Exception he = (HBCI_Exception) e;
                    if (he.isFatal()) throw he;
                }
                HBCIUtils.log(e, HBCIUtils.LOG_INFO);
                // Viele Kreditinstitute unterstützen den anonymen Login nicht. Dass sollte nicht
                // als Fehler
                // den Anwender beunruhigen
                HBCIUtils.log(
        ...
        if (passport.getBPD() != null) {
            if (!passport.isSupported()) {
                ...
                if (!HBCIUtilsInternal.ignoreError(
                        null, "client.errors.ignoreSecMechCheckErrors", msg))
            ...
            if (!Arrays.asList(passport.getSuppVersions()).contains(kernel.getHBCIVersion(0))) {
                ...
                if (!HBCIUtilsInternal.ignoreError(
                        null, "client.errors.ignoreVersionCheckErrors", msg))
                    throw new InvalidUserDataException(msg);
            }
        } else {
            HBCIUtils.log(
                    "can not check if requested parameters are supported", HBCIUtils.LOG_WARN);
        }
    }
    ...
    public void fetchKeys() {
        ...
        if (passport.needInstKeys() && !passport.hasInstEncKey()) {
            ...
            try {
                HBCIUtilsInternal.getCallback()
                        .status(passport, HBCICallback.STATUS_INST_GET_KEYS, null);
                ...
                if (!status.isOK()) {
                    HBCIUtils.log(
                            "fetching institute keys failed: " + status.getErrorString(),
                            HBCIUtils.LOG_ERR);
                    throw new ProcessException(
                            HBCIUtilsInternal.getLocMsg("ERR_INST_GETKEYSFAILED"), status);
                }
            } catch (Exception e) {
