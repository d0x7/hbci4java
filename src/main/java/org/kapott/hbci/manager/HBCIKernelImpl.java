...
public final class HBCIKernelImpl implements HBCIKernel {
    ...
    public void rawSet(String key, String value) {
        HBCIUtils.log(
                "setting raw property " + currentMsgName + "." + key + " to \"" + value + "\"",
                HBCIUtils.LOG_DEBUG2);

        if (currentMsgName == null)
            throw new HBCI_Exception(HBCIUtilsInternal.getLocMsg("EXCMSG_NORAWMSG"));
        if (key == null) throw new HBCI_Exception(HBCIUtilsInternal.getLocMsg("EXCMSG_KEYNULL"));
        if (value == null)
    ...
    public HBCIMsgStatus rawDoIt(
            boolean signit, boolean cryptit, boolean needSig, boolean needCrypt) {
    ...
    public HBCIMsgStatus rawDoIt(
        ...
        try {
            ...
            if (signit) {
                ...
                try {
                    if (!sig.signIt()) {
                        ...
                        if (!HBCIUtilsInternal.ignoreError(
                                null, "client.errors.ignoreSignErrors", errmsg)) {
                ...
                // alle rewrites erledigen, die *nach* dem hinzufügen der signatur stattfinden
                // müssen
                for (int i = 0; i < rewriters.length; i++) {
                    MSG old = msg;
            ...
            if (maxmsgsize != 0 && (outstring.length() >> 10) > maxmsgsize) {
                String errmsg =
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_MSGTOOLARGE",
                                new Object[] {
                                    Integer.toString(outstring.length() >> 10),
                                    Integer.toString(maxmsgsize)
                                });
                if (!HBCIUtilsInternal.ignoreError(
                        null, "client.errors.ignoreMsgSizeErrors", errmsg))
                    throw new HBCI_Exception(errmsg);
            }
            ...
            if (cryptit) {
                ...
                if (!msg.getName().equals("Crypted")) {
                    ...
                    if (!HBCIUtilsInternal.ignoreError(
                            null, "client.errors.ignoreCryptErrors", errmsg))
                        throw new HBCI_Exception(errmsg);
                }
                ...
                HBCIUtils.log(
                        "encrypted message to be sent: " + msg.toString(0), HBCIUtils.LOG_DEBUG2);
            }
            ...
            HBCIUtils.log(
                    "communicating dialogid/msgnum " + dialogid + "/" + msgnum,
                    HBCIUtils.LOG_DEBUG);
            MSG old = msg;
            ...
            if (crypted) {
                ...
                for (int i = 0; i < rewriters.length; i++) {
                    HBCIUtils.log(
                            "applying rewriter " + rewriters[i].getClass().getSimpleName(),
                            HBCIUtils.LOG_DEBUG);
                ...
                HBCIUtils.log(
                        "decrypted message after rewriting: " + newmsgstring, HBCIUtils.LOG_DEBUG2);
            ...
            HBCIUtils.log(
                    "received message after decryption: " + msg.toString(0), HBCIUtils.LOG_DEBUG2);
            ...
            // überprüfen einiger constraints, die in einer antwortnachricht eingehalten werden
            // müssen
            msgPath = msg.getPath();
            try {
                ...
                if (!hbciversion2.equals(hbciversion))
                    throw new HBCI_Exception(
                            HBCIUtilsInternal.getLocMsg(
                                    "EXCMSG_INVVERSION", new Object[] {hbciversion2, hbciversion}));
                String msgnum2 = msg.getValueOfDE(msgPath + ".MsgHead.msgnum");
                if (!msgnum2.equals(msgnum))
                    throw new HBCI_Exception(
                            HBCIUtilsInternal.getLocMsg(
                                    "EXCMSG_INVMSGNUM_HEAD", new Object[] {msgnum2, msgnum}));
                msgnum2 = msg.getValueOfDE(msgPath + ".MsgTail.msgnum");
                if (!msgnum2.equals(msgnum))
                    throw new HBCI_Exception(
                            HBCIUtilsInternal.getLocMsg(
                                    "EXCMSG_INVMSGNUM_TAIL", new Object[] {msgnum2, msgnum}));
                String dialogid2 = msg.getValueOfDE(msgPath + ".MsgHead.dialogid");
                if (!dialogid.equals("0") && !dialogid2.equals(dialogid))
                    throw new HBCI_Exception(
                            HBCIUtilsInternal.getLocMsg(
                                    "EXCMSG_INVDIALOGID", new Object[] {dialogid2, dialogid}));
                if (!dialogid.equals("0")
                        && !msg.getValueOfDE(msgPath + ".MsgHead.MsgRef.dialogid").equals(dialogid))
                ...
                String errmsg =
                        HBCIUtilsInternal.getLocMsg("EXCMSG_MSGCHECK")
                                + ": "
                                + HBCIUtils.exception2String(e);
                if (!HBCIUtilsInternal.ignoreError(
                        null, "client.errors.ignoreMsgCheckErrors", errmsg)) throw e;
            }
            ...
            if (currentMsgName.startsWith("DialogEnd")
                ...
                HBCIUtils.log(
                        "error while receiving DialogEnd response - "
