...
public final class HBCIDialog {
    ...
    private List<ArrayList<HBCIJobImpl>>
            msgs; /* this array contains all messages to be sent (excluding
    ...
    private Properties
            listOfGVs; // liste aller GVs in der aktuellen msg; key ist der hbciCode des jobs,
     ...
     *     (mid-level API).
     *     <p>This method processes the dialog initialization stage of an HBCIDialog. It creates a
     *     new rawMsg in the kernel and processes it. The return values will be passed to
     *     appropriate methods in the @c institute and @c user objects to update their internal
     *     state with the data received from the institute.
     */
    private HBCIMsgStatus doDialogInit() {
        ...
        try {
            HBCIPassportInternal mainPassport =
                    (HBCIPassportInternal) getParentHandler().getPassport();
            ...
            HBCIUtilsInternal.getCallback()
                    .status(mainPassport, HBCICallback.STATUS_DIALOG_INIT, null);
            ...
            while (true) {
                ...
                if (!isAnon) {
                    ...
                    if (mainPassport.needInstKeys()) {
                        ...
                        kernel.rawSet(
                                "KeyReq.SecProfile.version", mainPassport.getProfileVersion());
                        ...
                        kernel.rawSet(
                                "KeyReq.KeyName.keyversion", mainPassport.getInstEncKeyVersion());

                        if (mainPassport.hasInstSigKey()) {
                            kernel.rawSet(
                                    "KeyReq_2.SecProfile.method", mainPassport.getProfileMethod());
                            kernel.rawSet(
                                    "KeyReq_2.SecProfile.version",
                                    mainPassport.getProfileVersion());
                            ...
                            kernel.rawSet(
                                    "KeyReq_2.KeyName.userid", mainPassport.getInstSigKeyName());
                            kernel.rawSet(
                                    "KeyReq_2.KeyName.keynum", mainPassport.getInstSigKeyNum());
                            kernel.rawSet(
                                    "KeyReq_2.KeyName.keyversion",
                                    mainPassport.getInstSigKeyVersion());
                        }
                    }
                }
    ...
    private HBCIMsgStatus[] doJobs() {
        ...
        for (int j = 0; j < nof_messages; j++) {
            ...
            while (true) {
                ...
                try {
                    HBCIUtils.log(
                            "generating custom msg #" + (j + 1) + " (loop " + (loop + 1) + ")",
                            HBCIUtils.LOG_DEBUG);
                    ...
                    for (Iterator<HBCIJobImpl> i = tasks.iterator(); i.hasNext(); ) {
                        ...
                        if (task.needsContinue(loop)) {
                            ...
                            HBCIUtilsInternal.getCallback()
                                    .status(mainPassport, HBCICallback.STATUS_SEND_TASK, task);
                            task.setIdx(taskNum);
                    ...
                    if (taskNum == 0) {
                        HBCIUtils.log(
                                "loop "
                                        + (loop + 1)
                                        + " aborted, because there are no more tasks to be executed",
                                HBCIUtils.LOG_DEBUG);
                        addMsgStatus = false;
                        break;
                    }
                    ...
                    if (offset != 0) {
                        ...
                        // in fillOutStore wird auch "executed" fuer den jeweiligen Task auf true
                        // gesetzt.
                        for (Iterator<HBCIJobImpl> i = tasks.iterator(); i.hasNext(); ) {
                            HBCIJobImpl task = i.next();
                            if (task.needsContinue(loop)) {
                                ...
                                try {
                                    ...
                                    HBCIUtilsInternal.getCallback()
                                            .status(
                                                    mainPassport,
                                                    HBCICallback.STATUS_SEND_TASK_DONE,
                                                    task);
                                } catch (Exception e) {
        ...
        if (msgstatus_a.size() != 0) ret = (msgstatus_a.toArray(ret));
     ...
     *     <p>Works similarily to doDialogInit().
     */
    private HBCIMsgStatus doDialogEnd() {
        ...
        try {
            ...
            HBCIUtilsInternal.getCallback()
                    .status(mainPassport, HBCICallback.STATUS_DIALOG_END, null);
            ...
            HBCIUtilsInternal.getCallback()
                    .status(mainPassport, HBCICallback.STATUS_DIALOG_END_DONE, ret);
        } catch (Exception e) {
            ret.addException(e);
        }
     ...
     *
     * <p>bricht diese methode mit einer exception ab, so muessen alle nachrichten bzw. tasks, die
     * noch nicht ausgeführt wurden, von der aufrufenden methode neu erzeugt werden
     */
    ...
    private int getTotalNumberOfGVSegsInCurrentMessage() {
        ...
        HBCIUtils.log(
                "there are currently " + total + " GV segs in this message", HBCIUtils.LOG_DEBUG);
        return total;
    }
    ...
    public void addTask(HBCIJobImpl job) {
        ...
        try {
            HBCIUtils.log(
                    HBCIUtilsInternal.getLocMsg("EXCMSG_ADDJOB", job.getName()),
                    HBCIUtils.LOG_INFO);
            ...
            if ((maxGVA > 0 && gva_counter > maxGVA)
                ...
                if (maxGVSegTotal > 0 && total_counter > maxGVSegTotal) {
                    HBCIUtils.log(
                            "have to generate new message because current type of passport only allows "
                                    + maxGVSegTotal
                                    + " GV segs per message",
                            HBCIUtils.LOG_DEBUG);
                } else {
            ...
            HBCIUtils.log(
                    "task " + job.getName() + " will not be executed in current dialog",
                    HBCIUtils.LOG_ERR);
        }
    }
