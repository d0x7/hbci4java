/**
 * ******************************************************************** $Source:
 * /cvsroot/hibiscus/hbci4java/src/org/kapott/hbci/manager/FlickerCode.java,v $ $Revision: 1.9 $
 * $Date: 2011/06/24 16:53:23 $ $Author: willuhn $
 *
...
/**
 * Implementierung des Flicker-Codes fuer optisches ChipTAN. Basiert auf der
 * Javascript-Implementierung von http://6xq.net/media/00/20/flickercode.html
 *
 * <p>Die Javascript-Implementierung war jedoch nicht mehr aktuell (basiert auf HHD 1.3).
 */
public class FlickerCode {
    /**
     * Die Anzahl der Bytes, in der die Laenge des Challenge bei HHD 1.4 steht. Bei HHD 1.3 war das
     * noch 2 Zeichen lang. Wenn der Flicker-Code nicht in "Challenge HHDuc" uebertragen wurde
     * sondern direkt im Freitext-Challenge, koennen wir das Problem umgehen, indem wir in clean()
     * einfach eine "0" vorn anhaengen. Wenn er aber tatsaechlich im "Challenge HHDuc" steht, kann
     * man dem Code nicht ansehen, ob es ein HHD 1.3-Code ist. In dem Fall hilft nur Try&Error. Also
     * mit HHD 1.4 parsen. Und wenn das fehlschlaegt, dann HHD 1.3 versuchen.
     */
    ...
    private static final int LDE_LENGTH_DEFAULT = 2;
    /** Fallback-Laenge der LDE-Laengen-Angabe bei der Sparda. */
    private static final int LDE_LENGTH_SPARDA = 3;
    ...
    /**
     * Wandelt die Zahl in Hex-Schreibweise um und fuellt links mit Nullen auf, bis die Laenge "len"
     * erreicht ist.
     *
     * @param n die Zahl.
    ...
    /**
     * Liefert die Summe der Bit-Wertigkeiten fuer die genannten Bits (beginndend bei 0 und beim
     * kleinsten Bit, angegebens inclusive).
     *
     * <p>Beispiel: num = 156 (-> 10011100) bits = 5
     *
     * <p>Es wird die Summe der Bitwertigkeiten 2^0 bis 2^5 errechnet. Also der Wert von **011100 =
     * 2^4+2^3+s^2 = 28
     *
     * @param num Zahl, aus der die Summe berechnet werden soll.
     * @param bits Anzahl der Bits (beginnend bei 0 und beim kleinsten Bit, angegebenes inclusive),
     *     deren Wertigkeit addiert werden soll.
     * @return der errechnete Wert.
     */
    private static int getBitSum(int num, int bits) {
        int sum = 0;
        for (int i = 0; i <= bits; ++i) sum += (num & (1 << i));
        return sum;
    }
     ...
     * @param bit die Nummer des zu pruefenden Bits. Wobei "0" das kleinste (rechts) und "7" das
     *     groesste (links) Bit ist.
    ...
    /**
     * Entfernt das CHLGUC0026....CHLGTEXT aus dem Code, falls vorhanden. Das sind HHD 1.3-Codes,
     * die nicht im "Challenge HHDuc" uebertragen wurden sondern direkt im Challenge-Freitext,
     *
     * @param code
     * @return
     */
    ...
    private String createLuhnChecksum() {
        ...
        // a) Controlbytes
        for (Integer i : this.startCode.controlBytes) sb.append(toHex(i, 2));
        ...
        for (i = 0; i < payload.length(); i += 2) {
            luhnsum +=
                    (1 * Integer.parseInt(Character.toString(payload.charAt(i)), 16))
                            + quersumme(
                                    2
                                            * Integer.parseInt(
                                                    Character.toString(payload.charAt(i + 1)), 16));
        }
        ...
        // also 129 + 1 = 130
        int mod = luhnsum % 10;
        if (mod == 0) return "0"; // Siehe "Schritt 3" in tan_hhd_uc_v14.pdf, Seite 17
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof FlickerCode)) return false;
        ...
        if (this.rest == null) return (other.rest == null);
    ...
    public class DE {
        ...
        /**
         * Die Laengen-Angabe des DE im Roh-Format. Sie kann noch Control-Bits enthalten, sollte
         * daher also NICHT fuer Laengenberechnungen verwendet werden. In dem Fall stattdessen
         * <code>length
         * </code> verwenden.
         */
        public int lde = 0;
        ...
        /**
         * Das Encoding der Nutzdaten. Per Definition ist im Challenge HHDuc dieses Bit noch NICHT
         * gesetzt. Das Encoding passiert erst beim Rendering.
         */
         ...
         *
         * @return die codierten Nutzdaten. Wenn das DE keine Nutzdaten enthaelt, wird "" zurueck
         *     gegeben.
         */
        String renderLength() {
            ...
            if (enc == Encoding.BCD) return toHex(len, 2);
        ...
        /**
         * Liefert das zu verwendende Encoding fuer die Uebertragung via Flickercode. Im Normalfall
         * (also nach dem Parsen eines HHDuc) ist kein Encoding angegeben (im Challenge HHDuc ist
         * das per Definition nie gesetzt) machen wir ASC. Es sei denn, das Encoding wurde explizit
         * auf BCD gesetzt.
         *
         * @return das fuer das Rendering zu verwendende Encoding.
         */
        Encoding getEncoding() {
            if (this.data == null) return Encoding.BCD;
            ...
            if (this.data.matches("[0-9]{1,}")) return Encoding.BCD;
        ...
        String renderData() {
            ...
            Encoding enc = this.getEncoding();
            if (enc == Encoding.ASC) return toHex(this.data);
    ...
    /**
     * Bean fuer die Eigenschaften des Startcodes. Selbstverstaendlich sind hier so einige Sachen
     * anders codiert als im DE. Waer ja auch zu einfach sonst. Die Laengen-Angabe ist anders
     * codiert (hex statt dec). Und nach der Laenge kommen nicht sofort die Nutzdaten sondern erst
     * noch die Control-Bytes.
     */
    public class Startcode extends DE {
        ...
        String parse(String s) {
            ...
            if (isBitSet(this.lde, BIT_CONTROLBYTE)) {
                ...
                for (int i = 0; i < 10; ++i) {
                    ...
                    // folgen weitere
                    if (!isBitSet(controlByte, BIT_CONTROLBYTE)) break;
                }
            }
        ...
        /**
         * @see org.kapott.hbci.manager.FlickerCode.DE#renderLength() Ueberschrieben, weil wir hier
         *     noch reincodieren muessen, ob ein Controlbyte folgt.
         */
        String renderLength() {
            String s = super.renderLength();

            // HHD 1.3 -> gibt keine Controlbytes
            if (FlickerCode.this.version == HHDVersion.HHD13) return s;

            // HHD 1.4 -> aber keine Controlbytes vorhanden
            if (this.controlBytes.size() == 0) return s;

            // Controlbytes reincodieren
            int len = Integer.parseInt(s, 16);

            if (this.controlBytes.size() > 0) len += (1 << BIT_CONTROLBYTE);
        ...
        public boolean equals(Object obj) {
            if (!(obj instanceof Startcode)) return false;
            return this.toString().equals(obj.toString());
        }
    }
    //////////////////////////////////////////////////////////////////////////////
