...
public class AccountCRCAlgs {
    ...
    public static boolean alg_26(int[] blz, int[] number) {
        int startpos = 0;
        if (number[0] == 0 && number[1] == 0) startpos = 2;

        int sum =
                addProducts(number, startpos, startpos + 6, new int[] {2, 7, 6, 5, 4, 3, 2}, false);
        int crc = 11 - sum % 11;
        if (crc > 9) crc = 0;
    ...
    public static boolean alg_52(int[] blz, int[] number) {
        ...
        if (number[0] == 9) {
            ...
            // Kontonummer muss 8-stellig sein (mgl: number[2] darf 0 sein, d.h. Kontonummer faengt
            // mit 0
            // an
    ...
    public static boolean alg_53(int[] blz, int[] number) {
        ...
        if (number[0] == 9) {
            ...
            // Kontonummer muss 9-stellig sein (mgl: number[1] darf 0 sein, d.h. Kontonummer faengt
            // mit 0
            // an?)
    ...
    public static boolean alg_88(int[] blz, int[] number) {
        int sum = 0;
        if (number[2] == 9) sum = addProducts(number, 2, 8, new int[] {8, 7, 6, 5, 4, 3, 2}, false);
        else sum = addProducts(number, 3, 8, new int[] {7, 6, 5, 4, 3, 2}, false);
        int crc = 11 - sum % 11;
        if (crc > 9) crc = 0;
        return number[9] == crc;
    }
    ...
    public static boolean alg_96(int[] blz, int[] number) {
        ...
        if (!(ret = alg_19(blz, number)) && !(ret = alg_00(blz, number))) {
            long bigint = 0;
            if (number[0] == 0) bigint = calculateIntFromNumber(number);
            if (bigint >= 1300000 && bigint <= 99399999) ret = true;
        }
        return ret;
    }
    ...
    private static int addProducts(
            int[] number, int first, int last, int[] factors, boolean withChecksum) {
        int result = 0;
        for (int i = first; i <= last; i++) {
            int prod = number[i] * factors[i - first];
            if (withChecksum) prod = quersumme(prod, false);
            result += prod;
        }
        return result;
    }
