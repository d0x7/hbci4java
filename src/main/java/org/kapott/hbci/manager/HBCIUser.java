...
public final class HBCIUser implements IHandlerData {
    ...
    private void doDialogEnd(
            String dialogid, String msgnum, boolean signIt, boolean cryptIt, boolean needCrypt) {
        ...
        HBCIUtilsInternal.getCallback()
                .status(passport, HBCICallback.STATUS_DIALOG_END_DONE, status);
        ...
        if (!status.isOK()) {
            ...
            if (!HBCIUtilsInternal.ignoreError(
                    ...
                    msg + ": " + status.getErrorString())) throw new ProcessException(msg, status);
        }
    }
    ...
    private void sendAndActivateNewUserKeys(HBCIKey[] sigKey, HBCIKey[] encKey) {
        try {
            ...
            if (!passport.hasMySigKey()) {
                ...
                HBCIUtilsInternal.getCallback()
                        .status(passport, HBCICallback.STATUS_SEND_KEYS, null);
                ...
                HBCIMsgStatus ret =
                        kernel.rawDoIt(
                                HBCIKernelImpl.SIGNIT,
                                HBCIKernelImpl.CRYPTIT,
                                HBCIKernelImpl.NEED_SIG,
                ...
                HBCIUtilsInternal.getCallback()
                        .status(passport, HBCICallback.STATUS_SEND_KEYS_DONE, ret);
                ...
                if (!ret.isOK()) {
                    ...
                    throw new ProcessException(
                            HBCIUtilsInternal.getLocMsg("EXCMSG_SENDKEYERR"), ret);
                }

                try {
                    doDialogEnd(
                            result.getProperty("MsgHead.dialogid"),
                            "2",
                            HBCIKernelImpl.DONT_SIGNIT,
                            HBCIKernelImpl.CRYPTIT,
                            HBCIKernelImpl.DONT_NEED_CRYPT);
                } catch (Exception e) {
                    HBCIUtils.log(e);
                ...
                HBCIUtilsInternal.getCallback()
                        .status(passport, HBCICallback.STATUS_DIALOG_INIT, null);
                ...
                while (true) {
                    ...
                    kernel.rawSet(
                            "ProcPrep.prodName",
                            HBCIUtils.getParam("client.product.name", HBCIUtils.PRODUCT_ID));
                    kernel.rawSet(
                            "ProcPrep.prodVersion",
                            HBCIUtils.getParam("client.product.version", "3"));
                    ret =
                            kernel.rawDoIt(
                                    HBCIKernelImpl.SIGNIT,
                                    HBCIKernelImpl.CRYPTIT,
                    ...
                    if (need_restart) {
                        ...
                        if (restarted) {
                            HBCIUtils.log(
                                    "this dialog already has been restarted once - to avoid endless loops we stop here",
                                    HBCIUtils.LOG_WARN);
                ...
                if (!ret.isOK())
                    throw new ProcessException(
                            HBCIUtilsInternal.getLocMsg("EXCMSG_GETUPDFAIL"), ret);
                ...
                HBCIUtilsInternal.getCallback()
                        .status(
                                passport,
                                HBCICallback.STATUS_DIALOG_INIT_DONE,
        ...
        if (passport.needUserKeys()) {
            ...
            try {
                ...
                for (int i = 0; i < 2; i++) {
                    if (i == 0) {
                        String num =
                                passport.hasMySigKey() ? passport.getMyPublicSigKey().num : "0";
                        num = Integer.toString(Integer.parseInt(num) + 1);
                        newSigKey[0] =
                                new HBCIKey(country, blz, userid, num, "1", sigKey.getPublic());
                        newSigKey[1] =
                                new HBCIKey(country, blz, userid, num, "1", sigKey.getPrivate());
                    } else {
                        String num =
                                passport.hasMyEncKey() ? passport.getMyPublicEncKey().num : "0";
                        num = Integer.toString(Integer.parseInt(num) + 1);
                        newEncKey[0] =
                                new HBCIKey(country, blz, userid, num, "1", encKey.getPublic());
                        newEncKey[1] =
                                new HBCIKey(country, blz, userid, num, "1", encKey.getPrivate());
                    }
                }
            } catch (Exception ex) {
        ...
        try {
            ...
            while (true) {
                ...
                kernel.rawSet(
                        ...
                        HBCIUtils.getParam("client.product.name", HBCIUtils.PRODUCT_ID));
                kernel.rawSet(
                        "ProcPrep.prodVersion", HBCIUtils.getParam("client.product.version", "3"));
                ...
                if (need_restart) {
                    HBCIUtils.log(
                            "for some reason we have to restart this dialog", HBCIUtils.LOG_INFO);
                    if (restarted) {
                        HBCIUtils.log(
                                "this dialog already has been restarted once - to avoid endless loops we stop here",
                                HBCIUtils.LOG_WARN);
            ...
            if (!ret.isOK())
                throw new ProcessException(
                        HBCIUtilsInternal.getLocMsg("EXCMSG_SYNCSYSIDFAIL"), ret);
            ...
            HBCIUtilsInternal.getCallback()
                    .status(
                            passport,
                            HBCICallback.STATUS_INIT_SYSID_DONE,
                            new Object[] {ret, passport.getSysId()});
    ...
    public void fetchSigId() {
        try {
            ...
            if (!ret.isOK())
                throw new ProcessException(
                        HBCIUtilsInternal.getLocMsg("EXCMSG_SYNCSIGIDFAIL"), ret);
            ...
            HBCIUtilsInternal.getCallback()
                    .status(
                            passport,
                            HBCICallback.STATUS_INIT_SIGID_DONE,
                            new Object[] {ret, passport.getSigId()});
    ...
    public void updateUPD(Properties result) {
        ...
        if (p.size() != 0) {
            ...
            // Wir sichern wenigstens noch die TAN-Media-Infos, die vom HBCIHandler vorher abgerufen
            // wurden
            // Das ist etwas unschoen. Sinnvollerweise sollten die SEPA-Infos und TAN-Medien nicht
            // in den
            // UPD gespeichert werden. Dann gehen die auch nicht jedesmal wieder verloren und
            // muessen
            // nicht
            ...
            if (upd != null) {
                ...
                if (mediaInfo != null) {
                    HBCIUtils.log(
                            "rescued TAN media info to new UPD: " + mediaInfo, HBCIUtils.LOG_INFO);
            ...
            HBCIUtils.log(
                    "installed new UPD [old version: "
                            + oldVersion
                            + ", new version: "
                            + passport.getUPDVersion()
                            + "]",
                    HBCIUtils.LOG_INFO);
            HBCIUtilsInternal.getCallback()
                    .status(passport, HBCICallback.STATUS_INIT_UPD_DONE, passport.getUPD());
        }
    }
    ...
    public void fetchUPD() {
        try {
            ...
            HBCIUtils.log(
                    "fetching UPD (BPD-Version: " + passport.getBPDVersion() + ")",
                    HBCIUtils.LOG_INFO);
    ...
    private void updateUserData() {
        if (passport.getSysStatus().equals("1")) {
            if (passport.getSysId().equals("0")) fetchSysId();
            if (passport.getSigId().longValue() == -1) fetchSigId();
        }
    ...
    public void lockKeys() {
        ...
        try {
            ...
            while (true) {
                ...
                boolean need_restart =
                        passport.postInitResponseHook(status, passport.isAnonymous());
                if (need_restart) {
                    HBCIUtils.log(
                            "for some reason we have to restart this dialog", HBCIUtils.LOG_INFO);
                    if (restarted) {
            ...
            if (!status.isOK())
                throw new ProcessException(
                        HBCIUtilsInternal.getLocMsg("EXCMSG_LOCKFAILED"), status);
            ...
            HBCIUtilsInternal.getCallback()
                    .status(passport, HBCICallback.STATUS_LOCK_KEYS_DONE, status);
            doDialogEnd(
                    dialogid,
                    "3",
                    HBCIKernelImpl.DONT_SIGNIT,
