...
/**
 * Diese Klasse ist die Super-Klasse aller Exceptions, die durch den HBCI-Kernel erzeugt werden.
 * Beim Auftreten einer solchen Exception sollten die Messages der gesamten(!) Exception-Kette
 * angezeigt werden, um die Fehlerursache bestmöglich bestimmen zu können.
 *
 * <pre>
 * try {
...
public class HBCI_Exception extends RuntimeException {
    ...
    /**
     * Erzeugen einer neuen HBCI_Exception mit bestimmtem Cause. Die Message, die in dieser
     * Exception gespeichert wird, ist auf jeden Fall leer
     *
     * @param e "Ursache" dieser Exception, die in der Exception-Kette als <code>getCause()</code>
    ...
    public boolean isFatal() {
        ...
        if (t == this) return false; // sind wir selbst
        if (t instanceof HBCI_Exception) return ((HBCI_Exception) t).isFatal();
