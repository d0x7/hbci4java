...
public final class Sig {
    ...
    private void fillSigHead(SEG sighead) {
        ...
        sighead.propagateValue(
                ...
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".SecIdnDetails.func",
                (msg.getName().endsWith("Res") ? "2" : "1"),
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        if (u_cid.length() != 0) {
            // DDV
            sighead.propagateValue(
                    sigheadName + ".SecIdnDetails.cid",
                    "B" + u_cid,
                    SyntaxElement.DONT_TRY_TO_CREATE,
                    SyntaxElement.DONT_ALLOW_OVERWRITE);
        } else {
            // RDH und PinTan
            sighead.propagateValue(
                    sigheadName + ".SecIdnDetails.sysid",
                    u_sysid,
                    SyntaxElement.DONT_TRY_TO_CREATE,
                    SyntaxElement.DONT_ALLOW_OVERWRITE);
        }
        sighead.propagateValue(
                sigheadName + ".SecTimestamp.date",
                HBCIUtils.date2StringISO(d),
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".SecTimestamp.time",
                HBCIUtils.time2StringISO(d),
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        ...
        sighead.propagateValue(
                sigheadName + ".HashAlg.alg",
                u_hashalg,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".SigAlg.alg",
                u_sigalg,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".SigAlg.mode",
                u_sigmode,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);

        sighead.propagateValue(
                sigheadName + ".KeyName.KIK.country",
                u_keycountry,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".KeyName.KIK.blz",
                u_keyblz,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".KeyName.userid",
                u_keyuserid,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".KeyName.keynum",
                u_keynum,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".KeyName.keyversion",
                u_keyversion,
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);

        sighead.propagateValue(
                sigheadName + ".SecProfile.method",
                passports.getMainPassport().getProfileMethod(),
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(
                sigheadName + ".SecProfile.version",
                passports.getMainPassport().getProfileVersion(),
                SyntaxElement.DONT_TRY_TO_CREATE,
                SyntaxElement.DONT_ALLOW_OVERWRITE);
    }
    ...
    private void fillSigTail(SEG sighead, SEG sigtail) {
        ...
        sigtail.propagateValue(
                sigtailName + ".seccheckref",
    ...
    private String collectHashData(int idx) {
        ...
        List<SyntaxElement> sigtails =
                ((MultipleSEGs) (msgelementslist.get(msgelementslist.size() - 2))).getElements();

        // alle benötigten sighead-segmente zusammensuchen
        for (int i = numOfPassports - 1 - idx;
                i < (u_range.equals("1") ? (numOfPassports - idx) : numOfPassports);
                i++) {
            ...
            if (dontsignAttr.length() == 0) {
                try {
                    ...
                    for (int idx = 0; idx < numOfPassports; idx++) {
                        ...
                        List<SyntaxElement> sigheads =
                                ((MultipleSEGs) (msgelements.get(1))).getElements();
                        List<SyntaxElement> sigtails =
                                ((MultipleSEGs) (msgelements.get(msgelements.size() - 2)))
                                        .getElements();

                        // insert sighead segment in msg
                        if ((numOfPassports - 1 - idx) < sigheads.size()) {
                            SEGFactory.getInstance()
                                    .unuseObject(sigheads.get(numOfPassports - 1 - idx));
                        } else {
                    ...
                    for (int idx = 0; idx < numOfPassports; idx++) {
                        ...
                        if (passport.needUserSig()) {
                            ...
                            msg.propagateValue(
                                    sigtail.getPath() + ".sig",
                                    "B" + new String(signature, Comm.ENCODING),
                                    SyntaxElement.DONT_TRY_TO_CREATE,
                                    SyntaxElement.DONT_ALLOW_OVERWRITE);
                        }
                ...
                HBCIUtils.log(
                        "did not sign - message does not want to be signed", HBCIUtils.LOG_DEBUG);
    ...
    private void readSigHead() {
        ...
        */
        ...
        // Die Angabe der BLZ ist nicht unbedingt verpflichtend (für 280 aber schon...). Trotzdem
        // gibt
        // es wohl
        ...
        try {
            ...
            HBCIUtils.log(
                    "missing bank code in message signature, ignoring...", HBCIUtils.LOG_WARN);
        }
    ...
    private boolean hasSig() {
        ...
        if (seglist instanceof MultipleSEGs) {
            ...
            if (ret) {
                ...
                if (!sighead.getCode(gen).equals(sigheadCode)) ret = false;
            }
        } else ret = false;
    ...
    public boolean verify() {
        ...
        if (mainPassport.hasInstSigKey()) {
            ...
            HBCIUtils.log(
                    "can not check signature - no signature key available", HBCIUtils.LOG_WARN);
            ret = true;
        }
