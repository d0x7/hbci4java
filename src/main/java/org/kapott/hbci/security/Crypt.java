        ...
        if (passport.hasInstEncKey()) {
            ...
            if (dontcryptAttr.length() == 0) {
                try {
                    ...
                    gen.set(
                            newName + ".CryptData.data",
                            "B" + new String(crypteds[1], Comm.ENCODING));
                    gen.set(newName + ".CryptHead.CryptAlg.alg", u_alg);
                    gen.set(newName + ".CryptHead.CryptAlg.mode", u_mode);
                    gen.set(
                            newName + ".CryptHead.CryptAlg.enckey",
                            "B" + new String(crypteds[0], Comm.ENCODING));
                    gen.set(newName + ".CryptHead.CryptAlg.keytype", u_keytype);
                    gen.set(
                            newName + ".CryptHead.SecIdnDetails.func",
                            (newmsg.getName().endsWith("Res") ? "2" : "1"));
                    ...
                    gen.set(
                            newName + ".CryptHead.SecProfile.version",
                    ...
                    for (int i = 1; i <= 2; i++) {
                        SEG seg =
                                (SEG)
                                        (((MultipleSEGs) ((newmsg.getChildContainers()).get(i)))
                                                .getElements()
                                                .get(0));
                        seg.setSeq(997 + i, SyntaxElement.ALLOW_OVERWRITE);
                    }

                    newmsg.propagateValue(
                            newmsg.getPath() + ".MsgTail.SegHead.seq",
                            segnum,
                ...
                HBCIUtils.log(
                        "did not encrypt - message does not want to be encrypted",
                        HBCIUtils.LOG_DEBUG);
    ...
    private boolean isCrypted() {
        ...
        if (seglist instanceof MultipleSEGs) {
            ...
            if (ret) {
                ...
                if (!crypthead.getCode(gen).equals(sigheadCode)) ret = false;
            }
        } else ret = false;
        ...
        if (passport.hasMyEncKey()) {
            if (isCrypted()) {
                try {
                    ...
                    SEG msgtail =
                            (SEG)
                                    (((MultipleSEGs) (childs.get(childs.size() - 1)))
                                            .getElements()
                                            .get(0));
                    ...
                    byte[] cryptedstring =
                            cryptdata
                                    .getValueOfDE(msgName + ".CryptData.data")
                                    .getBytes(Comm.ENCODING);

                    // key extrahieren
                    SEG crypthead = (SEG) (((MultipleSEGs) (childs.get(1))).getElements().get(0));
                    byte[] cryptedkey =
                            crypthead
                                    .getValueOfDE(msgName + ".CryptHead.CryptAlg.enckey")
                                    .getBytes(Comm.ENCODING);
                    ...
                    if (!secfunc.equals(passport.getCryptFunction())) {
                        String errmsg =
                                HBCIUtilsInternal.getLocMsg(
                                        "EXCMSG_CRYPTSFFAIL",
                                        new Object[] {secfunc, passport.getCryptFunction()});
                        if (!HBCIUtilsInternal.ignoreError(
                                null, "client.errors.ignoreCryptErrors", errmsg))
                            throw new HBCI_Exception(errmsg);
                    }

                    // TODO: diese checks werden vorerst abgeschaltet, damit pin-tan reibungslos
                    // geht
                    /*
                                    // constraint checking
                    ...
                    */
                    ...
                    if (!compfunc.equals("0")) {
                        String errmsg =
                                HBCIUtilsInternal.getLocMsg("EXCMSG_CRYPTCOMPFUNCFAIL", compfunc);
                        if (!HBCIUtilsInternal.ignoreError(
                                null, "client.errors.ignoreCryptErrors", errmsg))
                            throw new HBCI_Exception(errmsg);
                    }
                ...
                HBCIUtils.log(
                        "did not decrypt - message is already cleartext", HBCIUtils.LOG_DEBUG);
