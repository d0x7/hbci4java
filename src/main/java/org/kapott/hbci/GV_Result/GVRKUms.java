...
/**
 * Ergebnisse der Abfrage von Kontoumsatzinformationen. Ein Objekt dieser Klasse entspricht einen
 * Kontoauszug. Ein Kontoauszug ist in einzelne Buchungstage unterteilt. Für jeden einzelnen
 * Buchungstag wiederum gibt es eine Anzahl von Umsatzzeilen (das entspricht je einem Eintrag auf
 * dem "normalen" Kontoauszug auf Papier). Jede einzelne Umsatzzeile wiederum enthält die einzelnen
 * Informationen zu genau einer Transaktion.
 *
 * <p>Es können auch alle Umsatzzeilen in einer einzigen Liste abgefragt werden (also nicht in
 * Buchungstage unterteilt .
 */
public class GVRKUms extends HBCIJobResultImpl {
     ...
     * Es dient nur zu Debugging-Zwecken und sollte eigentlich immer <code>null</code> bzw. einen
     * leeren String enthalten. Wenn das nicht der Fall ist, dann konnten die empfangenen
     * Kontoauszüge nicht richtig geparst werden, und dieser String enthält den "Schwanz" der
     * Kontoauszugsdaten, bei dem das Parsing-Problem aufgetreten ist.
     */
    ...
    /**
     * Gibt alle Transaktionsdatensätze in einer "flachen" Struktur zurück. D.h. nicht in einzelne
     * Buchungstage unterteilt, sondern in einer Liste analog zu einem "normalen" Kontoauszug.
     *
    ...
    private void parseMT94x(StringBuffer buffer, List<BTag> tage, StringBuffer rest) {
        ...
        try {
            ...
            while (buffer.length() != 0) {
                ...
                while (true) {
                    ...
                    if (st_ums.charAt(6) > '9') {
                        ...
                        // beim :61er Tag ist das Buchungsdatum optional. Wenn es nicht gesetzt ist,
                        // muss das
                        // Buchungsdatum des
                        ...
                        line.bdate =
                                dateFormat.parse(st_ums.substring(0, 2) + st_ums.substring(6, 10));
                        ...
                        // FE: ein Monat reicht nicht, es sollte schon ein halbes Jahr sein - es gab
                        // verschiedene Probleme mit Umsaetzen im falschen Jahr!!
                        // http://www.onlinebanking-forum.de/phpBB2/viewtopic.php?p=75348
                        if (Math.abs(line.bdate.getTime() - line.valuta.getTime())
                                > 180L * 24 * 3600 * 1000) {
                            int diff;
                    ...
                    line.value.setValue(
                            HBCIUtilsInternal.string2Long(
                                    (cd.equals(negValueIndikator) ? "-" : "")
                                            + st_ums.substring(next, npos).replace(',', '.'),
                                    100));
                    next = npos + 4;
                    ...
                    line.saldo.value =
                            new Value(
                                    saldo,
                                    (btag.start != null) ? btag.start.value.getCurr() : "EUR");
                    ...
                    if (next < st_ums.length() && st_ums.charAt(next) == '\r') {
                        ...
                        if (pos != -1) {
                            ...
                            try {
                                line.orig_value =
                                        new Value(
                                                st_ums.substring(pos + 9, slashpos)
                                                        .replace(',', '.'),
                    ...
                    if (st_multi != null) {
                        ...
                        if (!line.gvcode.equals("999")) {
                            ...
                            for (int i = 0; i < 10; i++) {
                                line.addUsage(
                                        Swift.getMultiTagValue(st_multi, Integer.toString(20 + i)));
                            }
                            ...
                            // fuer den Fall, dass in der BLZ sowas hier drin steht: "GENODEF1S06
                            // SVWZ+ ja"
                            ...
                            if (acc.blz != null) {
                                ...
                                if (space != -1) {
                                    HBCIUtils.log(
                                            "blz/bic \""
                                                    + acc.blz
                                                    + "\" contains invalid chars, trimming after first space",
                                            HBCIUtils.LOG_DEBUG);
                            ...
                            for (int i = 0; i < 4; i++) {
                                line.addUsage(
                                        Swift.getMultiTagValue(st_multi, Integer.toString(60 + i)));
                            }
                        } else {
                ...
                // Now check if the end balance (Schlusssaldo) equals balance of last statement. If
                // not, the
                // bank sent a wrong start balance
            ...
            // HBCIUtils.log("Parsing of MT940 ok until now; unparsed data:
            // "+buffer,HBCIUtils.LOG_DEBUG2);
        } catch (Exception e) {
            HBCIUtils.log(
                    "There is unparsed MT94x data - an exception occured while parsing",
                    HBCIUtils.LOG_ERR);
    ...
    public static class UmsLine implements Serializable {
        ...
        /**
         * Art der Buchung (bankinterner Code). Nur wenn hier ein Wert ungleich <code>999</code>
         * drinsteht, enthalten die Attribute <code>text</code>, <code>primanota</code>, <code>usage
         * </code>, <code>other</code> und <code>addkey</code> sinnvolle Werte. Andernfalls sind all
         * diese Informationen möglicherweise im Feld <code>additional</code> enthalten, allerdings
         * in einem nicht definierten Format (siehe auch <code>additional</code>).
         */
        public String gvcode;

        /**
         * Zusatzinformationen im Rohformat. Wenn Zusatzinformationen zu dieser Transaktion in einem
         * unbekannten Format vorliegen, dann enthält dieser String diese Daten (u.U. ist dieser
         * String leer, aber nicht <code>null</code>). Das ist genau dann der Fall, wenn der Wert
         * von <code>
         * gvcode</code> gleich <code>999</code> ist.
         *
         * <p>Wenn die Zusatzinformationen aber ausgewertet werden können (und <code>gvcode!=999
         * </code> ), so ist dieser String <code>null</code>, und die Felder <code>text</code>,
         * <code>primanota
         * </code>, <code>usage</code>, <code>other</code> und <code>addkey</code> enthalten die
         * entsprechenden Werte (siehe auch <code>gvcode</code>)
         */
        public String additional;

        /**
         * Beschreibung der Art der Buchung (optional). Nur wenn <code>gvcode!=999</code>! (siehe
         * auch <code>additional</code> und <code>gvcode</code>)
         */
        public String text;
        /**
         * Primanotakennzeichen (optional). Nur wenn <code>gvcode!=999</code>! (siehe auch <code>
         * additional</code> und <code>gvcode</code>)
         */
        public String primanota;
        /**
         * Liste von Strings mit den Verwendungszweckzeilen. Nur wenn <code>gvcode!=999</code>!
         * (siehe auch <code>additional</code> und <code>gvcode</code>)
         */
        public List<String> usage;
        /**
         * Gegenkonto der Buchung (optional). Nur wenn <code>gvcode!=999</code>! (siehe auch <code>
         * additional</code> und <code>gvcode</code>)
         */
        public Konto other;
        /**
         * Erweiterte Informationen zur Art der Buchung (bankintern, optional). Nur wenn <code>
         * gvcode!=999</code>! (siehe auch <code>additional</code> und <code>gvcode</code>)
         */
        ...
        public String toString() {
            ...
            ret.append(HBCIUtils.date2StringLocal(valuta))
                    .append(" ")
                    .append(HBCIUtils.date2StringLocal(bdate))
                    .append(" ");
            ret.append(customerref).append(":").append(instref).append(" ");
            ret.append(value.toString());
            ret.append(isStorno ? " (Storno)" : "");
            if (orig_value != null) ret.append(" (orig ").append(orig_value.toString()).append(")");
    ...
    /**
     * Enthält alle Transaktionen eines einzelnen Buchungstages. Dazu gehören das Datum des
     * jeweiligen Tages, der Anfangs- und Endsaldo sowie die Menge aller dazugehörigen Umsatzeilen
     */
    public static class BTag implements Serializable {
        /**
         * Konto, auf das sich die Umsatzdaten beziehen (Kundenkonto). Einige Kreditinstitute geben
         * fehlerhafte Kontoauszüge zurück, was zur Folge haben kann, dass dieses Feld nicht richtig
         * belegt ist. Tritt ein solcher Fall ein, so kann es vorkommen, dass von dem <code>Konto
         * </code> -Objekt nur das Feld <code>number</code> gefüllt ist, und zwar mit den
         * Informationen, die das Kreditinstitut zur Identifizierung dieses Kontos zurückgibt.
         *
         * <p>Normalerweise bestehen diese Informationen aus BLZ und Kontonummer, die dann auch
         * korrekt in das <code>Konto</code>-Objekt eingetragen werden. Liegen diese Informationen
         * aber gar nicht oder in einem falschen bzw. unbekannten Format vor, so werden diese Daten
        ...
        public String toString() {
            ...
            ret.append("Konto ")
                    .append(my.toString())
                    .append(" - Auszugsnummer ")
                    .append(counter)
                    .append(linesep);
            ret.append("  ")
                    .append((starttype == 'F' ? "Anfangs" : "Zwischen"))
                    .append("saldo: ")
            ...
            ret.append("  ")
                    .append((endtype == 'F' ? "Schluss" : "Zwischen"))
                    .append("saldo: ")
                    .append(end.toString());
            return ret.toString().trim();
        }
