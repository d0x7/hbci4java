...
public class RInvalidSuppHBCIVersion extends Rewrite {
    /**
     * @see org.kapott.hbci.rewrite.Rewrite#incomingClearText(java.lang.String,
    ...
    public String incomingClearText(String st, MsgGen gen) {
        ...
        try {
            ...
            for (int i = 0; i < 1000; i++) {
                String elemName =
                        HBCIUtilsInternal.withCounter(
                                myMsgName + ".BPD.BPA.SuppVersions.version", i);
    ...
    private boolean replace(
            StringBuffer msg, SyntaxElement elem, String search, HBCIVersion replace) {
        final String version = elem.toString();
        if (version == null || search == null || !version.equals(search)) return false;

        final String s = replace.getId();
        HBCIUtils.log(
                "there is an invalid hbci version number ('"
                        + version
                        + "') in this BPD - replacing it with '"
                        + s
                        + " '",
                HBCIUtils.LOG_WARN);
