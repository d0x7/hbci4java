...
/**
 * Rewriter-Modul für Kontoauszüge der 1822direkt-Bank (und vielleicht andere). Die Kontoauszüge
 * können nicht geparst werden, weil das verwendete SWIFT-Format von dem in der HBCI-Spezifikation
 * vorgeschriebenen Format abweicht. Dieses Modul korrigiert die Fehler in den Kontoauszugsdaten, so
 * dass Kontoauszüge mit <em>HBCI4Java</em> wieder zu parsen sind.
 */
        ...
        if (!temp.substring(temp.length() - 3, temp.length()).equals("\r\n-")) {
            ...
            if (posi >= 0) {
                if (posi != temp.length() - 1) {
                    ...
                    HBCIUtils.log(
                            "absolutely no ending sequence found - "
