...
/**
 * Rewriter-Modul für falsche Informationen über TAN-Verfahren. Einige Banken mit HBCI+
 * (HBCI-PIN/TAN) -Unterstützung stellen fälschlicherweise in die BPD die Information ein, dass das
 * Sicherheitsverfahren "TAN" unterstützt wird. "TAN" ist aber kein gültiger Code für
 * Sicherheitsmechanismen, so dass dieses Rewriter-Modul die "TAN"-Information aus den BPD entfernt.
 *
 * <p>Ist dieses Modul aktiv, so muss auch das Modul "<code>Olly</code>" aktiv sein, weil einige
 * hier vorgenommene Änderungen wiederum fehlerhafte Nachrichten erzeugen, die aber durch "<code>
 * Olly</code>" wieder korrigiert werden.
 */
        ...
        MSG msg =
                MSGFactory.getInstance()
                        .createMSG(
                                ...
                                gen,
                                MSG.CHECK_SEQ,
                                MSG.DONT_CHECK_VALIDS);

        // in einer Schleife durch alle SuppSecMethods-Datensätze laufen
        for (int i = 0; ; i++) {
            String elemBaseName =
                    HBCIUtilsInternal.withCounter(myMsgName + ".BPD.SecMethod.SuppSecMethods", i);
            ...
            if (method.equals("TAN")) { // "TAN" ist ungültiger Bezeichner
                HBCIUtils.log(
                        "there is an invalid sec type (TAN) in this BPD - removing it",
                        HBCIUtils.LOG_WARN);
                ...
                for (Iterator<MultipleSyntaxElements> it = parent.getChildContainers().iterator();
                        it.hasNext(); ) {
                ...
                if (number > 1) { // nur das eine fehlerhafte TAN:1 löschen
                    ...
                    endpos =
                            startpos
                                    ...
                                    + 1
                                    + msg.getElement(elemBaseName + ".version")
                                            .toString(0)
                                            .length();
