            ...
            if (firstPlus != -1) {
                ...
                if (secondPlus != -1) {
                    ...
                    if (value.length() > 1 && value.charAt(0) == '0') {
                        HBCIUtils.log(
                                "RSigIdLeadingZero: found leading zero ("
                                        + value
                                        + "), removing it",
                                HBCIUtils.LOG_WARN);
                        ...
                        HBCIUtils.log(
                                "RSigIdLeadingZero: setting new sigid: " + value,
                                HBCIUtils.LOG_WARN);
                    }
                } else {
                    HBCIUtils.log(
                            "RSigIdLeadingZero: can not find end of sigid in segment",
                            HBCIUtils.LOG_WARN);
                }
            } else {
                HBCIUtils.log(
                        "RSigIdLeadingZero: can not find sigid in segment", HBCIUtils.LOG_WARN);
            }
        }
