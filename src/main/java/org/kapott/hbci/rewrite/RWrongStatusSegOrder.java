    ...
    private List<Properties> createSegmentListFromMessage(String msg) {
        ...
        for (int i = 0; i < msg.length(); i++) {
            ...
            if (!quoteNext && ch == '@') {
                ...
                segmentInfo.setProperty(
                        "code", msg.substring(startPosi, msg.indexOf(":", startPosi)));
    ...
    public String incomingClearText(String st, MsgGen gen) {
        ...
        for (Iterator<Properties> i = segmentList.iterator(); i.hasNext(); ) {
            ...
            if (segmentCode.equals("HNHBK") || segmentCode.equals("HNSHK")) {
                ...
                if (!inHeader) {
                    HBCIUtils.log(
                            "RWrongStatusSegOrder: found segment "
                                    + segmentCode
                                    + " at invalid position",
                            HBCIUtils.LOG_WARN);
                    errorOccured = true;
        ...
        if (errorOccured) {
            ...
            HBCIUtils.log(
                    "RWrongStatusSegOrder: new message after reordering: " + new_msg,
