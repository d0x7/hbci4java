...
public final class GVDauerSEPAList extends AbstractSEPAGV {
    ...
    /**
     * @see org.kapott.hbci.GV.HBCIJobImpl#extractResults(org.kapott.hbci.status.HBCIMsgStatus,
    ...
    protected void extractResults(HBCIMsgStatus msgstatus, String header, int idx) {
        ...
        HBCIUtils.log(
                "parsing SEPA standing orders from msg data [size: " + result.size() + "]",
                HBCIUtils.LOG_INFO);
        ...
        entry.firstdate =
                HBCIUtils.string2DateISO(result.getProperty(header + ".DauerDetails.firstdate"));
        ...
        if (entry.aus_available) {
            ...
            if ((st = result.getProperty(header + ".Aussetzung.newvalue.value")) != null) {
                entry.aus_newvalue =
                        new Value(st, result.getProperty(header + ".Aussetzung.newvalue.curr"));
            }
        }
        ...
        entry.can_skip =
                result.getProperty(header + ".canskip") == null
                        || result.getProperty(header + ".canskip").equals("J");
        entry.can_delete =
