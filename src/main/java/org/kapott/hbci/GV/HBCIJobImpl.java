...
public abstract class HBCIJobImpl implements HBCIJob {
    ...
    private int
            contentCounter; /* Zähler, wie viele Rückgabedaten bereits in outStore eingetragen wurden
                            (entspricht der anzahl der antwort-segmente!)*/
    private Hashtable<String, String[][]>
            constraints; /* Festlegungen, welche Parameter eine Anwendung setzen muss, wie diese im
    ...
    private Hashtable<String, Integer>
            logFilterLevels; /* hier wird für jeden hl-param-name gespeichert, ob der dazugehörige wert
    ...
    protected HBCIJobImpl(
            HBCIHandler parentHandler, String jobnameLL, HBCIJobResultImpl jobResult) {
    ...
    private void findSpecNameForGV(String jobnameLL, HBCIHandler handler) {
        ...
        if (maxVersion == 0) {
            ...
            if (!HBCIUtilsInternal.ignoreError(
                    handler.getPassport(), "client.errors.ignoreJobNotSupported", msg))
                throw new JobNotSupportedException(jobnameLL);

            maxVersion = 1;
            HBCIUtils.log(
                    "Using segment version "
                            + maxVersion
                            + " for job "
                            + jobnameLL
                            + ", although not found in BPD. This may fail",
                    HBCIUtils.LOG_WARN);
        }
    ...
    protected void addConstraint(
            String frontendName, String destinationName, String defValue, int logFilterLevel) {
    ...
    protected void addConstraint(
        ...
        if (values == null) {
            // wenn es noch keine gibt, ein neues frontend-ding anlegen //FIXME: was ist ein
            // "frontend-ding"?
            values = new String[1][];
            values[0] = value;
        } else {
    ...
    public void verifyConstraints() {
        ...
        for (Iterator<String> i = constraints.keySet().iterator(); i.hasNext(); ) {
            ...
            for (int j = 0; j < values.length; j++) {
                ...
                // lowlevel-name (Pfadname) des parameters (z.B. wird Frontendname src.bic zum Pfad
                // My.bic
                ...
                if (content == null) {
                    String msg =
                            HBCIUtilsInternal.getLocMsg("EXC_MISSING_HL_PROPERTY", frontendName);
                    if (!HBCIUtilsInternal.ignoreError(
        ...
        try {
            ...
            seg =
                    SEGFactory.getInstance()
                            .createSEG(getName(), getName(), null, 0, gen.getSyntax());
     ...
     * Parameter. Diese müssten auf "normalem" Wege durch drei Aufrufe von {@link #setParam(String,
     * String)} erzeugt werden (je einer für die Länderkennung, die Bankleitzahl und die
     * Kontonummer). Durch Verwendung dieser Methode wird dieser Weg abgekürzt. Es wird ein
     * Kontoobjekt übergeben, für welches die entsprechenden drei <code>setParam(String,String)
     * </code>-Aufrufe automatisch erzeugt werden.
     *
    ...
    /**
     * @see org.kapott.hbci.GV.HBCIJob#setParam(java.lang.String, java.lang.Integer,
     *     org.kapott.hbci.structures.Konto)
     */
    public void setParam(String paramname, Integer index, Konto acc) {
        if (acceptsParam(paramname + ".country")
                && acc.country != null
                && acc.country.length() != 0) setParam(paramname + ".country", index, acc.country);
        ...
        if (acceptsParam(paramname + ".subnumber")
                && acc.subnumber != null
                && acc.subnumber.length() != 0)
    ...
    public void setParam(String paramname, Integer index, Value v) {
        if (acceptsParam(paramname + ".value"))
            setParam(
                    paramname + ".value",
                    index,
                    HBCIUtils.bigDecimal2String(v.getBigDecimalValue()));
    ...
    public void setParam(String paramName, Integer index, String value) {
        ...
        if (value == null || value.length() == 0) {
            String msg =
                    HBCIUtilsInternal.getLocMsg(
                            "EXCMSG_PARAM_EMPTY", new String[] {paramName, getName()});
            if (!HBCIUtilsInternal.ignoreError(
                    passport, "client.errors.ignoreWrongJobDataErrors", msg))
                throw new InvalidUserDataException(msg);
            value = "";
    ...
    /**
     * Legt die Versionsnummer des Segments manuell fest. Ist u.a. noetig, um HKTAN-Segmente in
     * genau der Version zu senden, in der auch die HITANS empfangen wurden. Andernfalls koennte es
     ...
     * IRGENDWO anders verwendet wird, muss man hoellisch aufpassen, dass alle Stellen, wo
     * "this.name" bzw. "this.segVersion" direkt oder indirekt verwendet wurde, ebenfalls
                ...
                if (key.startsWith("GVRes") && key.endsWith(".SegHead.ref")) {
                    ...
                    if ((Integer.parseInt(segref)) - offset == idx) {
                        ...
                        if (key.startsWith("GVRes_")) {
                            resnum =
                                    Integer.parseInt(
                                            key.substring(key.indexOf('_') + 1, key.indexOf('.')));
                        }
