...
public class GVWPDepotUms extends HBCIJobImpl {
    ...
    public GVWPDepotUms(HBCIHandler handler) {
        ...
        addConstraint(
                "my.country",
                "Depot.KIK.country",
                passport.getUPD().getProperty("KInfo.KTV.KIK.country"),
                LogFilter.FILTER_NONE);
        addConstraint(
                "my.blz",
                "Depot.KIK.blz",
                passport.getUPD().getProperty("KInfo.KTV.KIK.blz"),
    ...
    private TypedValue parseTypedValue(String st) {
        ...
        return new TypedValue(
                (neg ? "-" : "") + st.substring(pos1).replace(',', '.'), curr, saldo_type);
    }
    ...
    protected void extractResults(HBCIMsgStatus msgstatus, String header, int idx) {
        ...
        while (buffer.length() != 0) {
            try {
                ...
                if (pos2 < 0) pos2 = st_depot.length();
                entry.depot = new Konto();
                entry.depot.blz = st_depot.substring(pos1 + 2, pos2);
                if (pos2 < st_depot.length()) entry.depot.number = st_depot.substring(pos2 + 1);
                getMainPassport().fillAccountInfo(entry.depot);

                String st;
                i = 0;
                // Parse einzelnes Finanzinstrument (Repetitive Optional Subsequence B1 Financial
                // Instrument)
                ...
                if (st.substring(st.indexOf("//") + 2).equals("Y")) {
                    ...
                    while (true) {
                        ...
                        FinancialInstrument instrument =
                                new GVRWPDepotUms.Entry.FinancialInstrument();
                        ...
                        else oneinstrument_header = oneinstrument;
                        ...
                        if (haveISIN) {
                            ...
                            if (pos1 + 2 < st.length()
                                    && st.substring(pos1 + 2, pos1 + 6).equals("/DE/")) {
                                pos2 = st.indexOf("\r\n", pos1 + 6);
                                if (pos2 == -1) {
                                    pos2 = st.length();
                                }
                        ...
                        pos1 += 2;
                        if (pos1 < st.length()) instrument.name = st.substring(pos1);
                        ...
                        while (true) {
                            ...
                            if ("FIOP".equals(qualifier)
                                    || (instrument.startSaldo == null
                                            && "INOP".equals(qualifier))) {
                                instrument.startSaldo = parseTypedValue(st);
                            } else if ("FICL".equals(qualifier)
                                    || (instrument.endSaldo == null && "INCL".equals(qualifier))) {
                        ...
                        while (trans_start >= 0) {
                            ...
                            if (trans_end < 0) break;
                            String onetransaction =
                                    oneinstrument.substring(trans_start, trans_end + 9);
                            ...
                            if (link_start >= 0) {
                                ...
                                if (link_end >= 0) {
                                    String onelink =
                                            onetransaction.substring(link_start, link_end + 8);
                            ...
                            if (detail_start >= 0) {
                                int detail_end =
                                        onetransaction.indexOf(":16S:TRANSDET", detail_start);
                                if (detail_end >= 0) {
                                    String onedetail =
                                            onetransaction.substring(detail_start, detail_end + 12);
                                    ...
                                    if (t99a != null)
                                        if (t99a.startsWith(":DAAC")) {
                                            int neg = 0;
                                            if (t99a.charAt(7) == 'N') neg = 1;
                                            transaction.stueckzins_tage =
                                                    Integer.parseInt(t99a.substring(7 + neg));
                                            if (neg != 0)
                                                transaction.stueckzins_tage =
                                                        -transaction.stueckzins_tage;
                                        } else {
                                    ...
                                    while (true) {
                                        ...
                                        if (t19a.startsWith(":PSTA")) {
                                            int off = 7;
                                            if (t19a.charAt(off) == 'N') off++;
                                            transaction.betrag =
                                                    new BigDecimalValue(
                                                            t19a.substring(off + 3)
                                                                    .replace(',', '.'),
                                                            t19a.substring(off, off + 3));
                                            if (off > 7)
                                                transaction.betrag.setValue(
                                                        transaction.betrag.getValue().negate());
                                        } else if (t19a.startsWith(":ACRU")) {
                                            int off = 7;
                                            if (t19a.charAt(off) == 'N') off++;
                                            transaction.stueckzinsen =
                                                    new BigDecimalValue(
                                                            t19a.substring(off + 3)
                                                                    .replace(',', '.'),
                                                            t19a.substring(off, off + 3));
                                            if (off > 7)
                                                transaction.stueckzinsen.setValue(
                                                        transaction
                                                                .stueckzinsen
                                                                .getValue()
                                                                .negate());
                                    ...
                                    while (true) {
                                        ...
                                        if (t22f.startsWith(":TRAN")) {
                                            if (t22f.endsWith("SETT")) {
                                                ...
                                                transaction.transaction_indicator =
                                                        Transaction.INDICATOR_LEIHE;
                                            } else if (t22f.endsWith("COLL")) {
                                                transaction.transaction_indicator =
                                                        Transaction.INDICATOR_SICHERHEITEN;
                                            } else {
                                                System.out.println(
                                                        "Unbekannter 22F->TRAN: " + t22f);
                                    ...
                                    while (true) {
                                        ...
                                        if (t98a.startsWith(":ESET")) {
                                            ...
                                            String datum = t98a.substring(7);
                                            transaction.datum_valuta =
                                                    date_only_format.parse(datum);
                                        } else {
                                    ...
                                    if (move != null)
                                        if (move.startsWith(":MOVE")) {
                                            if (move.endsWith("REVE")) transaction.storno = true;
                                        } else {
                            ...
                            if (party_start >= 0) {
                                ...
                                if (party_end >= 0) {
                                    String oneparty =
                                            onetransaction.substring(party_start, party_end + 10);
