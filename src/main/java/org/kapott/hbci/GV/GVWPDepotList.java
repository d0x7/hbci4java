            ...
            try {
                ...
                while (true) {
                    ...
                    if (st.substring(1, 5).equals("HOLP")) {
                        ...
                        if (pos1 > 7) entry.total.setValue(entry.total.getValue().negate());
                        break;
                    }
                }
                ...
                if (st.substring(st.indexOf("//") + 2).equals("Y")) {
                    ...
                    while (true) {
                        ...
                        pos1 += 2;
                        if (pos1 < st.length()) gattung.name = st.substring(pos1);
                        ...
                        if (st != null) {
                            gattung.pricequalifier =
                                    (st.substring(1, 5).equals("MRKT"))
                            ...
                            gattung.price =
                                    new BigDecimalValue(st.substring(next).replace(',', '.'), curr);
                        }
                        ...
                        if (st_date != null) {
                            if (st_time != null) {
                                gattung.timestamp_price =
                                        date_time_format.parse(st_date + " " + st_time);
                            } else {
                        ...
                        if (st.charAt(pos1) == 'N')
                            gattung.saldo =
                                    new BigDecimalValue(
                                            "-" + st.substring(pos1 + 1).replace(',', '.'), curr);
                        else
                            gattung.saldo =
                                    new BigDecimalValue(st.substring(pos1).replace(',', '.'), curr);
                            ...
                            if (st.substring(1, 5).equals("HOLD")) {
                                ...
                                gattung.depotwert =
                                        new BigDecimalValue(
                                                ...
                                                st.substring(pos1, pos1 + 3));
                                if (pos1 > 7)
                                    gattung.depotwert.setValue(
                                            gattung.depotwert.getValue().negate());
                                break;
                            }
                        }
                        ...
                        while (true) {
                            ...
                            if (st.substring(1, 5).equals("ACRU")) {
                                pos1 = 7;
                                if (st.charAt(pos1) == 'N') pos1++;
                                gattung.stueckzinsbetrag =
                                        new BigDecimalValue(
                                                st.substring(pos1 + 3).replace(',', '.'),
                                                st.substring(pos1, pos1 + 3));
                                if (pos1 > 7)
                                    gattung.stueckzinsbetrag.setValue(
                                            gattung.stueckzinsbetrag.getValue().negate());
                                break;
                            }
                        }
                        ...
                        if (st != null) {
                            ...
                            gattung.xchg_kurs =
                                    Double.parseDouble(st.substring(15).replace(',', '.'));
                        }
                        ...
                        if (st != null) {
                            ...
                            gattung.countryEmittent =
                                    SwiftLegacy.getLineFieldValue(formtext, "1", 3);

                            st = SwiftLegacy.getLineFieldValue(formtext, "1", 4);
                            if (st != null) gattung.kauf = date_only_format.parse(st);
                            st = SwiftLegacy.getLineFieldValue(formtext, "1", 5);
                            if (st != null) gattung.faellig = date_only_format.parse(st);

                            st = SwiftLegacy.getLineFieldValue(formtext, "2", 0);
                            if (st != null) {
                                gattung.einstandspreis =
                                        new BigDecimalValue(st.replace(',', '.'), "%");
                            }
                            st = SwiftLegacy.getLineFieldValue(formtext, "2", 1);
                            if (st != null) gattung.einstandspreis.setCurr(st);

                            st = SwiftLegacy.getLineFieldValue(formtext, "2", 2);
                            if (st != null)
                                gattung.zinssatz =
                                        HBCIUtilsInternal.string2Long(st.replace(',', '.'), 1000);
