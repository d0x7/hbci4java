...
public class GVKontoauszug extends HBCIJobImpl {
    ...
    protected void extractResults(HBCIMsgStatus msgstatus, String header, int idx) {
        ...
        if (date != null && date.length() > 0) umsResult.setDate(HBCIUtils.string2DateISO(date));

        String year = result.getProperty(header + ".year");
        String number = result.getProperty(header + ".number");
        if (year != null && year.length() > 0) umsResult.setYear(Integer.parseInt(year));
        if (number != null && number.length() > 0) umsResult.setNumber(Integer.parseInt(number));

        umsResult.setStartDate(
                HBCIUtils.string2DateISO(result.getProperty(header + ".TimeRange.startdate")));
        umsResult.setEndDate(
                HBCIUtils.string2DateISO(result.getProperty(header + ".TimeRange.enddate")));
