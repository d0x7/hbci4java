...
public class GVFestList extends HBCIJobImpl {
    ...
    protected void extractResults(HBCIMsgStatus msgstatus, String header, int idx) {
        ...
        if (result.getProperty(header + ".Ausbuchungskto.number") != null) {
            ...
            entry.ausbuchungskonto.country =
                    result.getProperty(header + ".Ausbuchungskto.KIK.country");
            entry.ausbuchungskonto.number = result.getProperty(header + ".Ausbuchungskto.number");
            entry.ausbuchungskonto.subnumber =
                    result.getProperty(header + ".Ausbuchungskto.subnumber");
        ...
        entry.konditionen.ablaufdatum =
                HBCIUtils.string2DateISO(result.getProperty(header + ".FestCond.ablaufdate"));
        entry.konditionen.anlagedatum =
                HBCIUtils.string2DateISO(result.getProperty(header + ".FestCond.anlagedate"));
        ...
        if (result.getProperty(header + ".FestCondVersion.version") != null) {
            entry.konditionen.date =
                    HBCIUtils.strings2DateTimeISO(
                            result.getProperty(header + ".FestCondVersion.date"),
        ...
        if (st.equals("A")) entry.konditionen.zinsmethode = GVRFestCondList.Cond.METHOD_30_360;
        else if (st.equals("B"))
        ...
        else if (st.equals("F")) entry.konditionen.zinsmethode = GVRFestCondList.Cond.METHOD_30_365;

        entry.konditionen.zinssatz =
                HBCIUtilsInternal.string2Long(
                        result.getProperty(header + ".FestCond.zinssatz"), 1000);
        entry.konditionen.minbetrag =
                new Value(
        ...
        if (result.getProperty(header + ".Prolong.laufzeit") != null) {
            ...
            entry.verlaengerung.betrag =
                    new Value(
                            result.getProperty(header + ".Prolong.BTG.value"),
                            result.getProperty(header + ".Prolong.BTG.curr"));
            entry.verlaengerung.laufzeit =
                    Integer.parseInt(result.getProperty(header + ".Prolong.laufzeit"));
            entry.verlaengerung.verlaengern =
                    result.getProperty(header + ".Prolong.wiederanlage").equals("2");
        }
