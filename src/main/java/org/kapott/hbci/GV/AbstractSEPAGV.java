...
public abstract class AbstractSEPAGV extends HBCIJobImpl {
    /**
     * Token, der als End-to-End ID Platzhalter verwendet wird, wenn keine angegeben wurde. In
     * pain.001.001.02 wurde dieser Token noch explizit erwaehnt. Inzwischen nicht mehr. Nach
     * Ruecksprache mit Holger vom onlinebanking-forum.de weiss ich aber, dass VRNetworld den auch
     * verwendet und er von Banken als solcher erkannt wird.
     */
    ...
    /**
     * Liefert die Default-PAIN-Version, das verwendet werden soll, wenn von der Bank keine
     * geliefert wurden.
     *
    ...
    /**
     * Durchsucht das BPD-Segment "HISPAS" nach dem Property "cannationalacc" um herauszufinden, ob
     * beim Versand eines SEPA-Auftrages die nationale Bankverbindung angegeben sein darf.
     *
     * <p>Siehe FinTS_3.0_Messages_Geschaeftsvorfaelle_2013-05-28_final_version.pdf - Kapitel B.3.2
     *
     * @param handler
     * @return true, wenn der BPD-Parameter von der Bank mit "J" befuellt ist und die nationale
    ...
    protected boolean canNationalAcc(HBCIHandler handler) {
        ...
        if (passport instanceof HBCIPassportInternal) {
            ...
            if (o != null) {
                String s = o.toString();
                HBCIUtils.log(
                        "value of \"cannationalacc\" overwritten in passport, value: " + s,
                        HBCIUtils.LOG_INFO);
    ...
    /**
     * Diese Methode schaut in den BPD nach den unterstützen pain Versionen (bei LastSEPA
     * pain.008.xxx.xx) und vergleicht diese mit den von HBCI4Java unterstützen pain Versionen. Der
     * größte gemeinsamme Nenner wird zurueckgeliefert.
     *
     * @param handler
     * @param gvName der Geschaeftsvorfall fuer den in den BPD nach dem PAIN-Versionen gesucht
     *     werden soll.
    ...
    private PainVersion determinePainVersion(HBCIHandler handler, String gvName) {
        ...
        PainVersion globalVersion =
                this.determinePainVersionInternal(handler, GVSEPAInfo.getLowlevelName());
        ...
        if (globalVersion == null && jobVersion == null) {
            ...
            HBCIUtils.log(
                    "unable to determine matching pain version, using default: " + def,
                    HBCIUtils.LOG_WARN);
            return def;
        }
        ...
        if (jobVersion == null) {
            HBCIUtils.log(
                    "have no job-specific pain version, using global pain version: "
                            + globalVersion,
                    HBCIUtils.LOG_DEBUG);
    ...
    private PainVersion determinePainVersionInternal(HBCIHandler handler, final String gvName) {
        HBCIUtils.log(
                "searching for supported pain versions for GV " + gvName, HBCIUtils.LOG_DEBUG);
        ...
        while (e.hasMoreElements()) {
            ...
            try {
                ...
                if (version.getType() == this.getPainType()) {
                    ...
                    // TestPainVersion#test011 bzw.
                    // http://www.onlinebanking-forum.de/phpBB2/viewtopic.php?p=95160#95160
    ...
    /**
     * @see org.kapott.hbci.GV.HBCIJobImpl#setLowlevelParam(java.lang.String, java.lang.String) This
     *     is needed to "redirect" the sepa values. They dont have to stored directly in the
     *     message, but have to go into the SEPA document which will by created later (in
     *     verifyConstraints())
     */
    ...
    /**
     * This is needed for verifyConstraints(). Because verifyConstraints() tries to read the
     * lowlevel-values for each constraint, the lowlevel-values for sepa.xxx would always be empty
     * (because they do not exist in hbci messages). So we read the sepa lowlevel-values from the
     * special sepa structure instead from the lowlevel params for the message
     *
     * @param key
    ...
    /**
     * Gibt die SEPA Message ID als String zurück. Existiert noch keine wird sie aus Datum und User
     * ID erstellt.
     *
    ...
    protected final ISEPAGenerator getSEPAGenerator() {
        if (this.generator == null) {
            try {
                ...
                String msg =
                        HBCIUtilsInternal.getLocMsg("EXCMSG_JOB_CREATE_ERR", this.getPainJobName());
                if (!HBCIUtilsInternal.ignoreError(
                        null, "client.errors.ignoreCreateJobErrors", msg))
    ...
    protected void createSEPAFromParams() {
        ...
        try {
            ...
            throw new HBCI_Exception(
                    "*** the _sepapain segment for this job can not be created", e);
        }
    ...
    /**
     * @see org.kapott.hbci.GV.HBCIJobImpl#addConstraint(java.lang.String, java.lang.String,
     *     java.lang.String, int, boolean) Ueberschrieben, um die Default-Werte der SEPA-Parameter
     *     vorher rauszufischen und in "this.sepaParams" zu speichern. Die brauchen wir
     *     "createSEPAFromParams" beim Erstellen des XML - sie wuerden dort sonst aber fehlen, weil
     *     Default-Werte eigentlich erst in "verifyConstraints" uebernommen werden.
     */
    @Override
    ...
    /**
     * Bei SEPA Geschäftsvorfällen müssen wir verifyConstraints überschreiben um die SEPA XML zu
     * generieren
     */
    ...
    /**
     * Referenzierter pain-Jobname. Bei vielen Geschäftsvorfällen (z.B. Daueraufträgen) wird die
     * pain der Einzeltransaktion verwendet.
     *
     * @return Value
     */
