 ...
 *
 * <p>WICHTIG: Diese Klasse sowie die Ableitungen sollten auch ohne initialisiertes HBCI-System
 * funktionieren, um das XML ohne HBCI-Handler erstellen zu koennen. Daher sollte auf die Verwendung
 * von "HBCIUtils" & Co verzichtet werden. Das ist auch der Grund, warum hier das Java-Logging
 * verwendet wird und nicht das HBCI4Java-eigene.
 */
public abstract class AbstractSEPAGenerator implements ISEPAGenerator {
    ...
    protected void marshal(JAXBElement e, OutputStream os, boolean validate) throws Exception {
        ...
        // Wir verwenden hier hart UTF-8. Siehe
            ...
            if (file != null) {
                if (validate) {
                    ...
                    if (is != null) {
                        ...
                        if (f.isFile() && f.canRead()) source = new StreamSource(f);
                    }
                    ...
                    if (source == null)
                        throw new HBCI_Exception(
                                "schema validation activated against "
                                        + file
                                        + " - but schema file could not be found");
