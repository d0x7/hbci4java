...
public class GVSaldoReq extends HBCIJobImpl {
    ...
    protected void extractResults(HBCIMsgStatus msgstatus, String header, int idx) {
        ...
        String st =
                (cd.equals("D") ? "-" : "") + result.getProperty(header + ".booked.BTG.value", "0");
        info.ready.value = new Value(st, result.getProperty(header + ".booked.BTG.curr"));
        info.ready.timestamp =
                HBCIUtils.strings2DateTimeISO(
                        result.getProperty(header + ".booked.date"),
        ...
        if (cd != null) {
            st =
                    (cd.equals("D") ? "-" : "")
                            + result.getProperty(header + ".pending.BTG.value", "0");
            info.unready = new Saldo();
            info.unready.value = new Value(st, result.getProperty(header + ".pending.BTG.curr"));
            info.unready.timestamp =
                    HBCIUtils.strings2DateTimeISO(
                            result.getProperty(header + ".pending.date"),
