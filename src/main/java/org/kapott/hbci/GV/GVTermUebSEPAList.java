...
public final class GVTermUebSEPAList extends AbstractSEPAGV {
    ...
    protected void extractResults(HBCIMsgStatus msgstatus, String header, int idx) {
        ...
        try {
            ...
            // auch dann nicht, wenn wir genau wissen, dass das XML mit "<?xml version="1.0"
            // encoding="UTF-8"?>"
            // beginnt. Stattdessen muessen wir den selben Zeichensatz nehmen, der bei der
            // Byte->String
            // Conversion
            // beim Empfamg der rohen HBCI-Daten ueber TCP verwendet wurde. Siehe
            // CommStandard/CommPinTan.
            // Die eigentliche Codierung der XML-Datei spielt hier keine Rolle - wichtig ist, dass
            // die
            // Rueckwandlung String->Bytes (in pain.getBytes) den selben Zeichensatz verwendet wie
            // beim
            // Empfang der
            // Daten vom Server. Nur so ist sichergestellt, dass die Bytes wieder genauso aussehen,
            // wie
            // sie
            // beim Empfang vom Server kamen, wenn der XML-Parser sie kriegt. Er macht dann die
            // Conversion
            // Byte->String
        ...
        entry.can_change =
                result.getProperty(header + ".canchange") == null
                        || result.getProperty(header + ".canchange").equals("J");
        entry.can_delete =
                result.getProperty(header + ".candel") == null
                        || result.getProperty(header + ".candel").equals("J");
