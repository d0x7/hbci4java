...
/**
 * Hilfsklasse zum Erzeugen von DTAUS-Datensätzen für die Verwendung in Sammelüberweisungen und
 * Sammellastschriften. Diese Klasse kann verwendet werden, um den DTAUS-Datenstrom zu erzeugen, der
 * für Sammellastschriften und -überweisungen als Job-Parameter angegeben werden muss.
 *
 * <p>In einem DTAUS-Objekt werden ein oder mehrere Transaktionen gespeichert. Dabei müssen alle
 * Transaktionen entweder Lastschriften oder Überweisungen sein. Außerdem wird für alle
 * Transaktionen das gleiche "Auftraggeberkonto" angenommen (bei Überweisungen also das
 * Belastungskonto, bei Lastschriften das Konto, auf das der Betrag gutgeschrieben wird).
 *
 * <p>In der Regel wird zunächst ein <code>DTAUS</code>-Objekt erzeugt. Dazu wird der Konstruktor
 * {@link #DTAUS(Konto, int)} verwendet, womit gleichzeit das zu verwendende Auftraggeberkonto und
 * der Typ des Sammelauftrages (<code>TYPE_CREDIT</code> für Sammelüberweisungen, <code>TYPE_DEBIT
 * </code> für Sammellastschriften) festgelegt wird. Anschließend können beliebig viele {@link
 * DTAUS.Transaction}-Objekte erzeugt werden, welche jeweils eine Transaktion darstellen. Jedes so
 * erzeugte Objekt kann mit {@link #addEntry(DTAUS.Transaction)} zum Sammelauftrag hinzugefügt
 * werden. Die Methode {@link #toString()} liefert schließlich den so erzeugten Sammelauftrag im
 * DTAUS-Format.
 */
...
public class DTAUS {
    ...
    /**
     * Erzeugen eines neuen Objektes für die Aufnahme von Sammelaufträgen. <code>myAccount</code>
     * ist dabei das "eigene" Konto, welches bei Sammelüberweisungen als Belastungskonto und bei
     * Sammellastschriften als Gutschriftkonto verwendet wird. Von dem {@link Konto}-Objekt müssen
     * mindestens die Felder <code>blz</code>, <code>number</code>, <code>curr</code> und <code>name
     * </code> richtig gesetzt sein. <br>
     * <code>execdate</code> gibt das Datum an, wann dieser Sammelauftrag ausgeführt werden soll.
     * ACHTUNG: <code>execdate</code> wird zur Zeit noch nicht ausgewertet!
     *
     * @param myAccount Gegenkonto für die enthaltenen Aufträge
     * @param type
     *     <ul>
     *       <li><code>TYPE_CREDIT</code> für Sammelüberweisungen,
     *       <li><code>TYPE_DEBIT</code> für Sammellastschriften
     *     </ul>
     *
     * @param execdate Ausführungsdatum für diesen Sammelauftrag; <code>null</code>, wenn kein
     *     Ausführungsdatum gesetzt werden soll (sofortige Ausführung)
     */
    public DTAUS(Konto myAccount, int type, Date execdate) {
        ...
        if (myAccount.curr.equals("EUR")) this.curr = CURR_EUR;
        else if (myAccount.curr.equals("DEM")) this.curr = CURR_DM;
        else
            throw new InvalidUserDataException(
                    "*** invalid currency of this account: " + myAccount.curr);
    }
    ...
    /**
     * Hinzufügen eines einzelnen Auftrages zu diesem Sammelauftrag. Das {@link
     * DTAUS.Transaction}-Objekt, welches hier als Argument benötigt wird, muss mit '<code>
     * dtaus.new&nbsp;Transaction()</code>' erzeugt werden ('<code>dtaus</code>' ist dabei das
     * aktuelle <code>DTAUS</code>-Objekt).
     *
    ...
    /**
     * Rückgabe des Sammelauftrages im DTAUS-Format. Der Rückgabewert dieser Methode kann direkt als
     * Parameterwert für den Parameter '<code>data</code>' bei Sammelaufträgen verwendet werden (für
     * eine Parameterbeschreibung siehe Paketbeschreibung des Paketes <code>org.kapott.hbci.GV
     * </code> ).
     *
    ...
    public String toString() {
        ...
        switch (type) {
            ...
            default:
                throw new InvalidUserDataException(
                        "*** type of DTAUS order not set (DEBIT/CREDIT)");
        }
        ...
        ret.append(
                expand(Long.toString(curr == CURR_DM ? sumDM : 0), 13, (byte) 0x30, ALIGN_RIGHT));
        ...
        ret.append(
                expand(Long.toString(curr == CURR_EUR ? sumEUR : 0), 13, (byte) 0x30, ALIGN_RIGHT));
    ...
    private String expand(String st, int len, byte filler, int align) {
        if (st.length() < len) {
            try {
                ...
                if (align == ALIGN_LEFT) st = st + fillst;
                else if (align == ALIGN_RIGHT) st = fillst + st;
                else throw new HBCI_Exception("*** invalid align type: " + align);
            } catch (Exception e) {
    ...
    private void parseDTAUS(String dtaus) {
        ...
        while (true) {
            ...
            entry.value =
                    new Value(Long.parseLong(value_st), (this.curr == CURR_EUR) ? "EUR" : "DEM");
        ...
        if (x != entries.size()) {
            throw new HBCI_Exception(
                    "*** there were " + entries.size() + " c-sets, but e-set says " + x);
        }
    ...
    /**
     * Daten einer einzelnen Transaktion, die in einen Sammelauftrag übernommen werden soll. Vor dem
     * Hinzufügen dieser Transaktion zum Sammelauftrag müssen alle Felder dieses
     * Transaktions-Objektes mit den jeweiligen Auftragsdaten gefüllt werden.
     */
    public class Transaction {
        /**
         * Konto des Zahlungsempfängers bzw. des Zahlungspflichtigen. Soll dieser Einzelauftrag in
         * eine Sammelüberweisung eingestellt werden, so muss in diesem Feld die Kontoverbindung des
         * Zahlungsempfängers eingestellt werden. Bei Sammellastschriften ist hier die
         * Kontoverbindung des Zahlungspflichtigen einzustellen.
         *
         * <p>Von dem verwendeten {@link Konto}-Objekt müssen mindestens die Felder <code>blz</code>
         * , <code>number</code> und <code>name</code> richtig belegt sein.
         */
        public Konto otherAccount;

        /**
         * interne Kunden-ID. Wie die verwendet wird weiß ich leider nicht genau, kann im Prinzip
         * leer gelassen werden (ansonsten Maximallänge 11 Zeichen).
         */
        public String internalCustomerId;

        /**
         * Textschlüssel für den Auftrag. Bei Sammelüberweisungen ist dieses Feld mit '51'
         * vorbelegt, bei Sammellastschriften mit '05'. Dieser Wert kann überschrieben werden,
         * gültige Werte finden sich in den Job-Restrictions (siehe {@link
         * org.kapott.hbci.GV.HBCIJob#getJobRestrictions()}).
         */
        public String key;

        /**
         * Zusätzlicher Textschlüssel (wird i.d.R. bankintern verwendet). Dieser Wert muss aus drei
         * Ziffern bestehen und ist mit '000' vorbelegt. Das manuelle Setzen dieses Wertes ist in
         * den meisten Fällen nicht nötig (außer für Leute, die wissen was sie tun ;-) ).
         */
        public String addkey;

        /**
         * Geldbetrag, der bei diesem Einzelauftrag überwiesen (Sammelüberweisungen) bzw. eingezogen
         * (Sammellastschriften) werden soll
         */
        public Value value;
        ...
        public String toString() {
            ...
            try {
                ...
                if (internalCustomerId == null) {
                    ...
                    ret.append(
                            expand(
                                    SyntaxDTAUS.check(internalCustomerId),
                                    11,
                                    (byte) 0x30,
                                    ALIGN_LEFT));
                    ret.append((char) 0);
                ...
                ret.append(
                        expand(
                                Long.toString(
                                        value.getCurr().equals("DEM") ? value.getLongValue() : 0),
                                11,
                                (byte) 0x30,
                                ALIGN_RIGHT));
                ret.append(expand(myAccount.blz, 8, (byte) 0x20, ALIGN_RIGHT));
                ...
                ret.append(
                        expand(
                                Long.toString(
                                        value.getCurr().equals("EUR") ? value.getLongValue() : 0),
                                11,
                                (byte) 0x30,
                                ALIGN_RIGHT));
                ret.append(expand("", 3, (byte) 0x20, ALIGN_LEFT));
                ret.append(
                        expand(SyntaxDTAUS.check(otherAccount.name), 27, (byte) 0x20, ALIGN_LEFT));
                ret.append(expand("", 8, (byte) 0x20, ALIGN_LEFT));

                if (value.getCurr().equals("EUR")) sumEUR += value.getLongValue();
                else if (value.getCurr().equals("DEM")) sumDM += value.getLongValue();

                ret.append(expand(SyntaxDTAUS.check(myAccount.name), 27, (byte) 0x20, ALIGN_LEFT));

                String st = "";
                if (usage.size() != 0) st = SyntaxDTAUS.check(usage.get(0));
                ...
                ret.replace(
                        posForNumOfExt,
                        posForNumOfExt + 2,
                        expand(Integer.toString(numOfExt), 2, (byte) 0x30, ALIGN_RIGHT));
                ret.replace(
                        0,
                        4,
                        expand(
                                Integer.toString(basicLenOfCSet + 29 * numOfExt),
                                4,
                                (byte) 0x30,
                                ALIGN_RIGHT));
            } catch (NullPointerException e) {
                throw new HBCI_Exception(
                        "probably one or more DTAUS values which MUST be set are null - please refer the API doc",
                        e);
            }
