...
public class Swift {
    ...
    public static String getTagValue(String st, String tag, int counter) {
        String ret = null;
        Pattern patternNLTag =
                Pattern.compile(
                        "\\r\\n(-|-\\r\\n)?:\\d{2}[A-Z]?:"); // Zu dem "(-)?" siehe
                                                             // TestBrokenMT940.java
        ...
        if (pos != -1) {
            ...
            while (true) {
                ...
                if (endpos != -1 && endpos + 2 < st.length()) {
                    ...
                    if (st.charAt(endpos + 1) >= '0'
                            && st.charAt(endpos + 1) <= '9'
                            && st.charAt(endpos + 2) >= '0'
                            && st.charAt(endpos + 2) <= '9') {
