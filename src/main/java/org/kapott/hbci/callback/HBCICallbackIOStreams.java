...
/**
 * Callback-Klasse für Ein-/Ausgabe über IO-Streams. Dabei handelt es sich eine Callback-Klasse, die
 * Ausgaben auf einem PrintStream ausgibt und Eingaben über einen BufferedReader liest. Die Klasse
 * {@link org.kapott.hbci.callback.HBCICallbackConsole HBCICallbackConsole} ist eine abgeleitete
 * Klasse, welche STDOUT und STDIN für die beiden I/O-Streams verwendet.
 */
public class HBCICallbackIOStreams extends AbstractHBCICallback {
    ...
    /**
     * Schreiben von Logging-Ausgaben in einen <code>PrintStream</code>. Diese Methode implementiert
     * die Logging-Schnittstelle des {@link org.kapott.hbci.callback.HBCICallback}-Interfaces</a>.
     * Die Log-Informationen, die dieser Methode übergeben werden, werden formatiert auf dem
     * jeweiligen <code>outStream</code> ausgegeben. In dem ausgegebenen String sind in enthalten
     * das Log-Level der Message, ein Zeitstempel im Format "<code>yyyy.MM.dd HH:mm:ss.SSS</code>",
     * die Namen der ThreadGroup und des Threads, aus dem heraus die Log-Message erzeugt wurde, der
     * Klassenname der Klasse, welche die Log-Ausgabe erzeugt hat sowie die eigentliche Log-Message
     */
     ...
     * Informationen an den Anwender übergeben sollen, werden diese auf dem <code>outStream</code>
     * ausgegeben. Bei Callbacks, die Aktionen vom Anwender erwarten (Einlegen der Chipkarte), wird
     * eine entsprechende Aufforderung ausgegeben. Bei Callbacks, die eine Eingabe vom Nutzer
     * erwarten, wird die entsprechende Eingabeaufforderung ausgegeben und die Eingabe vom <code>
     * inStream</code> gelesen.
     */
        ...
        getOutStream()
                .println(
                        HBCIUtilsInternal.getLocMsg(
                                "CALLB_PASS_IDENT", passport.getClientData("init")));
        ...
        try {
            ...
            switch (reason) {
                    ...
                    if (reason == NEED_PASSPHRASE_SAVE) {
                        ...
                        if (!st.equals(st2))
                            throw new InvalidUserDataException(
                                    HBCIUtilsInternal.getLocMsg("EXCMSG_PWDONTMATCH"));
                    }
                ...
                case NEED_PROXY_USER:
                    ...
                    getOutStream().flush();
                    st = getInStream().readLine();
                    if (st.length() == 0) st = retData.toString();

                    if (reason == NEED_BLZ) {
                        logfilter.addSecretData(st, "X", LogFilter.FILTER_MOST);
                    } else if (reason == NEED_USERID
                            || reason == NEED_CUSTOMERID
                ...
                case NEED_NEW_INST_KEYS_ACK:
                    ...
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("EXPONENT")
                                            + ": "
                                            + HBCIUtils.data2hex(
                                                    iniletter.getKeyExponentDisplay()));
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("MODULUS")
                                            + ": "
                                            + HBCIUtils.data2hex(iniletter.getKeyModulusDisplay()));
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("HASH")
                                            + ": "
                                            + HBCIUtils.data2hex(iniletter.getKeyHashDisplay()));
                ...
                case HAVE_NEW_MY_KEYS:
                    ...
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("DATE")
                                            + ": "
                                            + HBCIUtils.date2StringLocal(date));
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("TIME")
                                            + ": "
                                            + HBCIUtils.time2StringLocal(date));
                    getOutStream()
                            .println(HBCIUtilsInternal.getLocMsg("BLZ") + ": " + passport.getBLZ());
                    getOutStream()
                            .println(
                    ...
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("KEYNUM")
                                            + ": "
                                            + passport.getMyPublicSigKey().num);
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("KEYVERSION")
                                            + ": "
                                            + passport.getMyPublicSigKey().version);
                    getOutStream()
                            .println(
                                    HBCIUtilsInternal.getLocMsg("EXPONENT")
                                            + ": "
                ...
                case HAVE_CRC_ERROR:
                    ...
                    getOutStream()
                            .print(
                                    HBCIUtilsInternal.getLocMsg("ACCNUMBER")
                                            + " ["
                                            + number
                                            + "]: ");
                    getOutStream().flush();
                ...
                case NEED_PT_TANMEDIA:
                    ...
                    if (medialist.length() > 0 && !medialist.contains("|")) {
                        ...
                        getOutStream()
                                .print(HBCIUtilsInternal.getLocMsg("CALLB_SELECT_ENTRY") + ": ");
                        getOutStream().flush();
                        retData.replace(0, retData.length(), getInStream().readLine());
                    }
                    break;
     ...
     * Bearbeitungsschritt (mit evtl. vorhandenen zusätzlichen Informationen) auf <code>outStream
     * </code> ausgegeben.
     */
        ...
        switch (statusTag) {
            ...
            case STATUS_INST_BPD_INIT_DONE:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_REC_INST_DATA_DONE", passport.getBPDVersion()));
                break;
            ...
            case STATUS_INIT_SYSID_DONE:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_REC_SYSID_DONE", o[1].toString()));
                getOutStream().println("status: " + ((HBCIMsgStatus) o[0]).toString());
                break;
            ...
            case STATUS_INIT_SIGID_DONE:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_REC_SIGID_DONE", o[1].toString()));
                getOutStream().println("status: " + ((HBCIMsgStatus) o[0]).toString());
                break;
            ...
            case STATUS_INIT_UPD_DONE:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_REC_USER_DATA_DONE", passport.getUPDVersion()));
                break;
            ...
            case STATUS_DIALOG_INIT_DONE:
                getOutStream()
                        .println(HBCIUtilsInternal.getLocMsg("STATUS_DIALOG_INIT_DONE", o[1]));
                getOutStream().println("status: " + ((HBCIMsgStatus) o[0]).toString());
                break;
            case STATUS_SEND_TASK:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_DIALOG_NEW_JOB", ((HBCIJob) o[0]).getName()));
                break;
            case STATUS_SEND_TASK_DONE:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_DIALOG_JOB_DONE", ((HBCIJob) o[0]).getName()));
                break;
            ...
            case STATUS_MSG_CREATE:
                getOutStream()
                        .println(
                                "  "
                                        + HBCIUtilsInternal.getLocMsg(
                                                "STATUS_MSG_CREATE", o[0].toString()));
                break;
            ...
            case STATUS_MSG_RAW_SEND:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_MSG_RAW_SEND", o[0].toString()));
                break;
            ...
            case STATUS_MSG_RAW_RECV:
                getOutStream()
                        .println(
                                HBCIUtilsInternal.getLocMsg(
                                        "STATUS_MSG_RAW_RECV", o[0].toString()));
                break;

            default:
                throw new HBCI_Exception(
                        HBCIUtilsInternal.getLocMsg("STATUS_INVALID", Integer.toString(statusTag)));
        }
    }
}
