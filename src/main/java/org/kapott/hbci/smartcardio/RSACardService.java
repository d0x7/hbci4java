...
public class RSACardService extends HBCICardService {
    ...
    static final int ISO7816_CLA_STD = SECCOS_CLA_STD;

    static final int ISO7816_INS_SELECT_FILE = SECCOS_INS_SELECT_FILE;
    static final int ISO7816_INS_VERIFY = SECCOS_INS_VERIFY;
    static final int ISO7816_INS_READ_BINARY = 0xB0;

    static final int ISO7816_PWD_TYPE_DF = SECCOS_PWD_TYPE_DF;
    ...
    private void selectFile(int fid) {
        ...
        CommandAPDU command =
                new CommandAPDU(ISO7816_CLA_STD, ISO7816_INS_SELECT_FILE, 0x00, 0x0C, data);
    ...
    //                                              (byte) 0x00, (byte) (ISO7816_PWD_TYPE_DF |
    // pwdId),
    //                                              body);
    //        send(command);
    //    }
    ...
    public void verifySoftPIN(int pwdId, byte[] softPin) {
        if (softPin.length > 8) throw new HBCI_Exception("illegal PIN size");
        ...
        CommandAPDU command =
                new CommandAPDU(
                        ...
                        ISO7816_INS_VERIFY,
                        (byte) 0x00,
                        (byte) (ISO7816_PWD_TYPE_DF | pwdId),
                        body);
        send(command);
    }
        ...
        if (pkCount != null && pkCount.length == 1) {
            for (int n = 0; n < pkCount[0]; n++) {
                ...
                if (pkData != null && pkData.length == 0x79) {
                    if (pkData[0] == (byte) (0x91 + idx)) verifyPKData = pkData;
                    if (pkData[0] == (byte) (0x96 + idx)) encipherPKData = pkData;
                    if (pkData[0] == (byte) (0x81 + idx)) signPKData = pkData;
                    if (pkData[0] == (byte) (0x86 + idx)) decipherPKData = pkData;
                }
            }
        }
        selectFile(0xA602);
    ...
    public int readSigId(int idx) {
        ...
        return ((((int) rawData[0]) & 0xFF) << 24)
                | ((((int) rawData[1]) & 0xFF) << 16)
                | ((((int) rawData[2]) & 0xFF) << 8)
                | (((int) rawData[3]) & 0xFF);
    }
    ...
    static class SimpleTLV {
        ...
        public SimpleTLV(byte[] data) {
            ...
            for (int n = 0; n < tags.size(); n++) tempTags[n] = tags.get(n);

            this.malformed = pos == Integer.MAX_VALUE;
            this.data = data.clone();
            this.tags = this.malformed ? new byte[0] : tempTags;
            this.contents =
                    this.malformed ? new byte[0][0] : contents.toArray(new byte[contents.size()][]);
        }
