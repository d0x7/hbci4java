...
public abstract class HBCICardService {
    static final int HBCI_DDV_EF_ID = 0x19;
    static final int HBCI_DDV_EF_BNK = 0x1A;
    static final int HBCI_DDV_EF_MAC = 0x1B;
    static final int HBCI_DDV_EF_SEQ = 0x1C;
    static final int SECCOS_SELECT_RET_NOTHING = 0x0c;
    static final int SECCOS_CLA_EXT = 0xb0;
    static final int SECCOS_CLA_SM_PROPR = 0x04;
    static final int SECCOS_CLA_SM1 = 0x08;
    static final int SECCOS_CLA_STD = 0x00;
    static final int SECCOS_INS_GET_CHALLENGE = 0x84;
    static final int SECCOS_INS_GET_KEYINFO = 0xee;
    static final int SECCOS_INS_INT_AUTH = 0x88;
    static final int SECCOS_INS_PUT_DATA = 0xda;
    static final int SECCOS_INS_READ_RECORD = 0xb2;
    static final int SECCOS_INS_SELECT_FILE = 0xa4;
    static final int SECCOS_INS_VERIFY = 0x20;
    static final int SECCOS_INS_UPDATE_RECORD = 0xdc;
    static final int SECCOS_INS_WRITE_RECORD = 0xd2;
    static final int SECCOS_KEY_TYPE_DF = 0x80;
    static final int SECCOS_PWD_TYPE_DF = 0x80;
    static final byte SECCOS_SM_CRT_CC = (byte) 0xb4;
    static final byte SECCOS_SM_REF_INIT_DATA = (byte) 0x87;
    static final byte SECCOS_SM_RESP_DESCR = (byte) 0xba;
    static final byte SECCOS_SM_VALUE_LE = (byte) 0x96;
    ...
    private static final int IOCTL_GET_FEATURE_REQUEST = SCARD_CTL_CODE(3400);
    ...
    public void init(Card card) {
        ...
        try {
            ...
            byte[] response =
                    this.smartCard.transmitControlCommand(IOCTL_GET_FEATURE_REQUEST, new byte[0]);
            for (int i = 0; i < response.length; i += 6) {
                Byte feature = new Byte(response[i]);
                Integer ioctl =
                        new Integer((0xff & response[i + 2]) << 24)
                ...
                HBCIUtils.log(
                        "  " + name + ": " + Integer.toHexString(ioctl.intValue()),
                        HBCIUtils.LOG_INFO);
    ...
    public void verifyHardPIN(int pwdId) {
        try {
            byte[] response =
                    this.smartCard.transmitControlCommand(
                            features.get(FEATURE_VERIFY_PIN_DIRECT),
                            this.createPINVerificationDataStructure(pwdId));
            ...
            if (sw == 0x6983)
                throw new HBCI_Exception(
                        "Chipkarte ist gesperrt oder besitzt ein unbekanntes Format");
            //
            //////////////////////////////////////////////////////////////////////////
    ...
    public void verifySoftPIN(int pwdId, byte[] softPin) {
        ...
        for (int i = 0; i < softPin.length; i++) {
            ...
            body[1 + (i >> 1)] |=
                    (byte) (((softPin[i] - (byte) 0x30) << (4 - (4 * (i & 1)))) & 0xFF);
        }
        CommandAPDU command =
                new CommandAPDU(
                        SECCOS_CLA_STD,
    ...
    protected byte[] getKeyInfo(int idx) {
        CommandAPDU command =
                new CommandAPDU(
                        SECCOS_CLA_EXT, SECCOS_INS_GET_KEYINFO, SECCOS_KEY_TYPE_DF, (idx + 1), 256);
        return receive(command);
    }
    ...
    protected byte[] getChallenge() {
        CommandAPDU command =
                new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_GET_CHALLENGE, 0x00, 0x00, 8);
        return receive(command);
    }
    ...
    /**
     * Sendet ein Kommando an den Kartenleser, prueft ob es erfolgreich ausgefuehrt wurde und
     * liefert die Antwort zurueck.
     *
     * @param command das Kommando.
     * @return die Antwort.
     */
    ...
    private void check(ResponseAPDU response, byte[] returncodes) {
        ...
        if (msg != null) throw new HBCI_Exception("Fehler " + code + ": " + msg);
    ...
    protected byte[] createPINVerificationDataStructure(int pwdId) throws IOException {
        ...
        verifyCommand.write(
                new byte[] {(byte) 8, (byte) 4}); // PIN size (max/min), volker: 12,4=>8,4
