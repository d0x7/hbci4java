...
public final class CommPinTan extends Comm {
        ...
        try {
            ...
            HBCIUtils.log(
                    HBCIUtilsInternal.getLocMsg(
                            "LOG_CONNECT", new Object[] {host, parentPassport.getPort(), path}),
                    HBCIUtils.LOG_INFO);
            this.url = new URL("https", host, parentPassport.getPort().intValue(), path);

            // creating instances of modified socket factories etc.
            this.mySocketFactory =
                    new PinTanSSLSocketFactory((AbstractPinTanPassport) parentPassport);
            ...
            if (proxyData.length == 2) {
                ...
                HBCIUtils.log(
                        "initializing HBCI4Java proxy authentication callback",
                        HBCIUtils.LOG_DEBUG);
    ...
    protected void ping(MSG msg) {
        try {
            ...
            if (!checkCert || debugging) {
                ...
                HBCIUtils.log(
                        "activating modified socket factory for"
                                + " checkCert="
                                + checkCert
                                + " and debugging="
                                + debugging,
                ...
                if (!checkCert) {
                    ...
                    HBCIUtils.log(
                            "activating modified hostname verifier because cert checking is disabled",
                            HBCIUtils.LOG_DEBUG);
    ...
    protected StringBuffer pong(MsgGen gen) {
        try {
            ...
            // dass es Banken gibt, die einen anonymen BPD-Abruf mit einem HTTP-Fehlercode
            // quittieren
