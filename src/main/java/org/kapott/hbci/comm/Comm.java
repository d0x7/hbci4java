...
public abstract class Comm {
    ...
    protected Comm(HBCIPassportInternal parentPassport) {
        ...
        HBCIUtilsInternal.getCallback()
                .callback(
                        ...
                        HBCIUtilsInternal.getLocMsg("CALLB_NEED_CONN"),
    ...
    public MSG pingpong(String msgName, MSG msg) {
        ...
        HBCIUtilsInternal.getCallback()
                .status(getParentPassport(), HBCICallback.STATUS_MSG_SEND, null);
        HBCIUtilsInternal.getCallback()
                .status(getParentPassport(), HBCICallback.STATUS_MSG_RAW_SEND, msg.toString(0));
        ping(msg);

        // nachricht empfangen
        HBCIUtilsInternal.getCallback()
                .status(getParentPassport(), HBCICallback.STATUS_MSG_RECV, null);
        String st = pong(gen).toString();
        HBCIUtilsInternal.getCallback()
                .status(getParentPassport(), HBCICallback.STATUS_MSG_RAW_RECV, st);
        ...
        try {
            ...
            HBCIUtilsInternal.getCallback()
                    .status(getParentPassport(), HBCICallback.STATUS_MSG_PARSE, "CryptedRes");
            try {
                HBCIUtils.log("trying to parse message as crypted message", HBCIUtils.LOG_DEBUG);
                retmsg =
                        MSGFactory.getInstance()
                                .createMSG("CryptedRes", st, st.length(), gen, MSG.DONT_CHECK_SEQ);
            } catch (ParseErrorException e) {
                // wenn das schiefgeht...
                HBCIUtils.log(
                        "message seems not to be encrypted; tring to parse it as "
                                + msgName
                                + "Res message",
                        HBCIUtils.LOG_DEBUG);
            ...
            throw new CanNotParseMessageException(
                    HBCIUtilsInternal.getLocMsg("EXCMSG_CANTPARSE"), st, ex);
        }
    ...
    public void close() {
        ...
        HBCIUtilsInternal.getCallback()
                .callback(
                        ...
                        HBCIUtilsInternal.getLocMsg("CALLB_CLOSE_CONN"),
                        HBCICallback.TYPE_NONE,
                        new StringBuffer());
    }
}
