...
public class PinTanSSLSocketFactory extends SSLSocketFactory {
    ...
    public PinTanSSLSocketFactory(AbstractPinTanPassport passport) {
        try {
            if (!passport.getCheckCert()) {
                HBCIUtils.log(
                        "creating socket factory with disabled cert checking", HBCIUtils.LOG_WARN);

                SSLContext sslContext = SSLContext.getInstance("SSL");
                sslContext.init(
                        null, new TrustManager[] {new PinTanSSLTrustManager()}, new SecureRandom());
    ...
    private OutputStream getLogger() {
        ...
        if (filename == null || filename.length() == 0) {
            HBCIUtils.log(
                    "no log.ssl.filename specified - logging to HBCI4Java logger",
                    HBCIUtils.LOG_WARN);
