...
public abstract class SyntaxElement {
    ...
    /**
     * < @internal @brief indicates if this element is really valid, i.e. will appear in an outgoing
     * hbci message resp. in returned results from an incoming message
     */
    ...
    private Node def;
    /**
     * wird fuer datenelemente benoetigt, die sonst unbeabsichtigt generiert werden koennten. das
     * problem ist, dass es datenelemente (bisher nur bei segmenten bekannt) gibt, die aus einigen
     * "required" unterelementen bestehen und aus einigen optionalen unterelementen. wenn *alle*
     * "required" elemente bereits durch predefined values bzw. durch automatisch generierte werte
     * vorgegeben sind, dann wird das entsprechende element erzeugt, da es auch ohne angabe der
     * optionalen unterelemente gueltig ist.
     *
     * <p>es ist aber u.U. gar nicht beabsichtigt, dass dieses element erzeugt wird (beispiel
     * segment "KIOffer", wo nur die DEG SegHead required ist, alle anderen elemente sind optional).
     * es kann also vorkommen, dass ein element *unbeabsichtigt* nur aus den vorgabedaten erzeugt
     * wird.
     *
     * <p>bei den elementen, bei denen das passieren kann, wird in der xml-spezifikation deshalb
     * zusaetzlich das attribut "needsRequestTag" angegeben. der wert dieses attributes wird hier in
     * der variablen @p needsRequestTag gespeichert.
     *
     * <p>beim ueberpruefen, ob das aktuelle element gueltig ist (mittels @c validate() ), wird
     * neben der gueltigkeit aller unterelemente zusaetzlich ueberprueft, ob dieses element ein
     * request-tag benoetigt, und wenn ja, ob es vorhanden ist. wenn die
     *
     * @p needsRequestTag -bedingung nicht erfuellt ist, ist auch das element ungueltig, und es wird
     *     nicht erzeugt.
     *     <p>das vorhandensein eines request-tags wird in der variablen @haveRequestTag
     *     gespeichert. dieses flag kann fuer ein bestimmtes element gesetzt werden, indem ihm der
     *     wert "requested" zugewiesen wird. normalerweise kann nur DE-elementen ein wert zugewiesen
     *     werden, diese benoetigen aber kein request-tag. wird also einem gruppierenden element der
     *     wert "requested" zugewiesen, dann wird das durch die methode @c propagateValue() als
     *     explizites setzen des @p haveRequestTag interpretiert.
     *     <p>alle klassen und methoden, die also daten fuer die erzeugung von nachrichten
     *     generieren, muessen u.U. fuer bestimmte syntaxelemente diesen "requested"-wert setzen.
     *     <p>needsRequestTag kann komplett weg, oder? -- nein. Für GV-Segmente gilt das schon. Die
     *     Überprüfung des requested-Werted findet aber in der *allgemeinen* SyntaxElement-Klasse
     *     statt, wo auch andere Segmente (z.b. MsgHead) erzeugt werden. Wenn als "allgemeiner"
     *     Check der Check "if SEG.isRequested" eingeführt werden würde, dann würde der nur bei
     *     tatsächlich gewünschten GV-Segmenten true ergeben. Bei MsgHead-Segmenten z.B. würde er
     *     false ergeben (weil diese Segmente niemals auf "requested" gesetzt werden). Deshalb darf
     *     diese "requested"-Überprüfung nur bei den Syntaxelementen stattfinden, bei denen das
     *     explizit gewünscht ist (needsRequestTag).
     */
    ...
    /**
     * es wird ein syntaxelement mit der id 'name' initialisiert; der pfad bis zu diesem element
     * wird in 'path' uebergeben; 'idx' ist die nummer dieses elementes innerhalb der
     * syntaxelementliste fuer dieses element (falls ein bestimmtes syntaxelement mehr als einmal
     * auftreten kann)
     */
    ...
    /**
     * beim parsen: initialisiert ein neues syntaxelement mit der id 'name'; in 'path' wird der pfad
     * bis zu dieser stelle uebergeben 'predelim' gibt das delimiter-zeichen an, das beim parsen vor
     * diesem syntax- element stehen muesste 'idx' ist die nummer des syntaxelementes innerhalb der
     * uebergeordneten liste (die liste repraesentiert das evtl. mehrmalige auftreten eines
     * syntaxelementes, siehe class syntaxelementlist) 'res' ist der zu parsende String 'predefs'
     * soll eine menge von pfad-wert-paaren enthalten, die fuer einige syntaxelemente den wert
     * angeben, den diese elemente zwingend haben muessen (z.b. ein bestimmter segmentcode o.ae.)
     */
    ...
    /**
     * liefert das delimiter-zeichen zurueck, dass innerhalb dieses syntaxelementes benutzt wird, um
     * die einzelnen child-elemente voneinander zu trennen
     */
    protected abstract char getInDelim();

    /**
     * erzeugt einen neuen Child-Container, welcher durch den xml-knoten 'ref' identifiziert wird;
     * wird beim erzeugen von elementen benutzt
     */
     ...
     * an, um welches element es sich dabei handelt; aus 'res' (der zu parsende String) wird der
     * wert fuer das element ermittelt (falls es sich um ein de handelt); in 'predefined' ist der
     * wert des elementes zu finden, der laut syntaxdefinition ('syntax') an dieser stelle
     * auftauchen mueste (optional; z.b. fuer segmentcodes); 'predelim*' geben die delimiter an, die
     * direkt vor dem zu erzeugenden syntaxelement auftauchen muessten
     */
    protected abstract MultipleSyntaxElements parseNewChildContainer(
            Node ref,
            char predelim0,
            char predelim1,
            StringBuffer res,
    ...
    private void initData(String type, String name, String ppath, int idx, Document syntax) {
        if (getElementTypeName().equals("SEG"))
            HBCIUtils.log(
                    "creating segment " + ppath + " -> " + name + "(" + idx + ")",
        ...
        if (ppath != null && ppath.length() != 0) temppath.append(ppath).append(".");
        ...
        if (syntax != null) {
            ...
            if (requestTag != null && requestTag.equals("1")) needsRequestTag = true;
            ...
            try {
                ...
                for (Node ref = def.getFirstChild(); ref != null; ref = ref.getNextSibling()) {
                    if (ref.getNodeType() == Node.ELEMENT_NODE) {
                        MultipleSyntaxElements child =
                                createAndAppendNewChildContainer(ref, syntax);
                        if (child != null) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    NodeList validvalueNodes =
                            ((Element) (validNode)).getElementsByTagName("validvalue");
                ...
                for (Iterator<MultipleSyntaxElements> i = getChildContainers().iterator();
                        i.hasNext(); ) {
    ...
    protected MultipleSyntaxElements createAndAppendNewChildContainer(Node ref, Document syntax) {
        ...
        if (ret != null) addChildContainer(ret);
        return ret;
    }
    ...
    /**
     * loop through all child-elements; the segments found there will be sequentially enumerated
     * starting with num startValue; if startValue is zero, the segments will not be enumerated, but
     * all given the number 0
     *
    ...
    public int enumerateSegs(int startValue, boolean allowOverwrite) {
        ...
        for (Iterator<MultipleSyntaxElements> i = getChildContainers().iterator(); i.hasNext(); ) {
            ...
            if (s != null) idx = s.enumerateSegs(idx, allowOverwrite);
        }
                ...
                for (Node ref = def.getFirstChild(); ref != null; ref = ref.getNextSibling()) {
                    if (ref.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        if (child != null) {
                            ...
                            // TODO: this is a very very dirty hack to fix the problem with the
                            // params-template;
                            // bei der SF "Params", die mit <SF type="Params" maxnum="0"/>
                            // referenziert wird,
                            // soll nach jedem erfolgreich in die SF aufgenommenen Param-Segment
                            // eine neue
                            // SF begonnen werden, damit das Problem mit dem am Ende der SF
                            // stehenden Template-
                            // Param-Segment nicht mehr auftritt
                            // dazu wird beim hinzufuegen von segmenten zur sf ueberprueft, ob diese
                            // evtl. bereits
                            // segmente enthaelt (hasValidChilds()). falls das der fall ist, so wird
                            // kein neues segment hinzugefuegt
                            // analoges gilt für die SF "GVRes" - hier muss dafür gesorgt werden,
                            // dass jede
                            // antwort in ein eigenes GVRes kommt, damit die zuordnung
                            // reihenfolge-erkennung
                            // der empfangenen GVRes-segmente funktioniert (in
                            // HBCIJobImpl.fillJobResult())
    ...
    protected MultipleSyntaxElements parseAndAppendNewChildContainer(
            Node ref,
            char predelim0,
            char predelim1,
            StringBuffer res,
            ...
            Hashtable<String, String> predefs,
            Hashtable<String, String> valids) {
        MultipleSyntaxElements ret =
                parseNewChildContainer(
                        ref, predelim0, predelim1, res, fullResLen, syntax, predefs, valids);
        if (ret != null) addChildContainer(ret);
        return ret;
    }

    /**
     * fuellt die hashtable 'values' mit den werten der de-syntaxelemente; dazu wird in allen
     * anderen typen von syntaxelementen die liste der child-elemente durchlaufen und deren
     * 'fillValues' methode aufgerufen
     */
    ...
    /**
     * @return the ArrayList containing all child-elements (the elements of the ArrayList are
     *     instances of the SyntaxElementArray class
     */
    ...
    /**
     * setzt den wert eines de; in allen syntaxelementen ausser DE wird dazu die liste der
     * child-elemente durchlaufen; jedem dieser child-elemente wird der wert zum setzen uebergeben;
     * genau _eines_ dieser elemente wird sich dafuer zustaendig fuehlen (das DE mit
     * 'path'='destPath') und den wert uebernehmen
     */
    // TODO: code splitten
    public boolean propagateValue(
            String destPath, String value, boolean tryToCreate, boolean allowOverwrite) {
        boolean ret = false;

        if (destPath.equals(getPath())) {
            if (value != null && value.equals("requested")) this.haveRequestTag = true;
            else
                throw new HBCI_Exception(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_INVVALUE", new Object[] {destPath, value}));
            ret = true;
        } else {
            // damit überspringen wir gleich elemente, bei denen es mit
            // sicherheit nicht funktionieren kann
            if (destPath.startsWith(getPath())) {
                for (Iterator<MultipleSyntaxElements> i = childContainers.listIterator();
                        i.hasNext(); ) {
                    MultipleSyntaxElements l = i.next();
                ...
                if (!ret && tryToCreate) {
                    ...
                    // nicht
                    HBCIUtils.log(
                            getPath() + ": could not set value for " + destPath,
                            HBCIUtils.LOG_INTERN);
                    ...
                    boolean found = false;
                    for (Iterator<MultipleSyntaxElements> i = childContainers.iterator();
                            i.hasNext(); ) {
                    ...
                    if (!found) {
                        ...
                        if (found) {
                            ...
                            HBCIUtils.log(
                                    "  inserting child container with syntaxIdx "
                                            + newChildIdx
                                            + " at position "
                                            + newPosi,
                                    HBCIUtils.LOG_INTERN);
                            ...
                            ret =
                                    child.propagateValue(
                                            destPath, value, tryToCreate, allowOverwrite);
                        }
                    } else {
                        HBCIUtils.log(
                                "  subtype "
                                        + subType
                                        + " already existing - will not try to create",
                                HBCIUtils.LOG_INTERN);
                    }
                }
            }
    ...
    public String getValueOfDE(String path, int zero) {
        ...
        for (ListIterator<MultipleSyntaxElements> i = childContainers.listIterator();
                i.hasNext(); ) {
            MultipleSyntaxElements l = i.next();
    ...
    /**
     * diese toString() methode wird benutzt, um den wert eines de-syntaxelementes in
     * human-readable-form zurueckzugeben. innerhalb eines de-elementes wird der wert in der
     * hbci-form gespeichert
     */
    ...
    /**
     * ueberpreuft, ob das syntaxelement alle restriktionen einhaelt; ist das nicht der fall, so
     * wird eine Exception ausgeloest. die meisten syntaxelemente koennen sich nicht selbst
     * ueberpruefen, sondern rufen statt dessen die validate-funktion der child-elemente auf
     */
