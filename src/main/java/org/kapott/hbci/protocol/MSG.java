...
public final class MSG extends SyntaxElement {
    ...
    /**
     * in 'clientValues' wird eine hashtable uebergeben, die als schluessel den pfadnames und als
     * wert den wert eines zu setzenden elementes enthaelt. mit der methode werden vom nutzer
     * einzugebenede daten (wie kontonummern, namen usw.) in die generierte nachricht eingebaut
     */
    ...
    private void setMsgSizeValue(MsgGen gen, int value, boolean allowOverwrite) {
        ...
        if (msgsizeElem == null) throw new NoSuchPathException(absPath);
    ...
    public String toString(int zero) {
        ...
        if (isValid())
            for (Iterator<MultipleSyntaxElements> i = getChildContainers().listIterator();
                    i.hasNext(); ) {
                MultipleSyntaxElements list = i.next();

                if (list != null) ret.append(list.toString(0));
            }
    ...
    protected MultipleSyntaxElements parseNewChildContainer(
        ...
        if ((segref.getNodeName()).equals("SEG"))
            ret =
                    MultipleSEGsFactory.getInstance()
                            .createMultipleSEGs(
                                    segref,
                                    getPath(),
                                    predelim0,
            ...
            ret =
                    MultipleSFsFactory.getInstance()
                            .createMultipleSFs(
                                    segref,
                                    getPath(),
                                    predelim0,
    ...
    public String getValueOfDE(String path) {
        ...
        if (ret == null) throw new NoSuchPathException(path);
