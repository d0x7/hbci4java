...
public abstract class MultipleSyntaxElements {
    ...
    /**
     * anlegen eines neuen syntaxelementarrays fuer ein syntaxelement; ref ist eine
     * xml-node-referenz auf das syntaxelement
     */
    ...
    /**
     * anlegen einer neuen syntaxelementlist beim parsen des strings res; - ref ist dabei die
     * referenz auf einen xml-node, der das syntaxelement festlegt, fuer den die syntaxelementlist
     * erzeugt werden soll; - predefs siehe SyntaxElement() - predelim0 ist der delimiter, der vor
     * dem ersten element innerhalb dieser syntaxelementlist auftreten muesste; - predelim1 ist der
     * delimiter, der vor dem zweiten, dritten, usw. element in der syntaxelementlist auftreten
     * muesste (der unterschied zwischen predelim0 und predelim1 ist der, dass predelim0 evtl. von
     * uebergeordneten elementen propagiert wird (z.b. wenn die syntaxelementlist selbst das erste
     * syntaxelement einer msg repraesentiert), predelim1 ist allerdings immer der delimiter, der
     * fuer das aktuell uebergeordnete syntaxelement zu verwenden ist)
     */
    ...
    /**
     * erzeugt einen neuen eintrag in der elements liste; dabei wird ein syntaxelement erzeugt, das
     * im xml-node ref referenziert wird; idx ist die indexnummer des zu erzeugenden syntaxelementes
    ...
    protected abstract SyntaxElement parseAndAppendNewElement(
            Node ref,
            String path,
            char predelim,
            int idx,
    ...
    private void initData(Node ref, String path, Document syntax) {
        ...
        if (path != null && path.length() != 0) temppath.append(path).append(".");
        ...
        if (st.length() != 0) minnum = Integer.parseInt(st);

        maxnum = 1;
        st = ((Element) ref).getAttribute("maxnum");
        if (st.length() != 0) maxnum = Integer.parseInt(st);
    ...
    protected boolean propagateValue(
        ...
        if (tryToCreate) {
            ...
            if (destPath.startsWith(path)
                ...
                if (nextDot == -1) nextDot = destPathLen;
    ...
    protected void validateOneElement(SyntaxElement elem, int idx) {
        ...
        if (maxnum > 0 && idx >= maxnum) throw new TooMuchElementsException(getPath(), idx);
    }
        ...
        try {
            while (!ready) {
                ...
                try {
                    ...
                    // [willuhn 2012-03-06, BUG 1129] weiterwerfen, wenn sie als fatal eingestuft
                    // ist
                    ...
                    if (save.length() > 1) {
                        ...
                        //////////////////////////////////////////////////////////////////////
                        // Dirty-Hack fuer Sonderfall optionale MultipleDEGs. Also eine Liste vieler
                        // DEGs,
                        ...
                        // Siehe
                        // http://www.onlinebanking-forum.de/forum/topic.php?t=19879&page=last#last_post
                        if ((this instanceof MultipleDEGs)
                                && this.minnum == 0
                                && this.maxnum > 1
                                && idx > 1) {
                            int size = this.maxnum - idx;
                            if (size > save.length()) size = save.length();
                            String rest = save.substring(0, size);
                            if (containsOnly(rest, '+')) {
                                HBCIUtils.log(
                                        "applying shortcut for optional MultipleDEGs, have no more content in according range",
                                        HBCIUtils.LOG_DEBUG);
                                res.replace(
                                        0,
                                        res.length(),
                                        res.substring(
                                                size - 1)); // Wir schneiden die "+++++..." alle weg
                                ready = true;
                                continue;
                            }
                        }
                    ...
                    if (idx < minnum)
                        throw new ParseErrorException(
                                "reststring in " + getPath() + ": " + res.toString(), e);

                    // es wird nur dann aufgehoert, weitere elemente dem aktuellen container hinzu-
                    // zufuegen, wenn ein element gefunden wurde, was offentsichlich nicht mehr
                    // dazu-
                    // gehoert (exception, aber nicht leeres element) --> dann stimmt naemlich
                    // entweder
                ...
                if ((maxnum != 0 && idx >= maxnum)
                        || (maxnum == 0
                                && save.toString().equals(res.toString())
                                && !emptyElementFound)) {
                    ready = true;
                }
            }
