...
public final class DE extends SyntaxElement {
    ...
    public DE(
        ...
        super(
                ((Element) dedef).getAttribute("type"),
                name,
                path,
                predelim,
                idx,
    ...
    public boolean propagateValue(
            String destPath, String valueString, boolean tryToCreate, boolean allowOverwrite) {
        boolean ret = false;
    ...
    public String getValueOfDE(String path) {
        ...
        if (path.equals(getPath())) ret = value.toString();
    ...
    public String getValueOfDE(String path, int zero) {
        ...
        if (path.equals(getPath())) ret = value.toString(0);
    ...
    private void initData(Node dedef, String name, String path, int idx, Document syntax) {
        ...
        if (st.length() != 0) minsize = Integer.parseInt(st);

        maxsize = 0;
        st = ((Element) dedef).getAttribute("maxsize");
        if (st.length() != 0) maxsize = Integer.parseInt(st);
    }
    ...
    /**
     * validierung eines DE: validate ist ok, wenn DE einen wert enthaelt und der wert in der liste
     * der gueltigen werte auftaucht
     */
        ...
        if (validssize != 0) {
            ...
            if (!ok) {
                if (!HBCIUtilsInternal.ignoreError(
                        ...
                        "*** invalid value for " + getPath() + ": " + valString))
    ...
    public void setValue(String st) {
        this.value =
                SyntaxDEFactory.getInstance()
                        .createSyntaxDE(getType(), getPath(), st, minsize, maxsize);
    }
    ...
    /**
     * anlegen eines de beim parsen funktioniert analog zum anlegen eines de bei der
     * message-synthese
     */
    private void parseValue(
            StringBuffer res, Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        ...
        if (preDelim != (char) 0 && temp.charAt(0) != preDelim) {
            if (len == 0) {
                throw new ParseErrorException(
                        HBCIUtilsInternal.getLocMsg("EXCMSG_ENDOFSTRG", getPath()));
            }

            // HBCIUtils.log("error string: "+res.toString(),HBCIUtils.LOG_ERR);
            // HBCIUtils.log("current:
            // "+getPath()+":"+type+"("+minsize+","+maxsize+")="+value,HBCIUtils.LOG_ERR);
            // HBCIUtils.log("predelimiter mismatch (required:"+getPreDelim()+"
            // found:"+temp.charAt(0)+")",HBCIUtils.LOG_ERR);
            throw new PredelimErrorException(
                    getPath(), Character.toString(preDelim), Character.toString(temp.charAt(0)));
        }

        this.value =
                SyntaxDEFactory.getInstance()
                        .createSyntaxDE(getType(), getPath(), res, minsize, maxsize);
            ...
            if (!valueString.equals(predefined)) {
                throw new ParseErrorException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_PREDEFERR", new Object[] {getPath(), predefined, value}));
            }
        }
        ...
        if (atLeastOne && !ok) {
            if (!HBCIUtilsInternal.ignoreError(
                    null,
                    "client.errors.ignoreValidValueErrors",
                    "*** invalid value for " + getPath() + ": " + valueString))
    ...
    public void extractValues(Hashtable<String, String> values) {
        if (isValid()) values.put(getPath(), value.toString());
    }
