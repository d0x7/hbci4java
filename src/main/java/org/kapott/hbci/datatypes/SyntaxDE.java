 ...
 *     <p>This package contains a collection of classes, where each class represents one of the
 *     datatypes defined in HBCI syntax descriptions (e.g. AN for alphanumeric data, Bin for binary
 *     data etc.). These classes are used during the message generation process either when building
 *     a new message to be sent or when parsing a received message. These classes are never
 *     instantiated directly, but they are loaded and instantiated dynamically (for a certain syntax
 *     element, the name of the class to be used for storing its value is determined by the value of
 *     the attribute @c type in the syntax description for this syntax element). The classname to be
 *     loaded is built from the @c type attribute by prepending @c org.kapott.hbci.datatypes.Syntax
 */
...
public abstract class SyntaxDE {
     ...
     *     <p>In this constructor the data element will be initialized with the given value @p x.
     *     During initializing process the size constraints for this element will be verified. If
     *     verification fails, an IllegalArgumentException will be thrown.
     */
     ...
     *     <p>This method is needed when parsing HBCI messages. It checks the first character of
     *     String @p res. If this character is one of the HBCI-specific data-element-delimiters, 1
     *     will be returned, else 0 (i.e. the return value is the index into the String @p res where
     *     we can start fetching the next syntax token)
     */
     ...
     * @return The position of the next delimiter character in @p res. If there is no next delimiter
     *     character, the return value will be the next character after the end of the String @p res
     *     (i.e. will equal @c res.length())
     * @internal
     * @brief Returns the index of the next delimiter token.
     *     <p>This method is needed when parsing an HBCI message. It scans
     * @p res starting at position @p startidx for the next occurence of an HBCI-specific
     *     data-element-delimiter and returns its position.
     */
    ...
    protected final void setContent(String st, int minsize, int maxsize) {
        ...
        if (len < minsize || (maxsize != 0 && len > maxsize)) {
            ...
            if (len == 0
                    || !HBCIUtilsInternal.ignoreError(
                            null, "client.errors.ignoreWrongDataLengthErrors", msg))
