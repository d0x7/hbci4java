...
public class SyntaxDEFactory {
    ...
    public SyntaxDE createSyntaxDE(
            String dataType, String path, String value, int minsize, int maxsize) {
        SyntaxDE ret = null;
        ...
        synchronized (this) {
            ...
            if (factory == null) {
                factory =
                        new ObjectFactory(
                                Integer.parseInt(
                                        HBCIUtils.getParam("kernel.objpool.Syntax", "1024")));
        ...
        if (ret == null) {
            ...
            try {
                c =
                        Class.forName(
                                "org.kapott.hbci.datatypes.Syntax" + dataType,
                                false,
            ...
            try {
                ret =
                        (SyntaxDE)
                                (con.newInstance(
                                        new Object[] {
                                            value, new Integer(minsize), new Integer(maxsize)
                                        }));
    ...
    public SyntaxDE createSyntaxDE(
            String dataType, String path, StringBuffer res, int minsize, int maxsize) {
        SyntaxDE ret = null;
        ObjectFactory factory;
            ...
            try {
                ret =
                        (SyntaxDE)
                                (con.newInstance(
                                        new Object[] {
                                            res, new Integer(minsize), new Integer(maxsize)
                                        }));
            } catch (InstantiationException e) {
            } catch (IllegalAccessException e) {
            } catch (InvocationTargetException e) {
                throw new ParseErrorException(
                        HBCIUtilsInternal.getLocMsg("EXCMSG_PROT_ERRSYNDE", path),
