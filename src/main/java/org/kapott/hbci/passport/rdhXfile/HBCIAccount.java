...
public class HBCIAccount extends TLV {
            ...
            if (key.getKeyType().equals(keytype)) {
                ...
                ret[0] =
                        new HBCIKey(
                                ...
                                getBLZ(),
                                getUserId(),
                                Integer.toString(key.getKeyNum()),
                                Integer.toString(key.getKeyVersion()),
                                key.getPublicKey());
    ...
    public static class UserKeys extends TLV {
        ...
        public void decrypt(SecretKey key) throws Exception {
            ...
            offset += len;
            // System.out.println("public and private modulus equal:
            // "+Arrays.equals(this.modulus,modulus2));
        ...
        public void setPrivateKey(Key key) {
            if (key instanceof RSAPrivateCrtKey2) {
                ...
                BigInteger modulus =
                        new BigInteger(+1, this.p).multiply(new BigInteger(+1, this.q));

                this.Ap =
                        trimba(
                                new BigInteger(+1, this.q)
                                        .modPow(new BigInteger(+1, this.p).subtract(one), modulus)
                                        .toByteArray());
                this.Aq =
                        trimba(
                                modulus.add(one)
                                        .subtract(new BigInteger(+1, this.Ap))
                                        .toByteArray());
            }
        }
