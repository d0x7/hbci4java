...
public class RDHXFile {
    ...
    public RDHXFile(byte[] data, byte[] passphrase) {
        ...
        while (posi < len) {
            ...
            if (tag == 0x4e56L) {
                ...
                throw new HBCI_Exception(
                        "*** invalid field tag found: 0x" + Long.toString(tlv.getTag(), 16));
            }
        ...
        if (getField(FileHeader.class) == null) {
            ...
            throw new HBCI_Exception(
                    "*** RDH-2/10-file does not contain a header field - aborting");
        }
        ...
        if (macfield != null) {
            ...
            HBCIUtils.log(
                    "RDH-2/10-file does not contain a MAC field - ignoring this for now",
                    HBCIUtils.LOG_ERR);
        }
    ...
    private byte[] calculateMAC() {
        try {
            ...
            HBCIUtils.log(
                    "using " + keysize + "-byte-key for MAC calculation", HBCIUtils.LOG_DEBUG);
