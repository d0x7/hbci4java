...
/**
 * Passport-Klasse für die Verwendung von RDH-2- und RDH-10-Schlüsseldateien mit <em>HBCI4Java</em>.
 * RDH-2/10-Schlüsseldateien sind Schlüsseldateien für RDH-Zugänge, die von anderer HBCI-Software
 * erzeugt und verwendet werden (z.B. von <em>VR-NetWorld</em>). Soll eine solche Schlüsseldatei
 * sowohl mit der anderen Software als auch mit <em>HBCI4Java</em> verwendet werden, so kann das mit
 * dieser Passport-Variante geschehen.
 */
        ...
        if (init) {
            ...
            try {
                ...
                if (hbciAccounts.length > 1) {
                    // wenn mehrere bankverbindungen existieren, callback für auswahl der
                    // "richtigen"
                ...
                if (askForMissingData(true, true, true, false, false, true, true)) saveChanges();
            } catch (Exception e) {
