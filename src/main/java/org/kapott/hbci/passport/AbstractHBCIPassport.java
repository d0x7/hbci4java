...
/**
 * Diese Klasse stellt die Basisklasse für alle "echten" Passport-Implementationen dar. Hier werden
 * bereits einige Methoden implementiert sowie einige zusätzliche Hilfsmethoden zur Verfügung
 * gestellt.
 *
 * <p>Aus einer HBCI-Anwendung heraus ist hier nur eine einzige Methode interessant, um eine Instanz
 * eines bestimmtes Passports zu erzeugen
 */
public abstract class AbstractHBCIPassport implements HBCIPassportInternal, Serializable {
    ...
    /**
     * Erzeugt eine Instanz eines HBCIPassports und gibt diese zurück. Der Typ der erzeugten
     * Passport-Instanz wird durch den Parameter <code>name</code> bestimmt. Gültige Werte sind zur
     * Zeit
     *
     * <ul>
     *   <li>DDV
     *   <li>RDHNew
     *   <li>RDH (nicht mehr benutzen!)
     *   <li>PinTan
     *   <li>SIZRDHFile
     *   <li>RDHXFile
     *   <li>Anonymous
     * </ul>
     *
     * <p>Der zusätzliche Parameter <code>init</code> gibt ein Objekt an, welches bereits während
     * der Instanziierung des Passport-Objektes in dessen internen <code>clientData</code>
     * -Datenstrukturen gespeichert wird (siehe {@link
     * org.kapott.hbci.passport.HBCIPassport#setClientData(String, Object)}). Auf dieses Objekt kann
     * dann mit {@link org.kapott.hbci.passport.HBCIPassport#getClientData(String)
     * getClientData("init")} zugegriffen werden. Ist <code>init==null</code>), wo wird <code>
     * init=name</code> gesetzt.
     *
     * <p>Beim Erzeugen eines Passport-Objektes tritt i.d.R. der {@link
     * org.kapott.hbci.callback.HBCICallback Callback} <code>NEED_PASSPHRASE</code> auf, um nach dem
     * Passwort für das Einlesen der Schlüsseldatei zu fragen. Von der Callback-Methode eventuell
     * zusätzlich benötigte Daten zu diesem Passport konnten bis zu dieser Stelle noch nicht via
     * <code>setClientData(...)</code> gesetzt werden, weil das Passport-Objekt noch gar nicht
     * existierte. Für diesen Zweck gibt es das <code>init</code>-Objekt, welches bereits beim
     * Erzeugen des Passport-Objektes (und <em>vor</em> dem Aufrufen eines Callbacks) zu den
     * zusätzlichen Passport-Daten hinzugefügt wird (mit der id "<code>init</code>").
     *
     * <p>Eine beispielhafte (wenn auch nicht sehr praxisnahe) Anwendung dieses Features wird im
     * Quelltext des Tools {@link org.kapott.hbci.tools.AnalyzeReportOfTransactions} gezeigt.
     * Zumindest das Prinzip sollte damit jedoch klar werden.
     *
     * @param name Typ der zu erzeugenden Passport-Instanz
     * @param init Objekt, welches schon während der Passport-Erzeugung via <code>
     *     setClientData("init",init)</code> zu den Passport-Daten hinzugefügt wird.
        ...
        try {
            ...
            throw new InvalidUserDataException(
                    "*** No passport implementation '"
                            + name
                            + "' found - there must be a class "
                            + className);
        } catch (InvocationTargetException ite) {
            Throwable cause = ite.getCause();
            if (cause instanceof HBCI_Exception) throw (HBCI_Exception) cause;

            throw new HBCI_Exception(
                    HBCIUtilsInternal.getLocMsg("EXCMSG_PASSPORT_INST", name), ite);
    ...
    /**
     * Erzeugt eine Instanz eines HBCI-Passports. Der Typ der erzeugten Passport-Instanz wird
     * hierbei dem Wert des HBCI-Parameters <code>client.passport.default</code> entnommen. Gültige
     * Werte für diesen HBCI-Parameter sind die gleichen wie beim Aufruf der Methode {@link
     * #getInstance(String)}.
     *
    ...
    public static byte[] checkForCryptDataSize(byte[] buffer, int size) {
        ...
        if (buffer.length != size) {
            HBCIUtils.log(
                    "checking for crypted_data_length=="
                            + size
                            + "; current length is "
                            + buffer.length,
                    HBCIUtils.LOG_DEBUG);
            if (buffer.length > size) {
                ...
                for (int i = 0; i < diff; i++) {
                    if (buffer[i] != 0x00) {
                        HBCIUtils.log(
                                "byte "
                                        + i
                                        + " in crypted_data is not zero, but it should be zero - please contact the author",
                                HBCIUtils.LOG_WARN);
                        ok = false;
                    }
                }
                ...
                if (ok) {
                    HBCIUtils.log(
                            "removing " + diff + " unnecessary null-bytes from crypted_data",
                            HBCIUtils.LOG_DEBUG);
                ...
                HBCIUtils.log(
                        "prepending " + diff + " null bytes to crypted_data", HBCIUtils.LOG_WARN);
                result = new byte[size];
    ...
    protected boolean askForMissingData(
            boolean needCountry,
            boolean needBLZ,
            boolean needHost,
            boolean needPort,
        ...
        if (needBLZ && (getBLZ() == null || getBLZ().length() == 0)) {
            ...
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_BLZ,
                            HBCIUtilsInternal.getLocMsg("BLZ"),
                            HBCICallback.TYPE_TEXT,
                            sb);
            if (sb.length() == 0)
                throw new InvalidUserDataException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_EMPTY_X", HBCIUtilsInternal.getLocMsg("BLZ")));
            setBLZ(sb.toString());
            dataChanged = true;
        }
        ...
        if (needCountry && (getCountry() == null || getCountry().length() == 0)) {
            ...
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_COUNTRY,
                            HBCIUtilsInternal.getLocMsg("COUNTRY"),
                            HBCICallback.TYPE_TEXT,
                            sb);
            if (sb.length() == 0)
                throw new InvalidUserDataException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_EMPTY_X", HBCIUtilsInternal.getLocMsg("COUNTRY")));
        ...
        if (needHost && (getHost() == null || getHost().length() == 0)) {
            ...
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_HOST,
                            HBCIUtilsInternal.getLocMsg("HOST"),
                            HBCICallback.TYPE_TEXT,
                            sb);
            if (sb.length() == 0)
                throw new InvalidUserDataException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_EMPTY_X", HBCIUtilsInternal.getLocMsg("HOST")));
            setHost(sb.toString());
            dataChanged = true;
        }
        ...
        if (needPort && (getPort() == null || getPort().intValue() == 0)) {
            StringBuffer sb =
                    new StringBuffer((this instanceof AbstractPinTanPassport) ? "443" : "3000");
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_PORT,
                            HBCIUtilsInternal.getLocMsg("PORT"),
                            HBCICallback.TYPE_TEXT,
                            sb);
            if (sb.length() == 0)
                throw new InvalidUserDataException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_EMPTY_X", HBCIUtilsInternal.getLocMsg("PORT")));
        ...
        if (needFilter && (getFilterType() == null || getFilterType().length() == 0)) {
            ...
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_FILTER,
                            HBCIUtilsInternal.getLocMsg("FILTER"),
                            HBCICallback.TYPE_TEXT,
                            sb);
            if (sb.length() == 0)
                throw new InvalidUserDataException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_EMPTY_X", HBCIUtilsInternal.getLocMsg("FILTER")));
        ...
        if (needUserId && (getUserId() == null || getUserId().length() == 0)) {
            ...
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_USERID,
                            HBCIUtilsInternal.getLocMsg("USERID"),
                            HBCICallback.TYPE_TEXT,
                            sb);
            if (sb.length() == 0)
                throw new InvalidUserDataException(
                        HBCIUtilsInternal.getLocMsg(
                                "EXCMSG_EMPTY_X", HBCIUtilsInternal.getLocMsg("USERID")));
        ...
        if (needCustomerId
            ...
            HBCIUtilsInternal.getCallback()
                    .callback(
                            this,
                            HBCICallback.NEED_CUSTOMERID,
                            HBCIUtilsInternal.getLocMsg("CUSTOMERID"),
                            HBCICallback.TYPE_TEXT,
                            sb);
    ...
    public final void setBLZ(String blz) {
        if (getPostProcessor() == null) setPostProcessor(PostProcessor.getPostProcessor(blz));
        else blz = getPostProcessor().processBLZ(blz);
    ...
    public final void setCountry(String country) {
        if (getPostProcessor() != null) country = getPostProcessor().processCountry(country);
        ...
        if (upd != null) {
            for (int i = 0; ; i++) {
                ...
                if ((st = upd.getProperty(header + ".KLimit.limittype")) != null) {
                    ...
                    limit.value =
                            new Value(
                                    upd.getProperty(header + ".KLimit.BTG.value"),
    ...
    public final void setHost(String host) {
        if (getPostProcessor() != null) host = getPostProcessor().processHost(host);
        this.host = host;
    }
    ...
    public final void setPort(Integer port) {
        if (getPostProcessor() != null) port = getPostProcessor().processPort(port);
        this.port = port;
    }
    ...
    public final void setFilterType(String filter) {
        if (getPostProcessor() != null) filter = getPostProcessor().processFilterType(filter);
    ...
    public final void setUserId(String userid) {
        if (getPostProcessor() != null) userid = getPostProcessor().processUserId(userid);
    ...
    public final String[] getSuppLangs() {
        ...
        if (bpd != null) {
            ...
            if (temp.size() != 0) ret = (temp.toArray(ret));
        }
    ...
    public final boolean canMixSecMethods() {
        ...
        if (bpd != null) {
            ...
            if (value != null && value.equals("J")) ret = true;
        }
    ...
    protected SecretKey calculatePassportKey(boolean forSaving) {
        try {
            ...
            LogFilter.getInstance()
                    .addSecretData(passphrase.toString(), "X", LogFilter.FILTER_SECRETS);

            String provider = HBCIUtils.getParam("kernel.security.provider");
            SecretKeyFactory fac =
                    provider == null
                            ? SecretKeyFactory.getInstance("PBEWithMD5AndDES")
                            : SecretKeyFactory.getInstance("PBEWithMD5AndDES", provider);
    ...
    public Properties getJobRestrictions(String gvname, String version) {
        ...
        for (Enumeration e = bpd.propertyNames(); e.hasMoreElements(); ) {
            ...
            if (key.startsWith("Params") && key.indexOf("." + searchstring + ".Par") != -1) {
                ...
                result.setProperty(
                        key.substring(key.indexOf(".", searchIdx + searchstring.length() + 4) + 1),
                        bpd.getProperty(key));
            }
        }
    ...
    public void setPersistentData(String id, Object o) {
        if (o != null) persistentData.put(id, o);
        else persistentData.remove(id);
    }
    ...
    /**
     * Ersetzt die Datei origFile gegen tmpFile. Nach dem Loeschen der Datei origFile wartet die
     * Methode jedoch maximal 20 Sekunden, um sicherzustellen, dass z.Bsp. Virenscanner die Datei
     * wieder losgelassen haben und sie wirklich verschwunden ist, bevor tmpFile auf den Namen von
     * origFile umbenannt wird. Wichtig ist, dass zum Zeitpunkt des Aufrufes dieser Methode alle
     * Streams auf die Dateien bereits geschlossen wurden. Die Schreibvorgaenge auf die Dateien
     * muessen also abgeschlossen sein. Heisst: "os.close()" nicht erst im finally-Block machen
     * sondern VOR dem Aufruf dieser Methode.
     *
    ...
    protected void safeReplace(File origFile, File tmpFile) {
        ...
        if (origFile.exists()) // Nur loeschen, wenn es ueberhaupt existiert
        {
            ...
            if (!origFile.delete())
                HBCIUtils.log(
                        "delete method for " + origFile + " returned false", HBCIUtils.LOG_ERR);
        }
        ...
        while (origFile.exists() && retry++ < 20) {
            try {
                HBCIUtils.log(
                        "wait a little bit, maybe another thread (antivirus scanner) holds a lock, file still exists",
                        HBCIUtils.LOG_WARN);
                Thread.sleep(1000L);
        ...
        if (origFile.exists()) throw new HBCI_Exception("could not delete " + origFile);

        // Das Rename versuchen wir jetzt auch wiederholt mehrfach
        retry = 0;
        HBCIUtils.log(
                "renaming " + tmpFile.getName() + " to " + origFile.getName(), HBCIUtils.LOG_DEBUG);
        while (!origFile.exists() && retry++ < 20) {
            if (!tmpFile.renameTo(origFile))
                HBCIUtils.log(
                        "rename method for " + tmpFile + " to " + origFile + " returned false",
                        HBCIUtils.LOG_ERR);
            ...
            try {
                HBCIUtils.log(
                        "wait a little bit, maybe another thread (antivirus scanner) holds a lock, file still not renamed",
                        HBCIUtils.LOG_WARN);
                Thread.sleep(1000L);
            } catch (InterruptedException e) {
                HBCIUtils.log("interrupted", HBCIUtils.LOG_WARN);
        ...
        if (!origFile.exists())
            throw new HBCI_Exception(
                    "could not rename " + tmpFile.getName() + " to " + origFile.getName());
    }
