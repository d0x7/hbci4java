...
public abstract class AbstractPinTanPassport extends AbstractHBCIPassport {
    ...
    public void setBPD(Properties p) {
        ...
        if (p != null && p.size() != 0) {
            ...
            int maxAllowedVersion =
                    Integer.parseInt(HBCIUtils.getParam("kernel.gv.HITANS.segversion.max", "0"));
            ...
            for (Enumeration e = p.propertyNames(); e.hasMoreElements(); ) {
                ...
                if (key.startsWith("Params")) {
                    ...
                    if (subkey.startsWith("TAN2StepPar")) {
                        ...
                        // willuhn 2011-05-13 Wir brauchen die Segment-Version, weil mittlerweile
                        // TAN-Verfahren
                        // mit identischer Sicherheitsfunktion in unterschiedlichen
                        // Segment-Versionen auftreten
                        // koennen
                        // Wenn welche mehrfach vorhanden sind, nehmen wir nur das aus der neueren
                        // Version
                        ...
                        if (subkey.startsWith("ParTAN2Step") && subkey.endsWith(".secfunc")) {
                            // willuhn 2011-06-06 Segment-Versionen ueberspringen, die groesser als
                            // die max.
                            // zulaessige sind
                            ...
                            // willuhn 2011-05-13 Checken, ob wir das Verfahren schon aus einer
                            // aktuelleren
                            ...
                            if (prev != null) {
                                ...
                                if (prevVersion > segVersion) {
                                    HBCIUtils.log(
                                            "found another twostepmech "
                                                    + secfunc
                                                    + " in segversion "
                                                    + segVersion
                                                    + ", allready have one in segversion "
                                                    + prevVersion
                                                    + ", ignoring segversion "
                                                    + segVersion,
                                            HBCIUtils.LOG_DEBUG);
                            ...
                            // willuhn 2011-05-13 Wir merken uns die Segment-Version in dem
                            ...
                            for (Enumeration e2 = p.propertyNames(); e2.hasMoreElements(); ) {
                                ...
                                if (key2.startsWith(paramHeader + ".")) {
                                    ...
                                    entry.setProperty(
                                            key2.substring(dotPos + 1), p.getProperty(key2));
                                }
                            }
    ...
    private void searchFor3920s(HBCIRetVal[] rets) {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (ret.code.equals("3920")) {
                ...
                HBCIUtils.log(
                        "autosecfunc: found 3920 in response - updated list of allowed twostepmechs with "
                                + allowedTwostepMechanisms.size()
                                + " entries",
                        HBCIUtils.LOG_DEBUG);
            }
        }
    ...
    private boolean searchFor3072s(HBCIRetVal[] rets) {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (ret.code.equals("3072")) {
                ...
                if (l2 > 0) {
                    HBCIUtils.log(
                            "autosecfunc: found 3072 in response - change user id",
                            HBCIUtils.LOG_DEBUG);
    ...
    public boolean postInitResponseHook(HBCIMsgStatus msgStatus, boolean anonDialog) {
        ...
        if (!msgStatus.isOK()) {
            ...
            if (msgStatus.isInvalidPIN()) {
                HBCIUtils.log(
                        "detected 'invalid PIN' error - clearing passport PIN", HBCIUtils.LOG_INFO);
                clearPIN();

                // Aufrufer informieren, dass falsche PIN eingegeben wurde (um evtl. PIN aus Puffer
                // zu
                // löschen, etc.)
                StringBuffer retData = new StringBuffer();
                HBCIUtilsInternal.getCallback()
        ...
        if (!anonDialog) {
            ...
            if (!oldTANMethod.equals(updatedTANMethod)) {
                ...
                restart_needed = true;
                HBCIUtils.log(
                        "autosecfunc: after this dialog-init we had to change selected pintan method from "
                                + oldTANMethod
                                + " to "
                                + updatedTANMethod
    ...
    public boolean isSupported() {
        ...
        if (bpd != null && bpd.size() != 0) {
            ...
            if (ret) {
                ...
                // autosecmech: hier wird ein flag uebergeben, das anzeigt, dass
                // getCurrentTANMethod()
                ...
                if (current.equals(Sig.SECFUNC_SIG_PT_1STEP)) {
                    ...
                    if (!isOneStepAllowed()) {
                        HBCIUtils.log(
                                "not supported: onestep method not allowed by BPD",
                                HBCIUtils.LOG_ERR);
                        ret = false;
                    } else {
                    ...
                    if (entry == null) {
                        ...
                        HBCIUtils.log(
                                "not supported: twostep-method "
                                        + current
                                        + " selected, but this is not supported",
                                HBCIUtils.LOG_ERR);
                        ret = false;
                    } else {
                        HBCIUtils.log(
                                "selected twostep-method "
                                        + current
                                        + " ("
    ...
    private boolean isOneStepAllowed() {
        ...
        if (bpd != null) {
            for (Enumeration e = bpd.propertyNames(); e.hasMoreElements(); ) {
                ...
                // TODO: willuhn 2011-05-13: Das nimmt einfach den ersten gefundenen Parameter,
                // liefert
                // jedoch faelschlicherweise false, wenn das erste gefundene kein
                // Einschritt-Verfahren ist
    ...
    /**
     * Kann vor <code>new HBCIHandler()</code> aufgerufen werden, um zu erzwingen, dass die Liste
     * der unterstützten PIN/TAN-Sicherheitsverfahren neu vom Server abgeholt wird und evtl. neu vom
     * Nutzer abgefragt wird.
     */
        ...
        if (currentTANMethod == null || recheckSupportedSecMechs) {
            ...
            if (isOneStepAllowed()) {
                ...
                if (allowedTwostepMechanisms.size() == 0
                        || allowedTwostepMechanisms.contains(Sig.SECFUNC_SIG_PT_1STEP)) {
            ...
            String[] secfuncs =
                    twostepMechanisms.keySet().toArray(new String[twostepMechanisms.size()]);
            Arrays.sort(secfuncs);
            int len = secfuncs.length;
            for (int i = 0; i < len; i++) {
                String secfunc = secfuncs[i];
                if (allowedTwostepMechanisms.size() == 0
                        || allowedTwostepMechanisms.contains(secfunc)) {
            ...
            if (options.size() == 1) {
                ...
                HBCIUtils.log(
                        "autosecfunc: there is only one pintan method ("
                                + autoSelection
                                + ") supported - choosing this automatically",
                        HBCIUtils.LOG_DEBUG);
                if (currentTANMethod != null && !autoSelection.equals(currentTANMethod)) {
                    HBCIUtils.log(
                            "autosecfunc: currently selected method ("
                                    + currentTANMethod
                                    + ") differs from auto-selected method ("
                                    + autoSelection
                                    + ")",
                            HBCIUtils.LOG_DEBUG);
                }
                ...
                if (currentTANMethod != null) {
                    ...
                    if (!ok) {
                        HBCIUtils.log(
                                "autosecfunc: currently selected pintan method ("
                                        + currentTANMethod
                                        + ") not in list of supported methods - resetting current selection",
                                HBCIUtils.LOG_DEBUG);
                ...
                if (currentTANMethod == null || this.currentTANMethodWasAutoSelected) {
                    ...
                    if (allowedTwostepMechanisms.size() == 0
                        ...
                        // autosecmech: hier merken, dass dieses verfahren AUTOMATISCH
                        // ausgewaehlt wurde, so dass wir spaeter immer mal wieder pruefen
                        // muessen, ob inzwischen nicht mehr/andere unterstuetzte secmechs bekannt
                        // sind
                        // (passiert z.b. wenn das anonyme abholen der bpd fehlschlaegt)
                        this.currentTANMethodWasAutoSelected = true;
                        ...
                        HBCIUtils.log(
                                "autosecfunc: we have to callback to ask for pintan method to be used",
                                HBCIUtils.LOG_DEBUG);
                        ...
                        HBCIUtils.log(
                                "autosecfunc: manually selected pintan method " + currentTANMethod,
                                HBCIUtils.LOG_DEBUG);
                    }
                }
                ...
                // wir automatisch 999
                HBCIUtils.log(
                        "autosecfunc: absolutely no information about allowed pintan methods available - automatically falling back to 999",
                        HBCIUtils.LOG_DEBUG);
    ...
    private void patchMessagesFor2StepMethods(HBCIDialog dialog) {
        if (!getCurrentTANMethod(false).equals(Sig.SECFUNC_SIG_PT_1STEP)) {
            ...
            HBCIUtils.log(
                    "afterCustomDialogInitHook: patching message queues for twostep method",
                    HBCIUtils.LOG_DEBUG);
            ...
            for (Iterator<ArrayList<HBCIJobImpl>> i = msgs.iterator(); i.hasNext(); ) {
                ...
                for (Iterator<HBCIJobImpl> j = msg_tasks.iterator(); j.hasNext(); ) {
                    ...
                    if (getPinTanInfo(segcode).equals("J")) {
                        ...
                        HBCIUtils.log(
                                "found task that requires a TAN: "
                                        + segcode
                                        + " - have to patch message queue",
                                HBCIUtils.LOG_DEBUG);

                        if (process.equals("1")) {
                            // prozessvariante 1
                            HBCIUtils.log(
                                    "process #1: adding new message with HKTAN(p=1,hash=...) before current message",
                                    HBCIUtils.LOG_DEBUG);
                            ...
                            // muessen wir explizit setzen, damit wir das HKTAN in der gleichen
                            // Version
                            ...
                            if (hktanVersion >= 5) {
                                // Bis HKTAN4/hhd1.3 wurde das noch als Challenge-Parameter
                                // uebermittelt. Jetzt hat
                                // es einen
                                // eigenen Platz in den Job-Parametern
                                hktan.setParam("ordersegcode", task.getHBCICode());

                                // Zitat aus HITANS5: Diese Funktion ermöglicht das Sicherstellen
                                // einer gültigen
                                // Kontoverbindung
                                // z. B. für die Abrechnung von SMS-Kosten bereits vor Erzeugen und
                                // Versenden einer
                                // (ggf. kostenpflichtigen!) TAN.
                                //  0: Auftraggeberkonto darf nicht angegeben werden
                                //  2: Auftraggeberkonto muss angegeben werden, wenn im
                                // Geschäftsvorfall enthalten
                            ...
                            try {
                                ...
                                HBCIUtils.log(
                                        "calculating hash for jobsegment: " + segdata,
                                        HBCIUtils.LOG_DEBUG2);

                                // zu verwendenden Hash-Algorithmus von dem Wert "orderhashmode" aus
                                // den BPD
                                // abhängig machen
                            ...
                            if (secmechInfo.getProperty("needchallengeklass", "N").equals("J")) {
                                HBCIUtils.log(
                                        "we are in PV #1, and a challenge klass is required",
                                        HBCIUtils.LOG_DEBUG);
                            ...
                            // prozessvariante 2
                            HBCIUtils.log(
                                    "process #2: adding new task HKTAN(p=4) to current message",
                                    HBCIUtils.LOG_DEBUG);
                            ...
                            HBCIUtils.log(
                                    "creating new msg with HKTAN(p=2,orderref=DELAYED)",
                                    HBCIUtils.LOG_DEBUG);
                            additional_msg_tasks = new ArrayList<HBCIJobImpl>();
                            ...
                            // willuhn 2011-05-09 TAN-Media gibts nur bei Prozess 1,3,4 - also nicht
                            // in hktan2
                            ...
                            HBCIUtils.log(
                                    "storing reference to this HKTAN in previous HKTAN segment",
                                    HBCIUtils.LOG_DEBUG);
                            ...
                            HBCIUtils.log(
                                    "storing reference to original job in new HKTAN segment",
                                    HBCIUtils.LOG_DEBUG);
                            hktan2.storeOriginalTask(task);

                            // die neue msg wird später (nach der aktuellen) zur msg-queue
                            // hinzugefügt
                        }
                    } else {
                        // kein tan-pflichtiger task, also einfach zur gepatchten msg-queue
                        // hinzufügen
                        HBCIUtils.log(
                                "found task that does not require a TAN: "
                                        + segcode
                                        + " - adding it to current msg",
                                HBCIUtils.LOG_DEBUG);
                        new_msg_tasks.add(task);
                    }
                }
                ...
                if (additional_msg_tasks != null) {
                    ...
                    HBCIUtils.log(
                            "adding newly created message with HKTAN(p=2) after current one",
                            HBCIUtils.LOG_DEBUG);
                    new_msgs.add(additional_msg_tasks);
                    additional_msg_tasks = null;
                }
    ...
    private void applyTanMedia(GVTAN2Step hktan) {
        if (hktan == null) return;

        // Gibts erst ab hhd1.3, siehe
        // FinTS_3.0_Security_Sicherheitsverfahren_PINTAN_Rel_20101027_final_version.pdf, Kapitel
        // B.4.3.1.1.1
        ...
        if (hktan_version >= 3) {
            ...
            HBCIUtils.log(
                    "nofactivetanmedia: " + num + ", needtanmedia: " + needed, HBCIUtils.LOG_DEBUG);
    ...
    /**
     * Ueberschrieben, um das "https://" am Anfang automatisch abzuschneiden. Das sorgte schon fuer
     * so viele unnoetige Fehler.
     *
    ...
    public String getHost() {
        ...
        if (host == null || host.length() == 0 || !host.startsWith("https://")) return host;
