...
/**
 * Passport-Klasse für RDH-Zugänge mit Sicherheitsmedium "Datei". Bei dieser Variante werden sowohl
 * die HBCI-Zugangsdaten wie auch die kryptografischen Schlüssel für die Signierung/Verschlüsselung
 * der HBCI-Nachrichten in einer Datei gespeichert. Der Dateiname kann dabei beliebig vorgegeben
 * werden. Da diese Datei vertrauliche Informationen enthält, wird der Inhalt verschlüsselt
 * abgespeichert. Vor dem Erzeugen bzw. Einlesen wird via Callback-Mechanismus nach einem Passwort
 * gefragt, aus dem der Schlüssel zur Verschlüsselung/Entschlüsselung der Schlüsseldatei berechnet
 * wird.
 *
 * <p>Wie auch bei {@link org.kapott.hbci.passport.HBCIPassportDDV} werden in der Schlüsseldatei
 * zusätzliche Informationen gespeichert. Dazu gehören u.a. die BPD und die UPD sowie die
 * HBCI-Version, die zuletzt mit diesem Passport benutzt wurde. Im Gegensatz zu den "Hilfsdateien"
 * bei DDV-Passports darf die Schlüsseldatei bei RDH-Passports aber niemals manuell gelöscht werden,
 * da dabei auch die kryptografischen Schlüssel des Kunden verlorengehen. Diese können nicht wieder
 * hergestellt werden, so dass in einem solchen Fall ein manuelles Zurücksetzes des HBCI-Zuganges
 * bei der Bank erfolgen muss!
 *
 * <p>Die Schlüsseldateien, die <em>HBCI4Java</em> mit dieser Klasse erzeugt und verwaltet, sind
 * <b>nicht kompatibel</b> zu den Schlüsseldateien anderer HBCI-Software (z.B. VR-NetWorld o.ä.). Es
 * ist also nicht möglich, durch Auswahl des Sicherheitsverfahrens "RDH" oder "RDHNew" und Angabe
 * einer schon existierenden Schlüsseldatei, die mit einer anderen HBCI-Software erstellt wurde,
 * diese Schlüsseldatei unter <em>HBCI4Java</em> zu benutzen! Es ist jedoch im Prinzip möglich, mit
 * der "anderen" Software die Kundenschlüssel sperren zu lassen und anschließend mit
 * <em>HBCI4Java</em> eine völlig neue Schlüsseldatei zu erzeugen. Das hat aber zwei Nachteile: Zum
 * einen muss nach dem Neuerzeugen der Schlüsseldatei auch ein neuer INI-Brief erzeugt und an die
 * Bank gesandt werden, um die neuen Schlüssel freischalten zu lassen. Außerdem lässt sich natürlich
 * die <em>HBCI4Java</em>-Schlüsseldatei nicht mehr in der "anderen" HBCI-Software benutzen. Ein
 * Parallel-Betrieb verschiedener HBCI-Softwarelösungen, die alle auf dem RDH-Verfahren mit
 * Sicherheitsmedium "Datei" (oder Diskette) basieren, ist meines Wissens nicht möglich.
 *
 * <p>Ein weiterer Ausweg aus diesem Problem wäre, eine technische Beschreibung des Formates der
 * Schlüsseldateien der "anderen" HBCI-Software zu besorgen und diese dem <a
 * href="mailto:hbci4java@kapott.org">Autor</a> zukommen zu lassen, damit eine Passport-Variante
...
public class HBCIPassportRDHNew extends AbstractRDHSWFileBasedPassport {
    ...
    public HBCIPassportRDHNew(Object initObject) {
        ...
        if (init) {
            ...
            try {
                ...
                int retries =
                        Integer.parseInt(HBCIUtils.getParam("client.retries.passphrase", "3"));
                ...
                while (true) { // loop for entering the correct passphrase
                    ...
                    PBEParameterSpec paramspec =
                            new PBEParameterSpec(CIPHER_SALT, CIPHER_ITERATIONS);
                    ...
                    try {
                        ...
                        // willuhn 2011-05-25 Wir lassen einen erneuten Versuch nur bei einer der
                        // beiden
                        // folgenden Exceptions zu.
                        // Die "CharConversionException" ist in der Praxis eine
                        // "
                        // com.sun.org.apache.xerces.internal.impl.io.MalformedByteSequenceException: Invalid
                        // byte 2 of 2-byte UTF-8 sequence."
                        // Sie fliegt in "db.parse()". Sprich: Der CipherInputStream kann nicht
                        // erkennen,
    ...
    protected String getElementValue(Element root, String name) {
        ...
        if (list != null && list.getLength() != 0) {
            ...
            if (content != null) ret = content.getNodeValue();
        }
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (n.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (keynode.getAttribute("owner").equals(owner)
                    ...
                    if (part.equals("public")) {
                        RSAPublicKeySpec spec =
                                new RSAPublicKeySpec(
                                        new BigInteger(getElementValue(keynode, "modulus")),
                        ...
                        if (privexponent == null) {
                            // only CRT
                            HBCIUtils.log(
                                    "private " + type + " key is CRT-only", HBCIUtils.LOG_DEBUG);
                            key =
                                    new RSAPrivateCrtKey2(
                                            new BigInteger(p),
                                            new BigInteger(q),
                                            new BigInteger(dP),
                                            new BigInteger(dQ),
                            ...
                            // only exponent
                            HBCIUtils.log(
                                    "private " + type + " key is exponent-only",
                                    HBCIUtils.LOG_DEBUG);
                            RSAPrivateKeySpec spec =
                                    new RSAPrivateKeySpec(
                                            new BigInteger(modulus), new BigInteger(privexponent));
                            key = KeyFactory.getInstance("RSA").generatePrivate(spec);
                        } else {
                            // complete data
                            HBCIUtils.log(
                                    "private " + type + " key is fully specified",
                                    HBCIUtils.LOG_DEBUG);
                    ...
                    ret =
                            new HBCIKey(
                                    getElementValue(keynode, "country"),
    ...
    protected void createPropsElement(Document doc, Element root, String elemName, Properties p) {
        if (p != null) {
            ...
            for (Enumeration e = p.propertyNames(); e.hasMoreElements(); ) {
                ...
                data.setAttribute(
                        "value",
                        notNull(value)); // Der Wert kann bei Properties eigentlich nicht null sein.
                base.appendChild(data);
            }
        }
    }

    protected void createKeyElement(
            Document doc, Element root, String owner, String type, String part, HBCIKey key) {
        if (key != null) {
            ...
            if (part.equals("public") && key.key != null) {
                createElement(
                        doc, keydata, "modulus", ((RSAPublicKey) key.key).getModulus().toString());
                createElement(
                        doc,
                        keydata,
                        "exponent",
                        ((RSAPublicKey) key.key).getPublicExponent().toString());
            } else {
                if (key.key instanceof RSAPrivateCrtKey) {
                    HBCIUtils.log(
                            "saving " + type + " key as fully specified", HBCIUtils.LOG_DEBUG);
                    createElement(
                            doc,
                            keydata,
                            "modulus",
                            ((RSAPrivateCrtKey) key.key).getModulus().toString());
                    createElement(
                            doc,
                            keydata,
                            "exponent",
                            ((RSAPrivateCrtKey) key.key).getPrivateExponent().toString());
                    createElement(
                            doc,
                            keydata,
                            "pubexponent",
                            ((RSAPrivateCrtKey) key.key).getPublicExponent().toString());
                    createElement(
                            doc, keydata, "p", ((RSAPrivateCrtKey) key.key).getPrimeP().toString());
                    createElement(
                            doc, keydata, "q", ((RSAPrivateCrtKey) key.key).getPrimeQ().toString());
                    createElement(
                            doc,
                            keydata,
                            "dP",
                            ((RSAPrivateCrtKey) key.key).getPrimeExponentP().toString());
                    createElement(
                            doc,
                            keydata,
                            "dQ",
                            ((RSAPrivateCrtKey) key.key).getPrimeExponentQ().toString());
                    createElement(
                            doc,
                            keydata,
                            "qInv",
                            ((RSAPrivateCrtKey) key.key).getCrtCoefficient().toString());
                    ...
                    createElement(
                            ...
                            keydata,
                            "modulus",
                            ((RSAPrivateKey) key.key).getModulus().toString());
                    createElement(
                            doc,
                            keydata,
                            "exponent",
                            ((RSAPrivateKey) key.key).getPrivateExponent().toString());
                } else if (key.key instanceof RSAPrivateCrtKey2) {
                    HBCIUtils.log("saving " + type + " key as crt-only", HBCIUtils.LOG_DEBUG);
                    createElement(
                            doc, keydata, "p", ((RSAPrivateCrtKey2) key.key).getP().toString());
                    createElement(
                            doc, keydata, "q", ((RSAPrivateCrtKey2) key.key).getQ().toString());
                    createElement(
                            doc, keydata, "dP", ((RSAPrivateCrtKey2) key.key).getdP().toString());
                    createElement(
                            doc, keydata, "dQ", ((RSAPrivateCrtKey2) key.key).getdQ().toString());
                    createElement(
                            doc,
                            keydata,
                            "qInv",
                            ((RSAPrivateCrtKey2) key.key).getQInv().toString());
                } else {
                    HBCIUtils.log(
                            "key has none of the known types - please contact the author!",
                            HBCIUtils.LOG_WARN);
                }
            }
        }
    ...
    public String getProfileVersion() {
        ...
        if (ret.length() == 0) {
            HBCIUtils.log(
                    "have to determine my rdh-profile-version, but have no information about it yet",
                    HBCIUtils.LOG_DEBUG);
            ...
            if (hasMySigKey()) {
                HBCIUtils.log(
                        "found user sig key in passport file, but no profile version, "
                ...
                HBCIUtils.log(
                        "no user keys found in passport - so we use the highest available profile",
                        HBCIUtils.LOG_DEBUG);
                ...
                HBCIUtils.log(
                        "using RDH profile " + ret + " taken from supported profiles (BPD)",
                        HBCIUtils.LOG_DEBUG);
            }
        }
        ...
        if (ret == null || ret.length() == 0) {
            ...
            HBCIUtils.log(
                    "unable to determine rdh-profile-version using BPD, using default version "
                            + ret,
                    HBCIUtils.LOG_WARN);
        }
