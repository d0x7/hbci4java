...
/**
 * Passport-Klasse für HBCI mit PIN/TAN. Dieses Sicherheitsverfahren wird erst in FinTS 3.0
 * spezifiziert, von einigen Banken aber schon mit früheren HBCI-Versionen angeboten.
 *
 * <p>Bei diesem Verfahren werden die Nachrichten auf HBCI-Ebene nicht mit kryptografischen
 * Verfahren signiert oder verschlüsselt. Als "Signatur" werden statt dessen TANs zusammen mit einer
 * PIN verwendet. Die PIN wird dabei in <em>jeder</em> HBCI-Nachricht als Teil der "Signatur"
 * eingefügt, doch nicht alle Nachrichten benötigen eine TAN. Eine TAN wird nur bei der Übermittlung
 * bestimmter Geschäftsvorfälle benötigt. Welche GV das konkret sind, ermittelt <em>HBCI4Java</em>
 * automatisch aus den BPD. Für jeden GV, der eine TAN benötigt, wird diese via Callback abgefragt
 * und in die Nachricht eingefügt.
 *
 * <p>Die Verschlüsselung der Nachrichten bei der Übertragung erfolgt auf einer höheren
 * Transportschicht. Die Nachrichten werden nämlich nicht direkt via TCP/IP übertragen, sondern in
 * das HTTP-Protokoll eingebettet. Die Verschlüsselung der übertragenen Daten erfolgt dabei auf
 * HTTP-Ebene (via SSL = HTTPS).
 *
 * <p>Wie auch bei {@link org.kapott.hbci.passport.HBCIPassportRDH} wird eine "Schlüsseldatei"
 * verwendet. In dieser werden allerdings keine kryptografischen Schlüssel abgelegt, sondern
 * lediglich die Zugangsdaten für den HBCI-Server (Hostadresse, Nutzerkennung, usw.) sowie einige
 * zusätzliche Daten (BPD, UPD, zuletzt benutzte HBCI-Version). Diese Datei wird vor dem Abspeichern
 * verschlüsselt. Vor dem Erzeugen bzw. erstmaligen Einlesen wird via Callback nach einem Passwort
 * gefragt, aus welchem der Schlüssel für die Verschlüsselung der Datei berechnet wird
 */
public class HBCIPassportPinTan extends AbstractPinTanPassport {
    ...
    public HBCIPassportPinTan(Object initObject) {
        ...
        if (init) {
            ...
            if (askForMissingData(true, true, true, true, true, true, true)) saveChanges();
        }
    }
    ...
    /**
     * Erzeugt die Passport-Datei wenn noetig. In eine extra Funktion ausgelagert, damit es von
     * abgeleiteten Klassen ueberschrieben werden kann.
     */
    ...
    /**
     * Liest die Daten aus der Passport-Datei ein. In eine extra Funktion ausgelagert, damit es von
     * abgeleiteten Klassen ueberschrieben werden kann. Zum Beispiel, um eine andere Art der
     * Persistierung zu implementieren.
     */
    protected void read() {
        create();
        ...
        try {
            ...
            while (true) {
                if (passportKey == null) passportKey = calculatePassportKey(FOR_LOAD);
                ...
                try {
                    o =
                            new ObjectInputStream(
                                    new CipherInputStream(new FileInputStream(fname), cipher));
            ...
            try {
                ...
                try {
                    ...
                    HBCIUtils.log(
                            "no current secmech found in passport file - automatically upgrading to new file format",
                            HBCIUtils.LOG_WARN);
                }
            } catch (Exception e) {
                HBCIUtils.log(
                        "no list of allowed secmechs found in passport file - automatically upgrading to new file format",
                        HBCIUtils.LOG_WARN);
            }
        } catch (Exception e) {
            ...
            if (pintanMethod.equals(Sig.SECFUNC_SIG_PT_1STEP)) {
                ...
                while (tok.hasMoreTokens()) {
                    ...
                    if (info.equals("J")) {
                        ...
                        HBCIUtils.log(
                                "the job with the code " + code + " needs a TAN",
                                HBCIUtils.LOG_DEBUG);
                        ...
                        if (tan.length() == 0) {
                            ...
                            HBCIUtils.log(
                                    "there should be only one job that needs a TAN!",
                                    HBCIUtils.LOG_WARN);
                        }

                    } else if (info.equals("N")) {
                        HBCIUtils.log(
                                "the job with the code " + code + " does not need a TAN",
                                HBCIUtils.LOG_DEBUG);
                ...
                HBCIUtils.log(
                        "twostep method - checking passport(challenge) to decide whether or not we need a TAN",
                        HBCIUtils.LOG_DEBUG);
                ...
                if (challenge == null) {
                    ...
                    HBCIUtils.log(
                            "will not sign with a TAN, because there is no challenge",
                            HBCIUtils.LOG_DEBUG);
                } else {
                    HBCIUtils.log(
                    ...
                    setPersistentData(
                            "pintan_challenge_hhd_uc",
                            null); // gleich wieder aus dem Passport loeschen
                    ...
                    if (hhd.getType() == Type.PHOTOTAN) {
                        ...
                        if (flicker != null) payload.append(flicker);
    ...
    /**
     * Versucht, aus Challenge und Challenge HHDuc den Flicker-Code zu extrahieren und ihn in einen
     * flickerfaehigen Code umzuwandeln. Nur wenn tatsaechlich ein gueltiger Code enthalten ist, der
     * als HHDuc-Code geparst und in einen Flicker-Code umgewandelt werden konnte, liefert die
     * Funktion den Code. Sonst immer NULL.
     *
     * @param challenge der Challenge-Text. Das DE "Challenge HHDuc" gibt es erst seit HITAN4.
     *     Einige Banken haben aber schon vorher optisches chipTAN gemacht. Die haben das HHDuc dann
     *     direkt im Freitext des Challenge mitgeschickt (mit String-Tokens zum Extrahieren
     *     markiert). Die werden vom FlickerCode-Parser auch unterstuetzt.
    ...
    private String parseFlickercode(String challenge, String hhduc) {
        ...
        if (hhduc != null && hhduc.trim().length() > 0) {
            try {
                ...
                HBCIUtils.log(
                        "unable to parse Challenge HHDuc "
                                + hhduc
                                + ":"
                                + HBCIUtils.exception2String(e),
                        HBCIUtils.LOG_DEBUG);
            }
        }
        ...
        if (challenge != null && challenge.trim().length() > 0) {
            try {
                ...
                // chipTAN- und smsTAN Verfahren verwendet wird, wo gar kein Flicker-Code enthalten
                // ist.
                ...
                HBCIUtils.log(
                        "challenge contains no HHDuc (no problem in most cases):"
                                + HBCIUtils.exception2String(e),
    ...
    public byte[][] encrypt(byte[] plainMsg) {
        try {
            ...
            byte[] encrypted =
                    new String(plainMsg, 0, plainMsg.length - padLength, "ISO-8859-1")
                            .getBytes("ISO-8859-1");
