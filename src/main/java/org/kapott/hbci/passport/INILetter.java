...
/**
 * Hilfsklasse für das Erzeugen von INI-Briefen (für RDH-Zugänge). Diese Klasse ermöglicht das
 * Erzeugen von INI-Briefen. Dazu werden Methoden bereitgestellt, mit deren Hilfe die für einen
 * INI-Brief benötigten Daten ermittelt werden können. Außerdem liefert die {@link
 * #toString()}-Methode einen vorgefertigten INI-Brief (kann als Vorlage benutzt werden).
 */
public class INILetter {
    /**
     * INI-Brief für Institutsschlüssel (wird für Vergleich mit tatsächlichem INI-Brief von der Bank
     * benötigt)
     */
    public static final int TYPE_INST = 1;
    /**
     * INI-Brief für Nutzerschlüssel erzeugen (muss nach dem Erstellen neuer Schlüssel an die Bank
     * versandt werden)
     */
     ...
     *
     * @param passport das Passport-Objekt (entspricht einem HBCI-Zugang), für den ein INI-Brief
     *     benötigt wird
     * @param type gibt an, für welche Schlüssel aus dem <code>passport</code> der INI-Brief
     *     benötigt wird ({@link #TYPE_INST} für die Bankschlüssel, {@link #TYPE_USER} für die
    ...
    public INILetter(HBCIPassport passport, int type) {
        ...
        if (type == TYPE_INST) {
            ...
            if (hbcikey == null) hbcikey = passport.getInstEncKey();
        } else {
    ...
    public static byte[] formatKeyData(BigInteger x, int minsize) {
        ...
        System.arraycopy(
                xArray, xArray.length - realbytes, retArray, finalsize - realbytes, realbytes);
    ...
    public byte[] getKeyHashDisplay() {
        try {
            ...
            MessageDigest dig;
            SignatureParamSpec hashSpec =
                    ((AbstractRDHPassport) (passport)).getSignatureParamSpec();
    ...
    public String toString() {
        ...
        out.println(
                "Sicherheitsprofil:           "
                        + passport.getProfileMethod()
                        + " "
                        + passport.getProfileVersion());
        out.println();
        out.println();
        ...
        for (int line = 0; true; line++) {
            if (line * 16 * 3 >= st.length()) break;
            out.println(
                    "    "
                            + st.substring(
                                    line * (16 * 3), Math.min((line + 1) * 16 * 3, st.length())));
        }
        ...
        for (int line = 0; true; line++) {
            if (line * 10 * 3 >= st.length()) break;
            out.println(
                    "    "
                            + st.substring(
                                    line * (10 * 3), Math.min((line + 1) * 10 * 3, st.length())));
        }
