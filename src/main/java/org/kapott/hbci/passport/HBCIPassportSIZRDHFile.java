 ...
 * SIZ-RDH-Schlüsseldateien sind Schlüsseldateien für RDH-Zugänge, die von anderer HBCI-Software
 * erzeugt und verwendet werden (z.B. von <em>StarMoney</em>). Soll eine solche Schlüsseldatei
 * sowohl mit der anderen Software als auch mit <em>HBCI4Java</em> verwendet werden, so kann das mit
 * dieser Passport-Variante geschehen. Soll die Schlüsseldatei nur noch mit <em>HBCI4Java</em>
 * benutzt werden, so ist eine Konvertierung der Schlüsseldatei in ein anderes Passport-Format
 * (vorzugsweise <code>RDHNew</code>) zu empfehlen. Dazu kann der <em>HBCI4Java Passport Editor</em>
 * oder das Tool {@link org.kapott.hbci.tools.ConvertSIZRDHPassport} verwendet werden.
 *
 ...
 * Bibliothek zur Laufzeit benötigt. Diese Bibliothek steht nur als Binary-Version unter <a
 * href="http://hbci4java.kapott.org#download">http://hbci4java.kapott.org</a> zur Verfügung. Erst
 * nach Installation dieser Bibliothek können SIZRDHFile-Passports benutzt werden.
 *
 * <p><b>Siehe auch die Datei <code>README.SIZRDHFile</code>.</b>
 */
            ...
            try {
                if (passphrase == null) {
                    ...
                    LogFilter.getInstance()
                            .addSecretData(retData.toString(), "X", LogFilter.FILTER_SECRETS);
                ...
                if (askForMissingData(false, true, true, false, false, true, true)) saveChanges();
            } catch (Exception e) {
