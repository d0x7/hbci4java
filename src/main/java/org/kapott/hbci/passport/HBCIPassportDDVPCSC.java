...
/**
 * Implementierung eines DDV-Passports, welcher intern die neue Chipkarten-API "javax.smartcardio"
 * von Java 6 verwendet. Die Implementierung basiert auf dem OCF-Code von HBCI4Java 2.5.8.
 */
public class HBCIPassportDDVPCSC extends HBCIPassportDDV {
    ...
    public HBCIPassportDDVPCSC(Object init) {
        ...
        try {
            ...
            this.setUseBio(
                    Integer.parseInt(HBCIUtils.getParam(getParamHeader() + ".usebio", "-1")));
            this.setUseSoftPin(
                    Integer.parseInt(HBCIUtils.getParam(getParamHeader() + ".softpin", "-1")));
            this.setSoftPin(new byte[0]);
            this.setPINEntered(false);
            this.setEntryIdx(
                    Integer.parseInt(HBCIUtils.getParam(getParamHeader() + ".entryidx", "1")));
            //
            ////////////////////////////////////////////////////////////////////////
            ...
            this.setFileName(
                    HBCIUtilsInternal.withCounter(path + "pcsc" + getCardId(), getEntryIdx() - 1));
            ...
            if (file.exists() && file.isFile() && file.canRead()) {
                ...
                while (true) // loop for entering the correct passphrase
                {
                    ...
                    try {
                        ...
                        throw new HBCI_Exception(
                                HBCIUtilsInternal.getLocMsg("EXCMSG_PASSPORT_READERR"), e2);
                    }
    ...
    protected void ctReadKeyData() {
        ...
        this.setInstSigKey(
                new HBCIKey(
                        ...
                        getBLZ(),
                        getUserId(),
                        Integer.toString(keyData[0].num),
                        Integer.toString(keyData[0].version),
                        null));
        ...
        this.setInstEncKey(
                new HBCIKey(
                        ...
                        getBLZ(),
                        getUserId(),
                        Integer.toString(keyData[1].num),
                        Integer.toString(keyData[1].version),
                        null));
    }

    /** @see org.kapott.hbci.passport.HBCIPassportDDV#ctEnterPIN() */
    protected void ctEnterPIN() {
        if (getUseSoftPin() == 1) this.cardService.verifySoftPIN(1, this.getSoftPin());
        else this.cardService.verifyHardPIN(1);
    }
