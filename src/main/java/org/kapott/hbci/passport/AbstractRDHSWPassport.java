...
public abstract class AbstractRDHSWPassport extends AbstractRDHPassport {
    ...
    private byte[] encryptKey(SecretKey msgkey) {
        try {
            ...
            BigInteger c =
                    m.modPow(
                            ((RSAPublicKey) (k)).getPublicExponent(),
    ...
    public byte[] decrypt(byte[] cryptedKey, byte[] cryptedMsg) {
        try {
            ...
            if (k instanceof RSAPrivateKey) {
                ...
                HBCIUtils.log(
                        "decrypting message key with (p,q,dP,dQ,qInv)-algorithm",
                        HBCIUtils.LOG_DEBUG);
        ...
        switch (profile) {
            ...
            default:
                throw new HBCI_Exception(
                        "*** dont know which keysize to use for profile rdh-" + profile);
        }

        HBCIUtils.log(
                "using keysize " + ret + " bits for newly generated keys", HBCIUtils.LOG_DEBUG);
        return ret;
    }
    ...
    public HBCIKey[][] generateNewUserKeys() {
        ...
        try {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (i == 0) {
                    newSigKey[0] =
                            new HBCIKey(country, blz, userid, num, version, pair.getPublic());
                    newSigKey[1] =
                            new HBCIKey(country, blz, userid, num, version, pair.getPrivate());
                } else {
                    newEncKey[0] =
                            new HBCIKey(country, blz, userid, num, version, pair.getPublic());
                    newEncKey[1] =
