...
/**
 * Passport-Klasse für Sicherheitsverfahren DDV mit Medium Chipkarte. Bei dieser Variante gibt die
 * Bank eine Chipkarte aus, auf der die Zugangsdaten des Nutzers für den HBCI-Zugang gespeichert
 * sind. Außerdem befinden sich auf der Karte die (symmetrischen) Schlüssel für die Erzeugung der
 * Signaturen und für die Verschlüsselung der Nachrichten.
 *
 * <p>Diese Klasse unterstützt DDV-Chipkarten vom Typ 0 und 1. Auf einer DDV-Chipkarte können
 * prinzipiell bis zu fünf HBCI-Zugangsdatensätze (für unterschiedliche Banken) gespeichert werden.
 * Diese Klasse ermöglicht die Benutzung eines beliebigen dieser Datensätze. Das hat aber in der
 * Praxis kaum Relevanz, weil dann alle HBCI-Zugänge die gleichen kryptografischen Schlüssel
 * benutzen müssten (es gibt nur ein Schlüsselpaar pro Chipkarte). Für Chipkarten, die von
 * Betreibern für HBCI-Testzugängen ausgegeben werden, ist diese Option jedoch nützlich, da hier
 * häufig tatsächlich mehrere Zugänge existieren und diese Zugangsdaten auf einer einzigen Chipkarte
 * gespeichert werden können.
 *
 * <p>Prinzipiell benötigt diese Passport-Variante also keine zusätzliche Schlüsseldatei, da alle
 * <em>benötigten</em> HBCI-Daten auf der Chipkarte gespeichert sind. Dennoch verwendet diese Klasse
 * eine zusätzliche Datei. In dieser werden u.a. die zuletzt empfangenen BPD und UPD sowie die
 * zuletzt benutzte HBCI-Version gespeichert, um beim nächsten Benutzen dieses HBCI-Zuganges diese
 * Daten nicht erneut abfragen zu müssen. Diese zusätzliche Datei wird automatisch angelegt, der
 * Dateiname setzt sich aus einem definierbaren Prefix (Pfad) und der Seriennummer der Chipkarte
 * zusammen.
 */
public class HBCIPassportDDV extends AbstractDDVPassport {
    ...
    public HBCIPassportDDV(Object init) {
        ...
        // init card
        HBCIUtils.log(
                "using chipcard terminal with port " + comport + " and terminal number " + ctnumber,
                HBCIUtils.LOG_DEBUG);
        try {
        ...
        try {
            ...
            if (askForMissingData(true, true, true, false, false, true, false)) saveBankData();
                        ...
                        o =
                                new ObjectInputStream(
                                        new CipherInputStream(
                                                new FileInputStream(getFileName()), cipher));
     ...
     * Dieser Wert wird vom CTAPI-Treiber des jeweils verwendeten Chipkartenterminals interpretiert.
     *
    ...
    /**
     * Gibt die logische Nummer zurück, unter der der Chipkartenleser zu verwenden ist. Wird nur ein
     * Chipkartenleser verwendet, so kann dieser Wert immer 0 sein. Bei gleichzeitiger Verwendung
     * mehrerer Chipkartenleser sollten die einzelnen Leser hier unterschiedliche Werte zugewiesen
     * bekommen. Dieser Wert wird vom CTAPI-Treiber benutzt, um die Chipkartenleser intern
    ...
    /**
     * Gibt zurück, ob zur PIN-Eingabe am Chipkartenterminal das Biometric-Interface verwendet
     * werden soll. Diese Funktion steht zur Zeit nur für Reiner-SCT- Chipkartenterminals zur
     * Verfügung.
     *
     * @return <code>1</code>, wenn die Biometrie-Einheit des Chipkartenterminals für die
     *     PIN-Eingabe benutzt werden soll; <code>0</code>, wenn die Biometrie-Einheit nicht benutzt
     *     werden soll, oder <code>-1</code>, wenn die Verwendung der Biometrie-Einheit automatisch
     *     erkannt werden soll.
     */
     ...
     * oder über die PC-Tastatur erfolgen soll. Dieser Wert wird benutzt, um die PIN-Eingabe sowohl
     * bei Klasse-2-Lesern mit eigener Tastatur wir auch für Klasse-1-Leser ohne separate Tastatur
     * zu ermöglichen.
     *
     * @return PIN-Eingabe über welche Tastatur
     *     <ul>
     *       <li>=0 PIN-Eingabe zwingend über Terminal-Keypad
     *       <li>=1 PIN-Eingabe zwingend über PC-Tastatur
     *       <li>=-1 automatische Erkennung, ob bevorzugtes Chipkarten-Terminal-Keypad verfügbar ist
     *     </ul>
     */
     ...
     * benutzt werden sollen. Auf einer Chipkarte können bis zu fünf Zugangsdatensätze gespeichert
     * sein, dieser Wert enthält die Nummer des benutzten Eintrages (von 1-5). Normalerweise wird
     * der Eintrag Nummer 1 (welcher auch meist der einzige Eintrag ist) verwendet.
     *
