...
public final class RIPEMD160 extends MessageDigestSpi {
    private static final int[] Ks = {0x00000000, 0x5a827999, 0x6ed9eba1, 0x8f1bbcdc, 0xa953fd4e};
    private static final int[] K2s = {0x50a28be6, 0x5c4dd124, 0x6d703ef3, 0x7a6d76e9, 0x00000000};
    ...
    private int f(byte j, int x, int y, int z) {
        ...
        if (j >= 0 && j <= 15) ret = (x ^ y ^ z);
        else if (j >= 16 && j <= 31) ret = ((x & y) | (~x & z));
        else if (j >= 32 && j <= 47) ret = ((x | ~y) ^ z);
        else if (j >= 48 && j <= 63) ret = ((x & z) | (y & ~z));
        else if (j >= 64 && j <= 79) ret = (x ^ (y | ~z));
        else ret = -1;
