...
public class ISO9796p1 extends SignatureSpi {
    ...
    private static byte[] getISfromSig(byte[] sig, byte[] exp, byte[] mod) {
        return (new BigInteger(+1, sig))
                .modPow(new BigInteger(+1, exp), new BigInteger(+1, mod))
                .toByteArray();
    }
    ...
    private static byte[] getIRfromIS(byte[] is, byte[] exp, byte[] mod, int[] ks)
        ...
        if (is_b.mod(new BigInteger("16")).equals(new BigInteger("6"))) ret = is_b;
    ...
    private static byte[] getMRfromIR(byte[] ir, int k, int[] ts) {
        ...
        for (int i = 0; i < 2 * t; i++) {
            ...
            if (bitsum >= k) mr[2 * t - 1 - i] &= (1 << (7 - (bitsum - k))) - 1;
        }

        mr[2 * t - 1] =
                (byte) ((Perm1((mr[2 * t - 2] >> 4) & 0x0F) << 4) | ((mr[2 * t - 1] >> 4) & 0x0F));
    ...
    private static byte[] getMPfromMR(byte[] mr, int t, int[] zs, int[] rs)
        ...
        if (rs[0] < 1 || rs[0] > 8) throw new SignatureException("r is not in range 1..8");
    ...
    private static byte[] getSigFromIS(byte[] is, byte[] modulus) {
        ...
        if (bIS.compareTo(bIS2) < 0) bSig = bIS;
        else bSig = bIS2;
    ...
    protected void engineSetParameter(AlgorithmParameterSpec param1)
        ...
        if (param1 instanceof SignatureParamSpec) this.param = (SignatureParamSpec) (param1);
        else {
